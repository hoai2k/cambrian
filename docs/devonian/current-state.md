...
5. **Attack/eating pass: delivered 9 September 2026** for every entry except Odaraia and Nahecaris (rebuilding), as previews awaiting the user's review — see `docs/attack-feeding-refinement.md` and `tools/attack-feeding-refinements.json`. What remains of it is the review itself, a controller playtest, and attack clips on the LODs.
