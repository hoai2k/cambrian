...
| [06 · The shore comes and goes](06-shore-visitors.md) | Plan, not built: Tanystropheus fishing at the bank with its neck down over the water, the runners (Macrocnemus, Coelophysis) waiting inland and dashing into the shallows for anything that stays still, and shore animals that arrive and leave on a schedule. Written as a delta against what `shore.ts` already does — a stillness trigger, size gates, a five-phase excursion and an occupancy schedule — with the one clip that needs authoring (`Fish`) and the eight steps to build it. |
