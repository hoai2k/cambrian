...
| 2 | Cartorhynchus | One front fin mushed rather than moving as a whole | Kabsch rigid fit per frame over the vertices each fore-blade bone dominates, RMS residual over L, worst phase of `Sprint`: L `upper` 3.06 % / `mid` 3.70 % / `tip` 4.20 %; R 3.22 % / 3.93 % / 5.15 %. Whole chain: L 2.21 %, R 3.18 % (`Swim` 1.72 / 2.59 %). Rest shape is mirror-symmetric (mirrored-L→R nearest vertex mean 0.50 % L, max 1.24 %), swept angles identical L/R (537° per `Sprint` cycle), but the R chain carries 18 % more weight (1358 vs 1150) and blends 232 of it into `chest` against 60 on the L — the right blade is pulled by the trunk more than the left. Both blades deform more than any other fore limb in the roster (next highest whole-chain: Odontochelys 1.79 %, Archelon 1.69 %). Stills: [Sprint 0.84 front](verification/cartorhynchus-Sprint-0.84-front.jpg), [top](verification/cartorhynchus-Sprint-0.84-top.jpg), [0.63 three-quarter](verification/cartorhynchus-Sprint-0.63-threequarter.jpg). | recorded as **T3D-18** (with the gape leak in Part B). **Fixed (T3D-18)**: the cause was the roots, not the weights. `thin_clusters` seats a blade at the one thin-shell vertex nearest the axis, and on this pair that was the trailing corner of the left blade and the leading corner of the right (y −0.107 against −0.211, mirror distance 0.117 L); `T.seat` then walked each root inwards "until 0.016 inside the skin", which stopped at x −0.075 on one side and 0.046 on the other because the depth it read was the local shell. The right polyline started in the chest, its measured radius grew to reach a blade it no longer ran through (0.102 against 0.081), and the chain took 1,388 vertices against 1,131. Now the base is the cluster's inner band in the trunk's **section** metric (Cheirolepis' `depth()`), 0.20 deep, and the root is that band's mean carried to 0.30 of the section on its own radial line: the base bands mirror 0.038 L, the seated roots 0.057 (the right base does stand 0.024 L further forward, which is the pose). Re-measured at 33 phases on the packaged file, `Sprint`: L `upper` 1.25 % / `mid` 0.74 % / `tip` 0.33 %, R 1.23 / 0.82 / 0.34; whole chain **2.36 / 2.46 %** (`Swim` 1.91 / 1.99) — within 4 % of each other at the chain and 2–10 % per bone; the builder's own 13-phase record in `validation.json` (`foreBladeRigidity`) reads 2.41 / 2.55. Trunk blend on the chain-dominated skin: builder's record L 70.6 / R 78.9 weight (6.9 % / 7.5 % of each chain's own weight, 12 % apart), 33-phase tool L 77 / R 91; `chest` is still twice as much on the right (29 against 13) because the right base sits nearer that station. Skin **3.72× → 2.98×** (`Haul`, `hind_upper_R`), 3 / 24 clips past 2×. The paired audit's paddles-out-travel-the-tail bar reads 1.54 (`Swim`) / 1.11 (`Sprint`). Stills after: [Sprint 0.84 front](verification/cartorhynchus-Sprint-0.84-front-after-T3D-18.png), [top](verification/cartorhynchus-Sprint-0.84-top-after-T3D-18.png), [0.63 three-quarter](verification/cartorhynchus-Sprint-0.63-threequarter-after-T3D-18.png). |
| 3 | Ceratites, Phragmoteuthis | Attacks read as rearing back / retreating | Mean arm-tip position along the forward axis, relative to rest, over L. **Ceratites** `Attack`: −2.4 % at phase 0.16 then **+7.8 % at 0.50**, back to 0; `Bite` −1.6 % → +5.9 %; `Grab` −2.2 % → +7.4 %; the attack anchor (`arm_02_04`) reaches 14.4 % L forward on `Attack`, 10.7 % on `Bite`. `Heavy` is −22.1 % and never forward — that clip is the builder's *Withdraw* (`build.py`: "this animal's move is literally called Withdraw") and reads as a retreat because it is one. **Phragmoteuthis** `Attack` −2.2 % at 0.125 then +4.3 % at 0.53; `Bite` −1.9 → +4.5 %; `Heavy` −2.6 → +4.4 %; `Grab` −2.2 → +4.5 %; the attack anchor (`arm_04_06`) reaches 2.9–4.3 % L forward, 15–18 % total displacement (the arm curls rather than extends). Net forward on every attack, with a short rear-back first; Phragmoteuthis' reach is a twentieth of a body — a grab, not a hit, but a short one. Stills: [Ceratites Attack 0.15](verification/ceratites-Attack-0.15-side.jpg) / [0.50](verification/ceratites-Attack-0.50-side.jpg) / [Heavy 0.50](verification/ceratites-Heavy-0.50-side.jpg); [Phragmoteuthis Attack 0.12](verification/phragmoteuthis-Attack-0.12-side.jpg) / [0.53](verification/phragmoteuthis-Attack-0.53-side.jpg). | OK (Ceratites; `Heavy` is by design). Phragmoteuthis' 4 % reach recorded as **T3D-23** — **fixed**: the strike is a dart of the whole body held through the close; `Attack` +13.6 % @0.55 / −2.4 % @0.13, `Bite` +14.5 / −1.7, `Grab` +14.5 / −2.4, `Heavy` +14.4 / −3.3 ([after](verification/phragmoteuthis-attack-after.png)) |
...
| Cartorhynchus | Ability@0.15 **29**/42/0 · Attack@0.38 **30**/41/0 · Bite@0.10 **47**/59/0 · Eat@0.35 **51**/61/0 · Grab@0.09 1/7/0 · Heavy@0.47 0/52/0 | 51 | **FAIL** — a sliver at the corner of the mouth on four clips ([Eat marked](verification/cartorhynchus-Eat-0.35-gape-opened.png)); recorded as **T3D-18**. **Closed by T3D-15, verified here**: these shots were taken on the file before T3D-15's rebuild (commit `ad20c13`, 06:25; the jaw junction landed at 08:17). Re-shooting that file reproduces them (Ability 25 / Attack 30 / Bite 47 / Eat 51), and a ray through the failing pixels of `Eat` finds no front face on the line and, first, the **inside of the far cheek** (`skull` 0.74 / `neck` 0.18 at raw y −0.31, the hinge station): the line of sight entered the head through the slot under the corner of the mouth that the rigid mandible shell opened against the skull-weighted throat — the slot `T.jaw_junction` closes, 2.02 % of L on this body ([before, marked](verification/cartorhynchus-Eat-0.35-gape-opened-before-T3D-15.png)). On the shipped file, before and after this session's rebuild of the paddles: Ability@0.15 0/14/0 · Attack@0.38 0/11/0 · Bite@0.10 0/13/0 · Eat@0.35 0/10/0 · Grab@0.09 0/5/0 · Heavy@0.47 0/12/0 — **0 px** through the body on every opening clip ([Eat after](verification/cartorhynchus-Eat-0.35-gape-opened-after.png)). No cap or flange was added: there was nothing left at the commissure to close. |
...
| Placodus (rebuilt here) | Ability@0.14 7/7/148 · Attack@0.13 11/11/731 · Bite@0.23 6/9/5 · CrushBite@0.26 10/10/418 · Eat@0.40 **19**/19/179 · Heavy@0.14 0/12/2 · Pry@1.93 0/0/0 | 19 | **FAIL by 7 px on `Eat`** — recorded as **T3D-19**. **Traced by ray (T3D-22's session), on the file T3D-15 rebuilt**: re-shot on that file Ability 11/11/130 · Attack 13/13/694 · Bite 6/11/8 · CrushBite 13/13/392 · Eat 10/10/164 · Heavy 0/18/3 · Pry 0/0/0 — the same ±3 px band as the shots above (24 samples, no denoise). A ray through every failing pixel of `Eat`, `Attack` and `CrushBite` meets **no front face on its line** and the same two surfaces first: 7–8 px the mandible's own lip flange (`T.rim_flange`, polygons 67/68/93, one polygon thick) at the front corner of the mouth, raw x 0.470–0.472 against the front cut at 0.473, |y| 0.026–0.030, z on the seam, seen edge-on from its back; 5–6 px the skull's front-cut cap (`T.cap_cut` at `JAW_FRONT_X`, polygons 17450/17460, x 0.4729–0.4730) seen from behind at the same corner. Every failing pixel's neighbours one or two pixels over are either honest backdrop between the parted lips or a front face of the crushing teeth, the floor shell or the mandible — a silhouette sliver at the corner where the lips part, not a line of sight through the head. The skin is double-sided at runtime, so the flange and cap draw both ways in play. Recorded, not closed ([Eat marked](verification/placodus-Eat-0.40-gape-opened-after.png), [Attack](verification/placodus-Attack-0.13-gape-opened-after.png), [CrushBite](verification/placodus-CrushBite-0.26-gape-opened-after.png)); `tools/triassic/creatures/placodus/gape-solid.json` carries the seven shots |
...
| Cartorhynchus | 537 / 333 | 436 / 271 | 0.92 0.98 0.98 1.02 | paddles (see A2); after T3D-18's re-seating 529 / 331 (a 33-phase sum), lag 0.91 0.92 0.99 1.01 |
...
| **Placodus** | **35 / 34** → **155 / 155** | 24 / 23 | 1.07 1.04 1.10 1.09 | was: the limbs barely move (a tenth of Henodus', a fifteenth of Aphaneramma's); the tail does the work (30.7 % of L at `tail_06` in `Sprint`). **Fixed (T3D-22)**: `Sprint` now paddles — 155° swept per cycle at every limb root on the packaged clip (fore 154.4 / hind 154.7 at 33 phases; the builder's own frame-to-frame sum records 155.4 / 155.3), from stretched forward to swept back along the flank, hind pair a third of a beat behind, over the unchanged tail wave. `Swim` is deliberately left at 24 / 23: the research reads the animal as a ballasted bottom-walker with a slow tail scull and short unmodified limbs, and the cruise keeps that; the dash is the era rule. Fore-blade rigid residual in `Sprint` L 1.48 % / R 1.21 % (chain), was 1.14 / 0.88. [Before](verification/placodus-Sprint-before-T3D-22.jpg), [after](verification/placodus-Sprint-after-T3D-22.jpg) |
...
| Phragmoteuthis | jaw | skull | **arm_04_06** (chain) | 2.9 / −6.0 (disp 15.6) | 4.0 / −8.9 | Bite 4.3, grasp on `arm_03_06` | reach is 3–4 % of L; **T3D-23** — **fixed**: 13.6 / −2.4, Heavy 14.4 / −3.3, Bite 14.5, Grab 14.5 (the body's own dart, held through the close) |
...
| Cartorhynchus | 3.72× | | 12 / 24 | same; **2.98×** after T3D-18 (`Haul` / `hind_upper_R`, 3 / 24) — the fore roots re-seated |
...
| Placodus | 12.36× | `Pry` / `fore_paddle_L` | 19 / 25 | same; after T3D-22's stroke `Sprint` itself reads 11.33× on that edge (was 9.42×), the body's worst unchanged |
