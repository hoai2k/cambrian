...
| T3D-07 | **active** | `/root/roster_finish` | Render and publish the 12 still-missing Triassic roster portrait sets without changing creature bodies: Cartorhynchus, Coelophysis, Cymbospondylus, Dinocephalosaurus, Henodus, Keichousaurus, Macrocnemus, Mixosaurus, Nothosaurus, Placodus, Shonisaurus and Tanystropheus. | `node tools/triassic/publish-portraits.mjs --check` reports no missing or stale sets; published files are model renders. |
