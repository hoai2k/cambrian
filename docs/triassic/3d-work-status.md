...
| T3D-08D | **active** | `/root/roster_finish` | Port the separate-palate/floor measured-room call into the current Aphaneramma and Mystriosuchus builders. Rebuild with `/Applications/Blender.app/Contents/MacOS/Blender -b -t 2 --python tools/triassic/creatures/{aphaneramma,mystriosuchus}/build.py`, then run each packaged decoded audit. | The rebuilt authored/puppet/LOD assets contain rigid skull/jaw oral groups, each creature audit passes, and the shared throat review remains clean. |
| T3D-08E | **active** | `/root/roster_finish` | Port the separate-palate/floor measured-room call into the current Mosasaurus and Odontochelys builders. Rebuild with `/Applications/Blender.app/Contents/MacOS/Blender -b -t 2 --python tools/triassic/creatures/{mosasaurus,odontochelys}/build.py`, then run each packaged decoded audit. | The rebuilt authored/puppet/LOD assets contain rigid skull/jaw oral groups, each creature audit passes, and the shared throat review remains clean. |
...
| T3D-09 | **unclaimed** | — | Port Coelophysis, Macrocnemus and Tanystropheus from the separate `shorekit.oral_lining` implementation to the same rigid palate/floor contract. This is a cross-kit change and must retain their runner-specific action and skinning repairs. | Rebuilt paired assets have skull-owned palate and jaw-owned floor shells; their runner action audits and the throat review pass. |
