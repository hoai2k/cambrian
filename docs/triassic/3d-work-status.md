...
| T3D-07 | **active** | `/root/roster_finish` | Render and publish the three actually missing Triassic portrait sets without changing creature bodies: Cartorhynchus, Cymbospondylus and Mixosaurus. The publisher audit confirms the other nine originally flagged bodies already have complete direct-public model renders. | `node tools/triassic/publish-portraits.mjs --check` reports no missing or stale sets; published files are model renders. |
