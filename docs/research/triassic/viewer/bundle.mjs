...
for (const [id, list] of Object.entries(canonical)) {
  ...
  for (const entry of list) {
    const file = basename(entry.src), stem = basename(file, extname(file));
    const out = join(DEPLOY_DIR, 'canonical', `${stem}.webp`);
    await sharp(join(HERE, CANON_DIR, file)).resize({ width: DEPLOY_WIDTH, withoutEnlargement: true }).webp({ quality: DEPLOY_QUALITY }).toFile(out);
    ...
    deployed[id].push({ ...entry, src: `canonical/${stem}.webp`, thumb: entry.thumb ?? null });
  }
}
