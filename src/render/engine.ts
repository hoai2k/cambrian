  ...
  private handleEvents(game: Game) {
    ...
    for (const e of evs) {
      ...
      switch (e.kind) {
        ...
        case 'eat': { this.bubbles.emit(e.pos, 5, 0.3, 1.2, 0.05, 0.8); const s = e.strength ?? 0.5; const big = s > 0.5 && heavy('crunch', e.actor) === 'crunch-huge'; world(big ? 'crunch-huge' : 'eat', e.pos, s); break; }
