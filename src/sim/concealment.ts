...
export function camouflageMatch(a: Actor, world: WorldData, actors: Actor[]): CamouflageMatch {
 ...
 let best:CamouflageMatch={colors:environmentColors(floorColor(a.pos.x,a.pos.z)),scheme:'environment',label:SAY.matchSeafloor,actor:-1,distance:Math.max(0,a.pos.y-floor-clearanceOf(a))};
 ...
 for(const f of world.floraHash.query(a.pos.x,a.pos.z,best.distance+world.floraReach,[])){
  ...
  take(d,environmentColors(floraColor(f)),SAY.matchPlant(f.kind));
 }
 for(const b of world.boulderHash.query(a.pos.x,a.pos.z,best.distance+8,[])){
  ...
  take(d,environmentColors(rockColor(b)),b.variant??SAY.matchRock);
 }
...
export function hideLabel(id:CreatureId){return BURROWERS.has(id)?SAY.burrow:SAY.camouflage;}
/** Names the hide button as `{ability}`; the UI fills it in for that player's device. */
export function hideDescription(id:CreatureId){return BURROWERS.has(id)?SAY.burrowDescription:SAY.camouflageDescription;}
