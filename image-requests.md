# Image requests — Cambrian Explosion

Everything below is currently a placeholder (procedural SVG or CSS). Each entry says where the file goes, what size, and what it replaces. Drop the finished file at the path listed and the game picks it up without code changes unless noted.
...
## Notes for all assets

- Text-free wherever possible so localisation never needs new art.
- Colours: teal ink `#06161c`, foam `#eefaf6`, ember `#ffb36b`, coral `#ff5b6e`, lagoon `#61f2d5`.
- Optimise: WebP for photos/paintings, SVG for marks, PNG only where transparency + fine detail is needed. Keep any single file under 600 KB.
