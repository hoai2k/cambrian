 ...
 * hero card and thumbnail exist, and that the images were generated from the current model
 * appearance. Missing files fail; stale images warn (or fail with --strict).
 */
...
for (const id of ids) {
  for (const f of [`${id}.glb`, `${id}.lod1.glb`, `${id}.png`, `${id}.card.png`, `${id}.thumb.png`]) if (!fs.existsSync(`${dir}/${f}`)) err(`${id}: missing ${f}`);
