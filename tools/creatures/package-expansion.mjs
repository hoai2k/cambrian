/**
 * Lossless meshopt packaging for the thirteen expansion creatures only.
 * Run: node tools/creatures/package-expansion.mjs [odaraia sidneyia ...]
 * Defaults to all thirteen. Full models retain every clip and texture. Existing
 * LOD geometry is retained, with only Idle/Swim/Crawl/Death and vertex pigments.
 * Backups and audit statistics stay in ../expansion-authoring/packaging/.
 */
...
const STATS = path.join(LOCAL, 'packaging.json');
...
const ids = process.argv.slice(2).length ? [...new Set(process.argv.slice(2))] : IDS;
...
async function originalHashes() {
  ...
  assert(names.length >= ORIGINALS.length * 2, 'Missing original assets: cannot certify original roster');
  return result;
}
