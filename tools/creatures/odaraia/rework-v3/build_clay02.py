"""Frozen Odaraia clay authoring source. Execute only through HASHED_HANDOFF_CLAY02.md after user resumes.
...
OUT = ROOT / 'clay02'
if OUT.exists():
    raise RuntimeError('clay02 already exists: never overwrite a candidate. Return to Astra.')
...
def shell_mesh(half=False):
    ...
    for layer in range(2):
        for k in range(NZ+1):
            ...
            for j in range(NA+1):
                fraction=j/NA
                theta=(-limit+2*limit*fraction) if not half else (limit*fraction)
...
# Short peduncles flow into a real flared posterior cup. This is source geometry,
# not a completed eye-interface audit. Globe front remains anatomically exposed.
for s,label in [(-1,'L'),(1,'R')]:
    eye_c=Vector((s*.50,-.035,2.205));axis=Vector((s*.65,0,.76)).normalized()
    vertical=Vector((0,1,0));lateral=axis.cross(vertical).normalized()
    profile=[(-.38,.078),(-.30,.103),(-.22,.126),(-.155,.178),
             (-.09,.211),(-.025,.229),(-.014,.229),(-.014,.216),
             (-.07,.201),(-.12,.176),(-.17,.125),(-.20,.066),(-.21,.02)]
    support=Mesh();vs=[];fs=[];n=40
    for depth,radius in profile:
        for j in range(n):
            angle=2*pi*j/n
            vs.append(eye_c+axis*depth+radius*(.93*cos(angle)*vertical+sin(angle)*lateral))
    for k in range(len(profile)-1):
        for j in range(n):fs.append((k*n+j,k*n+(j+1)%n,(k+1)*n+(j+1)%n,(k+1)*n+j))
    for k in [0,len(profile)-1]:
        cap=len(vs);vs.append(eye_c+axis*profile[k][0])
        for j in range(n):fs.append((cap,k*n+j,k*n+(j+1)%n))
    support.add(vs,fs);support.obj(f'Eye {label} | continuous peduncle flared posterior cup',BODY)
...
for i in range(32):
    ...
    base_reach=(1.10-.44*t)*(1.07 if i<5 else 1)
    for s,label in [(-1,'L'),(1,'R')]:
        # Traveling rest-phase offsets, not count reduction: distinguish a raised
        # distal endopod from its broad, lower and backswept ovate swimming paddle.
        wave=sin(i*.63+(0 if s<0 else .85))
        reach=base_reach*(1+.105*wave)
        sweep=.13*sin(i*.63+.55+(0 if s<0 else .85))
        root=Vector((s*body_r*.79,.105-.075*t,z))
        a=root
        knee=Vector((s*(.335-.07*t),reach*.75,z-.09+sweep*.28))
        d=Vector((s*(.275-.035*t),reach*.94,z+.32+sweep))
        def endopod_at(u):
            if u<.58:
        ...
        for q in range(17):
            u=q/16
            ep.append(bezier(root,root+Vector((s*.12,base_reach*.24,-.11)),(s*(.45-.10*t),base_reach*.62,z-.31+sweep*.35),(s*(.48-.12*t),base_reach*(.78+.065*wave),z-.40+sweep*.55),u))
            ew.append(.007+(.040-.012*t)*sin(pi*u)**.78)
        ...
        filt=Mesh()
        for q in range(4,20):
            u=(bounds[q]+bounds[q+1])*.5;p=endopod_at(u)
        ...
        LIMB_METADATA.append({'segment':i+1,'side':label,'root':list(root),'tip':list(d),'podomeres':20,'restPhase':wave,'paddleTip':list(ep[-1])})
...
def aim(ob,target):
    # Explicit orthonormal basis: camera local +Y is projected WORLD +Y.
    # Blender default track-quaternion roll does not satisfy this game's coordinates.
    back=(ob.location-Vector(target)).normalized()
    up=Vector((0,1,0))-back*back.dot(Vector((0,1,0)))
    if up.length<1e-6:raise RuntimeError('Camera direction parallel to world +Y; author decision required')
    up.normalize();right=up.cross(back).normalized();up=back.cross(right).normalized()
...
# Five complete-animal views. Sixth isolates the complete head plus first six limb
# pairs as an appendage study; nothing in its selected study geometry is cropped.
VIEWS=[
 ('01-inverted-oblique',(-8,3.3,5.0),(0,.12,-.30),False,False),
 ('02-inverted-side',(-10,.18,-.25),(0,.18,-.25),False,False),
 ('03-inverted-front',(0,.9,10),(0,.15,-.20),False,False),
 ('04-dorsal-underside',(-6,-6,4),(0,-.10,-.30),False,False),
 ('05-half-shell-cutaway',(-8,3.3,5.0),(0,.12,-.30),True,False),
 ('06-appendage-head-closeup',(-6,3.2,4.2),(0,.48,1.48),True,True),
]
FRAMING=[]
def setup_view(view,record=False):
    name,pos,target,cut,detail=view
    for ob in MODEL:ob.hide_render=False
    coat.hide_render=cut;cutaway.hide_render=not cut
    if detail:
        for ob in MODEL:
            keep=ob.name.startswith(('Head |','Eye ','Mouth'))
            if ob.name.startswith('Limb '):keep=int(ob.name[5:7])<=6
            ob.hide_render=not keep
    camera.location=pos;right,up,back=aim(camera,target)
    points=[ob.matrix_world@v.co for ob in MODEL if not ob.hide_render for v in ob.data.vertices]
    if not points:raise RuntimeError('No visible study geometry')
    xs=[q.dot(right) for q in points];ys=[q.dot(up) for q in points]
    xmid=(min(xs)+max(xs))/2;ymid=(min(ys)+max(ys))/2
    # Recenter within the existing image plane without changing the camera basis.
    camera.location += right*(xmid-camera.location.dot(right))+up*(ymid-camera.location.dot(up))
    width=max(xs)-min(xs);height=max(ys)-min(ys)
    # Query Blender's own camera frame instead of assuming ortho_scale semantics.
    data.ortho_scale=1.0
    unit_frame=data.view_frame(scene=SCENE)
    unit_width=max(q.x for q in unit_frame)-min(q.x for q in unit_frame)
    unit_height=max(q.y for q in unit_frame)-min(q.y for q in unit_frame)
    data.ortho_scale=max(width/unit_width,height/unit_height)*1.12
    frame=data.view_frame(scene=SCENE)
    frame_width=max(q.x for q in frame)-min(q.x for q in frame)
    frame_height=max(q.y for q in frame)-min(q.y for q in frame)
    xmin=(min(xs)-xmid)/frame_width+.5;xmax=(max(xs)-xmid)/frame_width+.5
    ymin=(min(ys)-ymid)/frame_height+.5;ymax=(max(ys)-ymid)/frame_height+.5
    if min(xmin,ymin)<.049 or max(xmax,ymax)>.951:raise RuntimeError('Projected frame margin failed')
    if record:
        FRAMING.append({'view':name,'scope':'isolated complete head and first six limb pairs' if detail else 'complete visible animal',
          'worldUp':[0,1,0],'cameraImageUp':list(up),'upDotWorldUp':up.y,
          'orthoScale':data.ortho_scale,'normalisedBounds':[xmin,ymin,xmax,ymax],
          'visibleObjects':len([ob for ob in MODEL if not ob.hide_render])})
    return name

setup_view(VIEWS[0])
SCENE['odaraia_stage']='clay02, UNREVIEWED, no rig/material/export approval'
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'odaraia-clay02.blend'))
for view in VIEWS:
    name=setup_view(view,True)
    SCENE.render.filepath=str(OUT/(name+'.png'));bpy.ops.render.render(write_still=True)
(OUT/'camera-framing-report.json').write_text(json.dumps(FRAMING,indent=2))
setup_view(VIEWS[0])
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'odaraia-clay02.blend'))
outputs=[]
...
print('ODARAIA_CLAY02_SOURCE_HYGIENE_PASS; SIX_VIEWS_RENDERED; ASTRA_VISUAL_REVIEW_REQUIRED')
