...
function trunk(P, t, env, { power = 0, beats = 0, ramp = 0, amp = .16, loopU } = {}) {
  ...
  for (const s of SIDES) for (let i = 0; i < 6; i++) P.bend(`swimmer_${s}_${i}`, FWD, .2 * A * Math.sin(ph - i * .6 + (s < 0 ? Math.PI : 0)));
  for (const s of SIDES) P.bend(`tail_${s}`, UP, .14 * A * Math.sin(ph - 5));
...
/**
 * Bones this performance authors. The raptorial legs and antennae are the performance; the swimmerets, tail and trunk keep the shipped motion.
 */
export const authored = (n) => /^(raptor_|antenna)/.test(n);
...
export const clips = [
  {
    ...
    pose(u, P, t) {
      ...
      trunk(P, t, alive(u), { amp: .8, power: .9 * S, beats: 1.5, ramp: ss(.1, .55, u) });
    },
  },
  {
    ...
    pose(u, P, t) {
      ...
      trunk(P, t, alive(u), { amp: .9, power: .4 * W + 1.2 * S, beats: 2.0, ramp: ss(.28, .66, u) });
    },
  },
  {
