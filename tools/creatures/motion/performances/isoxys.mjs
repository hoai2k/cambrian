...
function trunk(P, t, env, { power = 0, beats = 0, ramp = 0, burst = 0, amp = .16, loopU } = {}) {
  ...
  for (const s of SIDES) for (let i = 0; i < 13; i++) {
    ...
    for (let k = 0; k < 3; k++) P.bend(`leg_${s}_${pad(i)}_${k}`, FWD, (k ? .05 : .1) * (A + burst) * Math.sin(phase));
    P.bend(`paddle_${s}_${pad(i)}`, FWD, .14 * (A + burst) * Math.sin(phase + .4));
  }
...
/**
 * Bones this performance authors. The raptorial hooks are the performance; the swimming limbs, paddles and tail keep the shipped motion.
 */
export const authored = (n) => /^raptor_/.test(n);
...
export const clips = [
  {
    ...
    pose(u, P, t) {
      ...
      trunk(P, t, alive(u), { amp: .8, power: .8 * S, beats: 1.4, ramp: ss(.1, .55, u) });
    },
  },
  {
    ...
    pose(u, P, t) {
      ...
      trunk(P, t, alive(u), { amp: .9, power: .3 * W + 1.2 * S, beats: 2.0, ramp: ss(.3, .66, u), burst: .5 * hold(.34, .44, .6, .8, u) });
    },
  },
  {
    ...
    pose(u, P, t) {
      ...
      trunk(P, t, alive(u), { amp: .8, power: .8 * R, beats: 1.6, ramp: ss(.1, .62, u) });
    },
  },
  {
