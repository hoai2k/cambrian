...
function trunk(P, t, env, { beat = 0, amp = .16, loopU } = {}) {
  ...
  for (const s of SIDES) for (let i = 0; i < 12; i++) P.bend(`flap_${s}_${pad(i)}`, DOWN, .2 * amp * env * Math.sin(ph - i * .5) + .6 * beat * Math.sin(Math.PI * Math.min(1, Math.max(0, 1 - i * .06))));
  for (const s of SIDES) for (let i = 0; i < 3; i++) P.bend(`tail_${s}_${i}`, UP, .05 * amp * env * Math.sin(ph - 5) + .25 * beat);
...
export const clips = [
  ...
  },
  {
    ...
    pose(u, P, t) {
      ...
      trunk(P, t, Math.sin(Math.PI * u) ** 2, { beat: hold(.36, .46, .62, .85, u) });
    },
  },
  {
