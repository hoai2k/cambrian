...
// --- a player corpse dissolves just before the end of the death watch, and respawns at it ---
{
  ...
  for (let i = 0; i < 60 * (CORPSE_WINDOW + 2); i++) {
    g.step(1 / 60, m);
  ...
  check('player corpse sparkles just before respawn', sparkAt > CORPSE_WINDOW - 0.6 && sparkAt < CORPSE_WINDOW - 0.2, `sparkles at ${sparkAt.toFixed(2)}s`);
  check('player respawns at the end of the death watch', respawnAt > CORPSE_WINDOW - 0.1 && respawnAt < CORPSE_WINDOW + 0.3, `respawn at ${respawnAt.toFixed(2)}s, watch is ${CORPSE_WINDOW}s`);
}
// --- swallowed player: the same watch, and the sparkles come from the predator ---
{
