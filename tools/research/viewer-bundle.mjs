...
/**
 * The body we have actually built, in front of the pose it was built from.
 *
 * This is the comparison the whole page is for, and until now it stopped one step short: a
 * reviewer could hold the canonical image against other people's art but not against our own
 * model. The game preview goes first in the row, so the sweep reads model → canonical →
 * references, and a body that has drifted from its pose is obvious without leaving the page.
 *
 * Only a *delivered* model qualifies. The Triassic's unshipped animals carry placeholder
 * portraits cut from the canonical image itself, and showing one of those here would put the
 * canonical image next to a crop of the canonical image and call it a model.
 */
let modelPreviews = 0;
if (era.models) {
  const shipped = new Set(JSON.parse(await readFile(join(ROOT, era.models.shipped), 'utf8')).creatures ?? []);
  for (const id of ids) {
    if (!shipped.has(id)) continue;
    const abs = join(ROOT, era.models.dir, `${id}.card.png`);
    if (!(await stat(abs).catch(() => null))) continue;
    (canonical[id] ??= []).unshift({
      abs, src: relative(HERE, abs), thumb: await thumbnail(abs, `${id}-model-preview`),
      kind: 'model', label: 'game preview',
    });
    modelPreviews++;
  }
}
const rank = (c) => (c.kind === 'model' ? -1 : c.kind === 'model-reference' ? 3 : c.kind === 'turnaround' ? 2 : c.label === 'canonical' ? 0 : 1);
...
// Rejections already applied, so the page opens showing the board as the repository has it.
let rejectedRefs = {};
try { rejectedRefs = JSON.parse(await readFile(join(HERE, 'rejected.json'), 'utf8')); } catch { /* none yet */ }
const bundle = (canon) => `/* Generated by tools/research/viewer-bundle.mjs — do not edit. */\n`
  ...
  + `window.REFERENCE_REJECTED = ${JSON.stringify(rejectedRefs)};\n`
...
console.log(`data.js written — ${ids.size} subjects, ${withCanon} with a canonical image, ${ids.size - withCanon} without, ${modelPreviews} built models, ${modelReferences} modelling references`);
...
for (const [id, list] of Object.entries(canonical)) {
  ...
  for (const entry of list) {
    ...
    const stem = entry.kind === 'model-reference' ? `${id}-model-${fileStem}`
      : entry.kind === 'model' ? `${id}-game-preview` : fileStem;
