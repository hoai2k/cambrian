...
// --- hauling through weed: cover carries a limbed body and drags on a swimmer ---
{
   ...
   *
   * Held down on the way through. Weed grows off the floor and the floor falls away here, so a
   * swimmer holding its depth simply sailed over the top of the bed: it finished four units clear
   * of plants 1.8 tall, touched nothing, and came out with exactly the distance it went in with,
   * which read as the drag being broken when it had never been asked for. The same input goes to
   * both bodies and to both runs of each, so what is compared is still only the weed.
   */
  const cross = (id: CreatureId, planted: boolean) => {
    ...
    const from = { ...p.pos };
    for (let i = 0; i < 150; i++) step({ my: -1, camYaw: 0, sink: true });   // straight into the bed, down in it
