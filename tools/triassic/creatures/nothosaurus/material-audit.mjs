...
function clipHashes(d){return Object.fromEntries(d.getRoot().listAnimations().map(a=>[a.getName(),sha(Buffer.from(JSON.stringify(a.listChannels().map(c=>[c.getTargetNode().getName(),c.getTargetPath(),data(c.getSampler().getInput()),data(c.getSampler().getOutput())]))))]));}
function tailChannels(d,name){const a=d.getRoot().listAnimations().find(a=>a.getName()===name);return a.listChannels().filter(c=>c.getTargetNode().getName().startsWith('tail_')).map(c=>[c.getTargetNode().getName(),c.getTargetPath(),data(c.getSampler().getInput()),data(c.getSampler().getOutput())]);}
function assertTailClose(current,baseline){assert.equal(current.length,baseline.length);for(let i=0;i<current.length;i++){assert.equal(current[i][0],baseline[i][0]);assert.equal(current[i][1],baseline[i][1]);for(const k of[2,3]){assert.equal(current[i][k].length,baseline[i][k].length);for(let j=0;j<current[i][k].length;j++)assert(Math.abs(current[i][k][j]-baseline[i][k][j])<1e-6);}}}
...
for(const m of source.getRoot().listMeshes())for(const p of m.listPrimitives()){const v=p.getAttribute('POSITION'),uv=p.getAttribute('TEXCOORD_0');for(let i=0;i<v.getCount();i++){const a=v.getElement(i,[]),b=[-5*a[2],5*a[1],5*a[0]],k=key(b);if(!dict.has(k))dict.set(k,[]);dict.get(k).push({pos:b,uv:uv.getElement(i,[])});}}
