"""Rebuild Mixosaurus: authored Tripo skin and measured voxel-volume twin on one shared rig.

Blender 5.2. The body is carried into its own measured frame first -- head at -Y, up +Z, one unit
long -- because this generation lies **25.2 degrees** across the file's axes, the widest of the
four, and its fluke spans further across the body than the tail is long, so neither the file's
axes nor a bounding box would have found the animal. `tx()` then applies the engine scale and
`export_yup` puts the head at glTF +Z, where every shipped body in this repository keeps it.

Mixosaurus is the small quick one, and it is built to read as such against Cymbospondylus. It is
the oldest animal with a dorsal fin, and that fin plus the tail lobe is what the research says it
swims with: stable, sustained, *more fish-like than its basal relatives*. So the wave here is
**carangiform** -- half a wavelength on the body, the front third almost still, the amplitude
piled into the last quarter -- where Cymbospondylus carries nearly a whole wavelength and works
from the shoulder back. The signature clip is `Dart`: a C-start, the fast-start every small fish
escapes and strikes with, and the roster's own `shoalDart`.
...
CLIPS = {'Idle': 2.0, 'Swim': 1.1, 'Sprint': .7, 'TurnLeft': 1.0, 'TurnRight': 1.0,
         'Dive': 1.0, 'Rise': 1.0, 'Attack': .7, 'Bite': .35, 'Heavy': .9, 'Hit': .45,
         'Death': 1.6, 'Guard': 1.0, 'Parry': .3, 'Dodge': .4, 'Eat': 1.2, 'Stagger': 1.0,
         'Ability': .8, 'Grab': 1.0, 'Breath': 1.8, 'Growth': 1.2,
         'Dart': .6, 'Breathe': 2.4}
...
LIMBS, caudal_cluster, dorsal_cluster, other = {}, None, None, []
for c in clusters:
    mid = (c['yRange'][0] + c['yRange'][1]) / 2
    lateral = abs(c['centroid'][0] - cx(mid))
    if c['reachRadius'] > .12 and lateral > .04:
        key = ('pec' if mid < 0 else 'pel') + ('L' if c['centroid'][0] < cx(mid) else 'R')
        LIMBS[key] = c
    elif c['yRange'][1] > Y1 - .06:
        caudal_cluster = c
    elif lateral < .04 and c['centroid'][2] > cz(mid) + .04 and c['reachRadius'] > .12:
        # The oldest dorsal fin in any backboned animal, and the reason this silhouette is worth
        # having at one metre. It is a median blade, so it is found by standing above the axis
        # rather than out to one side of it, and it gets a bone of its own.
        dorsal_cluster = c
    else:
        other.append(c)
if sorted(LIMBS) != ['pecL', 'pecR', 'pelL', 'pelR'] or not dorsal_cluster:
    print('MIXO_CLUSTERS', json.dumps(
...
# The dorsal fin is seated inside the back rather than sitting on it, like every other appendage
# root here, and it is a stiffened blade in life -- fibre bundles, not a flag -- so it gets one
# joint and only ever leans.
DORSAL_Y = float((dorsal_cluster['yRange'][0] + dorsal_cluster['yRange'][1]) / 2)
DORSAL_ROOT = T.seat(Vector(dorsal_cluster['seat']), on_axis(DORSAL_Y), depth, margin=.012)
bone('dorsal', DORSAL_ROOT, 'body')
DORSAL_SEATING = depth(DORSAL_ROOT)
assert DORSAL_SEATING > .008, ('the dorsal fin root is not seated in the back', DORSAL_SEATING)
...
def dorsal_weights(q, thin):
    """The median dorsal blade, taken by standing above the measured axis over its own station
    range, with a radial blend so the root follows the back when the body bends."""
    lo, hi = dorsal_cluster['yRange']
    if not lo - .02 < q.y < hi + .02:
        return None
    blade = T.smooth((THIN + THIN_BAND - thin) / THIN_BAND)
    d = q.z - cz(q.y)
    if blade <= 0 or d <= 0:
        return None
    g = blade * T.smooth((d - half_depth(q.y) * .75) / .030)
    return ({'dorsal': 1.}, g) if g > 0 else None
...
# Carangiform. Half a wavelength on the body against Cymbospondylus' three quarters, a gain curve
# that is nearly nothing until the peduncle, and a beat twice as quick: the two animals are built
# out of the same three numbers and are meant to be told apart by them.
WAVE_SPAN = 3.1          # radians of phase from the neck to the last caudal joint
GAIN = [.03 + .97 * (i / 9) ** 2.6 for i in range(10)]
LAG = [WAVE_SPAN * i / 9 for i in range(10)]
CAUDAL_LAG = WAVE_SPAN + .80     # the lobes trail the peduncle
...
for clip, duration in CLIPS.items():
    ...
    for f in range(last + 1):
        ...
        amp = {'Idle': .26, 'Swim': 1.0, 'Sprint': 1.45, 'Eat': .34, 'Guard': .20, 'Grab': .30,
               'Breath': .34, 'Breathe': .30, 'Growth': .24, 'Dodge': 1.20, 'Dart': .70,
        ...
        # A **C-start**: the whole body folds into a C in under a fifth of the clip, holds for two
        # frames, and unfolds explosively into a straight line. It is not a small Attack -- there
        # is no gape in it and no reach, only the fold and the launch -- and it is the roster's
        # `shoalDart`.
        cstart = ramp(u, .02, .17, 1.) * (1 - ramp(u, .22, .40, 2.6)) if clip == 'Dart' else 0.
        launch = ramp(u, .24, .40, 2.4) * (1 - ramp(u, .55, .95, 1.)) if clip == 'Dart' else 0.
        ...
        if clip == 'Dart':
            body.rotation_euler.z += .34 * cstart - .12 * launch
            body.location.y = .05 * cstart - .72 * launch
            body.location.x = .16 * cstart
        if clip == 'Ability':
        ...
        for i, n in enumerate(CHAIN):
            q = pb[n]
            z = .205 * GAIN[i] * amp * wave(i, beat)
            z += turn * (.018 + i * .009)
            z += .040 * dead * sin(i * .8)
            if clip == 'Dart':
                # Every joint folds the same way at once -- that is what makes it a C rather than
                # a wave -- and then snaps through straight and overshoots the other way.
                z += .40 * (.35 + .65 * (i / 9)) * cstart
                z -= .46 * (.35 + .65 * (i / 9)) * launch
        ...
        if clip == 'Dart':
            pb['skull'].rotation_euler.z += .16 * cstart - .10 * launch
        if clip == 'Ability':
        ...
        # The dorsal fin is a keel: it holds a line at sprint, which is this animal's passive, so
        # it barely moves in Swim and leans hard into a turn and a dart.
        pb['dorsal'].rotation_euler.z = .05 * amp * wave(6, beat) + .22 * turn
        pb['dorsal'].rotation_euler.y = -.16 * turn
        if clip == 'Dart':
            pb['dorsal'].rotation_euler.z += .18 * cstart
        ...
        for key, names in LIMB_NAMES.items():
            ...
            if clip == 'Dart':
                # The fins are clamped to the flank for the launch, which is what a fast-start
                # actually looks like from above.
                up.rotation_euler.x += .30 * cstart - .18 * launch
                up.rotation_euler.z += s * .36 * launch
            if clip == 'Guard':
...
meta = {
    'id': ID, 'name': NAME, 'species': 'Mixosaurus cornalianus',
    'provenance': 'Middle Triassic · Besano Formation, Monte San Giorgio',
    'description': 'Small early ichthyosaur with the oldest dorsal fin of any backboned animal, '
                   'long forefins, short hindfins and a lobed tail. Authored Tripo body and '
                   'measured procedural volume twin share one armature, one set of inverse binds, '
                   'one set of sockets and one set of actions.',
    'modelLength': BODY_LENGTH, 'lengthMeters': 1.5, 'locomotion': 'Swim',
    'clips': list(CLIPS), 'looping': LOOPS, 'anchors': [a['name'] for a in anchors],
    'puppet': ID + '.puppet.glb',
    'sources': ['docs/triassic/canonical/mixosaurus.png',
                'tools/triassic/creatures/mixosaurus/tripo-raw/mixosaurus.raw.glb'],
    'notes': [
        'Carangiform, not anguilliform: half a wavelength on the body, the front third almost '
        'still, and the amplitude piled into the peduncle. The research calls this animal more '
        'fish-like than its basal relatives, and it is built to read against Cymbospondylus.',
        'The dorsal fin -- the oldest in any backboned animal, and this animal\'s headline -- is '
        'the generation\'s own blade on its own seated joint. It is a keel: it holds the line at '
        'sprint and leans into a turn and a dart, and it never flaps.',
        'Dart is a C-start, the roster\'s shoalDart: the whole body folds one way in a sixth of a '
        'second and snaps through straight. It is not a short Attack.',
        'The body was measured into its own frame before anything was rigged: this generation '
        'lies 25.2 degrees across the file axes -- the most crooked of the four -- and its roll '
        'was read off the countershading rather than transported.',
        'This generation has **no modelled mouth**: Placodus\' geometric method finds nothing on '
        'the rostrum, so the mouth line was read off the albedo as Dinocephalosaurus\' was. There '
        'is therefore no source dentition either, and none was authored; see the README.',
        'The caudal fin is deeply forked with two near-equal lobes, which the research denies '
        'this animal -- "not the post-Triassic lunate fluke". That is the pose and the '
        'generation, recorded rather than remodelled; see the README.',
...
profile_report = {
    ...
    'appendageRootSeatingRaw': {**LIMB_SEATING, 'dorsal': DORSAL_SEATING},
...
print('MIXO_FRAME', json.dumps({k: v for k, v in frame.items() if k != 'perStation'}))
print('MIXO_REPORT', json.dumps({k: report[k] for k in
      ('authoredTriangles', 'twinTriangles', 'twinTriangleFraction', 'bones', 'maxInfluences')}))
print('MIXO_ENVELOPE', json.dumps(report['envelope']))
print('MIXO_MOUTH', json.dumps({k: report['mouth'][k] for k in
...
print('MIXO_POSE', json.dumps({'spine': {k: v for k, v in POSE_DEVIATION['spine'].items() if k != 'perStation'},
      'tail': {k: v for k, v in POSE_DEVIATION['tail'].items() if k != 'perStation'},
      'limbAsymmetry': LIMB_ASYMMETRY.get('allPairs')}))
print('MIXO_SEAMS', json.dumps({k: round(v, 9) for k, v in seams.items()}))
