...
const out='local/askeptosaurus';fs.mkdirSync(out,{recursive:true});
const browser=await chromium.launch({executablePath:process.env.CHROME_PATH??'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true,args:['--use-gl=angle','--use-angle=swiftshader','--enable-unsafe-swiftshader','--ignore-gpu-blocklist','--no-sandbox']});
...
const choices=await select.locator('option').allTextContents();assert(choices.includes('Backup Model'));assert.equal(await page.locator('.clip').count(),24);
await page.screenshot({path:'local/askeptosaurus/viewer-full.png'});
...
await page.screenshot({path:'local/askeptosaurus/viewer-backup.png'});
...
assert.equal(await page.locator('.clip').count(),24);
await page.screenshot({path:'local/askeptosaurus/viewer-twin.png'});
