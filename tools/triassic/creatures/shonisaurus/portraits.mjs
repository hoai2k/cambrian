...
const mouths=[];for(const [j,kind]of['full','puppet'].entries())for(const [i,[clip,t]]of[['Idle',.5],['Ability',.5],['Heavy',.25]].entries()){mouths.push({input:label(`${kind} · ${clip} ${clip==='Heavy'?'feeding gape':'closed'}`,900,36),left:i*900,top:j*636});mouths.push({input:`${r}/${kind}-closed-${clip}-${t}.png`,left:i*900,top:j*636+36});}const mouthSheet=await sharp({create:{width:2700,height:1272,channels:3,background:'#15232c'}}).composite(mouths).png().toBuffer();await sharp(mouthSheet).resize(1800).jpeg({quality:88}).toFile(`${h}/closed-mouth-review.jpg`);
