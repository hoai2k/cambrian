...
# The anterior aperture is the only soft part of the coil silhouette. The previous
# radial falloff gave the outer whorl back to `head`, so testing only vertices
# already pure on `shell` could pass while the visible rim bent. These two planes
# bound the aperture below/forward of the coil; the rest of the shell is rigid.
SHELL_APERTURE_Y = float(COIL[1]) - SHELL_R * .64
SHELL_APERTURE_Z = float(COIL[2]) + SHELL_R * .18


def rigid_shell_share(q):
    radius = math.hypot(q.y - float(COIL[1]), q.z - float(COIL[2]))
    distance = max(q.y - SHELL_APERTURE_Y, q.z - SHELL_APERTURE_Z)
    return smooth(1. + distance / .025) * smooth((SHELL_R + .035 - radius) / .015)
...
def weights(p):
    ...
    # **The lips are claimed before the arms are.** A peristome belongs to the mouth whatever is
    # standing over it, and thirteen arms stand over this one: taking the arms first left the skin
    # round the opening weighted to whichever arm reached across it, so the rim stayed put while the
    # beak swung and the lining parted from it. `gape-solid.py` measured 168 px of backdrop through
    # that gap against a 12 px tolerance.
    ring, ventral = lip_share(q)
    arms, claimed = arm_claims(q)
    claimed *= (1 - ring)
    arms = {n: v * (1 - ring) for n, v in arms.items()}
    g = shell_share(q) * (1 - claimed) * (1 - ring)
    f = funnel_share(q) * (1 - claimed) * (1 - g) * (1 - ring)
    lip = ring
...
for o in (auth, puppet):
    ...
    # Enforce the measured shell region AFTER diffusion. The soft collar blends
    # toward it, but no relaxation pass may put head/arm weights back on the coil.
    for i, v in enumerate(o.data.vertices):
        fixed = rigid_shell_share(v.co)
        if fixed >= 1. - 1e-8:
            per_vertex[i] = {'shell': 1.}
        elif fixed > 0:
            w = {n: value * (1. - fixed) for n, value in per_vertex[i].items()}
            w['shell'] = w.get('shell', 0.) + fixed
            top = sorted(w.items(), key=lambda item: -item[1])[:4]
            total = sum(value for _, value in top)
            per_vertex[i] = {n: value / total for n, value in top}
    for n in ORDER:
