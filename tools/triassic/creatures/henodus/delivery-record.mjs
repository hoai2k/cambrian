...
const files=fs.readdirSync(DIR).filter(f=>f.startsWith('henodus.')&&!f.includes('.preview.')).sort();
const out={};
...
fs.writeFileSync('tools/triassic/creatures/henodus/delivery-files.json',JSON.stringify(out,null,2)+'\n');
