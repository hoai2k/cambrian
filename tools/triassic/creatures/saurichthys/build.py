...
# hinge plug went (667 px on Hybodus at Attack@0.43, unmoved by anything done to the lining, because none of
# it was about the rim; 373 px here on the same fault). `T.rim_flange` extrudes the boundary and draws the new ring in towards the
...
# instead of closing them (Macrocnemus measured that at 12 px in every clip at any gape). On this
# rostrum the cut runs to the tip of the snout, so the taper is measured from the tip.
#
...
def lumen_shells(y):
    ...
    # Tapered over the last twelfth at the front, the sac had run out well before the cut had: on a
    # rostrum this long that is two hundredths of the body of open lip with nothing behind it.
    ...
    # Both halves reach a little way into the flesh above and below the cut: drawn exactly to it
    # the lumen ends where the skin's cut edge begins, and on a rostrum with two hundred pixels of
    # one-polygon-thick lip a pixel straddling that boundary is part skin and part nothing at all.
    # Each half is held inside its own jaw's measured section whatever the swell asks.
    _roof_lift = max(0., min(.40 * (_top - _c), .9 * (_top - _c) - .0008))
    _floor_drop = min(1.75 * wz, .92 * (_c - _bot))
    return (wy, min(max(_roof_lift + .10 * wz, _throat * .9 * (_top - _c)), .90 * (_top - palate_line(y))),
            min(_floor_drop, .90 * (floor_line(y) - _bot)))
