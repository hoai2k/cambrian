...
const files=fs.readdirSync(DIR).filter(f=>f.startsWith('macrocnemus.')&&!f.includes('.preview.')).sort();
const out={};
...
fs.writeFileSync('tools/triassic/creatures/macrocnemus/delivery-files.json',JSON.stringify(out,null,2)+'\n');
