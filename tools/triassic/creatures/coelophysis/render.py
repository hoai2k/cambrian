"""Render the re-imported exported Coelophysis: portraits, the review poses and the shore chain.
...
LOCAL = ROOT / 'local/triassic-authoring/coelophysis'
...
SOURCE = (LOCAL / ('coelophysis' + SUFFIX + '.unpacked.glb')) if '--decoded' in sys.argv \
    else (OUT / ('coelophysis' + SUFFIX + '.glb'))
...
CHAIN = ([('Lower', t) for t in [0., .5, 1., 1.45]]
         ...
         + [('Run', t) for t in [0., .07, .14, .22, .29, .36, .43, .51]]
         + [('Charge', t) for t in [.2, .5, .85]]
         + [('Retreat', t) for t in [.25, .7]])
if '--chain-only' in sys.argv:
    for clip, t in CHAIN:
        pose(clip, t)
        render(REVIEW / ('%s-%s-cside.png' % (clip, t)), 900, 480, (8, 0, .6), (0, 0, .2), 6.2)
        pose(clip, t)
        render(REVIEW / ('%s-%s-ctop.png' % (clip, t)), 900, 640, (0, 0, 9), (0, 0, 0), 7.6, TOP)
    sys.exit(0)

pose('Idle', 0)
if SUFFIX and '--review-only' not in sys.argv:
    render(OUT / 'coelophysis.puppet.png', 1200, 900)
...
if not SUFFIX and '--review-only' not in sys.argv:
    for suffix, w, h in [('select.png', 1600, 1200), ('card.png', 800, 600), ('thumb.png', 256, 192), ('png', 1200, 900)]:
        render(OUT / ('coelophysis.' + suffix), w, h)
    if '--portrait-only' in sys.argv:
        sys.exit(0)

MOUTHS = [('mouth-closed', 'Idle', 0., 1.30, False), ('mouth-Bite', 'Bite', .25, 1.30, False),
          ('mouth-Snap', 'SnapLeft', .22, 1.30, False), ('mouth-Heavy', 'Heavy', .2, 1.30, False),
...
POSES = [('Idle', 0), ('Idle', 1.6), ('Crawl', .2), ('Crawl', .7), ('Crawl', 1.2),
         ...
         ('Charge', .2), ('Charge', .5), ('Charge', .85), ('Retreat', .25), ('Retreat', .7)]
for clip, t in POSES:
    pose(clip, t)
    render(REVIEW / ('%s-%s.png' % (clip, t)))
for name, loc, tgt, scale, rot in [('side', (8, 0, .1), (0, 0, 0), 6.2, None),
                                   ('top', (0, 0, 9), (0, 0, 0), 6.2, TOP),
                                   ('front', (0, -9, .5), (0, 0, 0), 2.8, None),
                                   ('belly', (0, 0, -9), (0, 0, 0), 6.2, BELLY)]:
    pose('Idle', 0)
