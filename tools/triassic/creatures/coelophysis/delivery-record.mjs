...
const files=fs.readdirSync(DIR).filter(f=>f.startsWith('coelophysis.')&&!f.includes('.preview.')).sort();
const out={};
...
fs.writeFileSync('tools/triassic/creatures/coelophysis/delivery-files.json',JSON.stringify(out,null,2)+'\n');
