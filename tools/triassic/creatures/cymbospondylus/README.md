...
/opt/blender/blender -b --factory-startup --python tools/triassic/creatures/cymbospondylus/render.py -- --decoded --twin
...
- **No portraits are written.** `render.py -- --portraits` will produce them into this directory's
  `portraits/`, but the roster's placeholder cards cut from the canonical pose stay where they are
  until a human decides this body ships.
