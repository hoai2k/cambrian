"""Render and inspect the re-imported exported models: the portraits and the review sheets.
  ...
  ... --python tools/triassic/creatures/keichousaurus/render.py -- --puppet --decoded
...
LOCAL = ROOT / 'local/triassic-authoring/keichousaurus'; SUFFIX = '.puppet' if '--puppet' in sys.argv else ''
REVIEW = LOCAL / ('puppet-review' if SUFFIX else 'authored-review'); REVIEW.mkdir(parents=True, exist_ok=True)
bpy.ops.object.select_all(action='SELECT'); bpy.ops.object.delete(use_global=False)
SOURCE = (LOCAL / ('keichousaurus' + SUFFIX + '.unpacked.glb')) if '--decoded' in sys.argv else (OUT / ('keichousaurus' + SUFFIX + '.glb'))
...
def render(file, w=800, h=600, loc=(5.6, -4.2, 3.2), target=(0, 0, .1), scale=5.6):
...
if SUFFIX and '--review-only' not in sys.argv:
    render(OUT / 'keichousaurus.puppet.png', 1200, 900)
...
if not SUFFIX and '--review-only' not in sys.argv:
    for suffix, w, h in [('select.png', 1600, 1200), ('card.png', 800, 600), ('thumb.png', 256, 192), ('png', 1200, 900)]:
        render(OUT / ('keichousaurus.' + suffix), w, h)
    if '--portrait-only' in sys.argv: sys.exit(0)
MOUTH = [('mouth-closed', 'Idle', 0), ('mouth-Bite', 'Bite', .25), ('mouth-Attack', 'Attack', .1)]
if '--mouth-only' in sys.argv:
    for name, clip, t in MOUTH:
        pose(clip, t); render(REVIEW / (name + '.png'), 1000, 750, (2.3, -3.5, 1.05), (.10, -2.15, .48), 1.15)
    sys.exit(0)
POSES = [('Idle', 0), ('Swim', 0), ('Swim', .4), ('Swim', .8), ('Swim', 1.2), ('Sprint', 0), ('Sprint', .25), ('Sprint', .5),
         ('TurnLeft', .7), ('TurnRight', .7), ('Dive', .6), ('Rise', .6), ('Attack', .14), ('Attack', .45),
         ('Bite', .25), ('Heavy', .15), ('Heavy', .45), ('Hit', .3), ('Death', 1.6), ('Guard', .5), ('Parry', .2),
         ('Dodge', .25), ('Eat', .4), ('Stagger', .6), ('Ability', .45), ('Grab', .6), ('Breath', 1.2), ('Growth', .75),
         ('Shoal', .2), ('Shoal', .7), ('Shoal', 1.1), ('Breathe', .9), ('Breathe', 2.1)]
for clip, t in POSES:
    pose(clip, t); render(REVIEW / ('%s-%s.png' % (clip, t)))
for name, loc, tgt, scale in [('side', (7, 0, .1), (0, 0, .1), 5.6), ('top', (0, 0, 8), (0, 0, 0), 5.6),
                              ('front', (0, -8, .9), (0, 0, .1), 2.6), ('belly', (0, 0, -8), (0, 0, 0), 5.6)]:
