...
const files=fs.readdirSync(DIR).filter(f=>f.startsWith('keichousaurus.')&&!f.includes('.preview.')).sort();
const out={};
...
fs.writeFileSync('tools/triassic/creatures/keichousaurus/delivery-files.json',JSON.stringify(out,null,2)+'\n');
