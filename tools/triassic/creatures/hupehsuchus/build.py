"""Rebuild Hupehsuchus: authored Tripo skin and measured voxel-volume twin on one shared rig.
...
Hupehsuchus is the strange one, and two published readings of it disagree. Fang et al. 2023 read
the skull as converging on a baleen whale and the grooves along the jaw margins as keratin racks:
continuous ram filter feeding, like a bowhead. Motani, Pyenson & Jiang 2025 re-examined it and
found no intraoral space for baleen, ambiguous grooves, flawed morphometrics, and a kilogram-scale
animal that cannot energetically afford balaenid filtering -- and put the skull closest to a
**pelican**. This build takes the 2025 reading, which is the stronger case and is also the roster's
(`filterGulp`, "a lunge through a school with the pouch open ... it feeds on shoals of small fish,
not on blooms"). So the mouth here is a **pouch**, not a sieve: it opens wide and low and the
throat drops, and the feeding clips are lunges rather than a cruise with the mouth held open.

The other thing this animal is, is **stiff**. Rows of dorsal osteoderms run from frac 0.25 ahead
of mid-body to frac 0.21 behind it, over bipartite neural spines, and the research is explicit
that the trunk does not undulate and the tail does. So the gain curve here is the flattest of the
four: nothing at all through the armour, and everything behind it.
...
SCALE = 4.0      # engine authoring units; the runtime normalises by the box
...
# `Ability` is the roster's own 2.4 s mobile filter gulp; `Gulp` is the shorter single lunge at a
# shoal, and the two are deliberately different performances rather than one clip twice.
CLIPS = {'Idle': 2.6, 'Swim': 1.6, 'Sprint': 1.1, 'TurnLeft': 1.5, 'TurnRight': 1.5,
         'Dive': 1.4, 'Rise': 1.4, 'Attack': 1.0, 'Bite': .5, 'Heavy': 1.2, 'Hit': .6,
         'Death': 1.8, 'Guard': 1.2, 'Parry': .4, 'Dodge': .5, 'Eat': 1.6, 'Stagger': 1.2,
         'Ability': 2.4, 'Grab': 1.1, 'Breath': 2.4, 'Growth': 1.4,
         'Gulp': 1.4, 'Breathe': 3.0}
...
LIMBS, caudal_cluster, armour, other = {}, None, [], []
for c in clusters:
    mid = (c['yRange'][0] + c['yRange'][1]) / 2
    lateral = abs(c['centroid'][0] - cx(mid))
    if c['reachRadius'] > .12 and lateral > .04 and abs(mid) > .05:
        key = ('pec' if mid < 0 else 'pel') + ('L' if c['centroid'][0] < cx(mid) else 'R')
        LIMBS[key] = c
    elif c['yRange'][1] > Y1 - .06:
        caudal_cluster = c
    elif c['centroid'][2] > cz(mid) and c['yRange'][0] > Y0 + .18:
        # The **osteoderm rows**, and they are not fins. They read as blade-thin because they are
        # plates standing off the back, so the same measurement that finds a paddle finds them;
        # what tells them apart is that they sit on the midline above the axis and reach nowhere.
        # They get no bone: they ride the trunk, which is the whole point of them.
        armour.append(c)
    else:
        other.append(c)
if sorted(LIMBS) != ['pecL', 'pecR', 'pelL', 'pelR'] or not armour:
    print('HUPE_CLUSTERS', json.dumps(
...
assert len(armour) >= 3, ('the dorsal osteoderm rows did not measure', len(armour))
ARMOUR_SPAN = [min(c['yRange'][0] for c in armour), max(c['yRange'][1] for c in armour)]
ARMOUR_VERTICES = sum(c['count'] for c in armour)
...
# outward normal back into the mesh, and a vertex that hits is looking across the mouth slit at
# the lip opposite. **It does not reach this animal.** Over the whole front third it answers with
# 29 vertices, and they lie at y -0.29 to -0.13 -- behind the rostrum entirely, in the cheek
# crease -- where the snout itself runs from -0.448 to about -0.30. This generation has no
# modelled mouth: the rostrum is one smooth closed tube with the mouth painted on it, exactly as
# Dinocephalosaurus' was. So the mouth line is read off the albedo instead.
MOUTH_GAP = .030
CAV = T.mouth_cavity(auth, front_fraction=.34, gap=MOUTH_GAP)
# The accepted skin's lip continues behind the eye. The old .165 measurement
# window stopped on the narrow rostrum and mistook its end for a jaw hinge.
SNOUT_BACK = Y0 + .255
CAV_ON_SNOUT = int((CAV[:, 1] < SNOUT_BACK).sum()) if len(CAV) else 0
MOUTH_METHOD = 'albedo light/dark boundary (the geometric method found no modelled cavity)'
assert CAV_ON_SNOUT < 40, ('a modelled cavity turned up after all -- use it', CAV_ON_SNOUT)

# Walking **outwards from the belly** on each flank, because the pale belly is one solid block
# with a sharp step off it where the dark back is mottled and crosses mid value several times.
LIP = T.albedo_mouth_line(auth, luminance_at, Y0 + .012, SNOUT_BACK, cz, half_depth, stations=34)
assert len(LIP) >= 14, ('the mouth line did not measure off the albedo', len(LIP))
_ly = np.array([r['y'] for r in LIP])
_lm = T.blur1d(np.array([r['mid'] for r in LIP]), 1.4)
# **The cut follows the lip the model has, not a straight line near it.** A plane pass cannot cut a
# curve, so the head is sheared vertically by -seam(y) first, which carries the measured line onto
# z = 0 exactly; the cut is taken there and the shear undone. The straight ramp is still fitted, but
# only to record what a straight cut would have cost -- which is the number below, and on a lipped
# snout it is not small.
_fit = np.polyfit(_ly, _lm, 1)
MOUTH_SLOPE, MOUTH_INTERCEPT = float(_fit[0]), float(_fit[1])
_residual = _lm - np.polyval(_fit, _ly)
_radius = np.array([max(half_depth(float(y)), 1e-4) for y in _ly])
MOUTH_FIT_WORST = float(np.max(np.abs(_residual) / _radius))
RAMP_DEVIATION_RAW = float(np.max(np.abs(_residual)))
MOUTH_DISAGREEMENT = float(np.max([r['disagreement'] for r in LIP]))
assert MOUTH_FIT_WORST < .40, ('the measured mouth line is not a ramp', MOUTH_FIT_WORST)


def seam(y):
    """The mouth line: the measured light/dark boundary itself, lightly smoothed."""
    return float(np.interp(y, _ly, _lm))


# By construction the cut is the measurement, so its deviation from the measured line is whatever
# the smoothing moved it by, and that is what is recorded rather than asserted to be zero.
CUT_DEVIATION_RAW = float(np.max(np.abs(np.array([r['mid'] for r in LIP]) - _lm)))


HINGE_Y = float(SNOUT_BACK)
JAW_FRONT_Y = float(Y0 + .008)
# What relief the generated snout carries, recorded rather than added to. This animal is
# heterodont in life -- pointed in front, blunt and crushing behind -- and the generation models
# none of it; see the README.
...
NECK_Y, CHEST_Y, BODY_Y = HINGE_Y + .030, HINGE_Y + .055, Y0 + .380
TAIL_Y = [BODY_Y + .075 + .058 * i for i in range(7)]
...
bone('caudal_upper', on_axis(CAUDAL_Y, +.030), 'tail_05')
bone('caudal_lower', on_axis(CAUDAL_Y, -.030), 'tail_05')
# A **pouch** joint under the throat. The 2025 reading of this animal is a pelican, and a pelican's
# gulp is the floor of the mouth dropping, not the jaw swinging further. Nothing is modelled for it:
# the joint carries the generation's own throat skin and is translated down, which is animation
# rather than geometry, and it is the one bone in these four deliveries that keyframes a location.
POUCH_Y = HINGE_Y + .012
POUCH_ROOT = Vector((cx(POUCH_Y), POUCH_Y, seam(POUCH_Y) - .004))
bone('pouch', POUCH_ROOT, 'jaw')
POUCH_SEATING = depth(POUCH_ROOT)
...
AXIAL_NAMES = ['skull', 'neck', 'chest', 'body'] + ['tail_%02d' % i for i in range(7)]
...
def pouch_weights(q, thin):
    """The floor of the mouth and the skin behind the chin: what drops when the pouch fills. It is
    taken as the ventral half of the head behind the jaw's front and in front of the shoulder, and
    it blends off both ends so nothing tears."""
    if not JAW_FRONT_Y < q.y < HINGE_Y + .085:
        return None
    below = seam(q.y) - q.z
    if below <= 0:
        return None
    g = (T.smooth(below / max(head_half_depth(q.y) * .9, .004))
         * T.smooth((q.y - JAW_FRONT_Y) / .020)
         * T.smooth((HINGE_Y + .085 - q.y) / .045))
    return ({'pouch': 1.}, g * .85) if g > 0 else None
...
for o in (auth, puppet):
    ...
    # Close the actual cut cross-sections; an ellipsoid hidden with oral meshes
    # cannot stand in for the body wall behind the open jaw.
    T.cap_cut(o, lambda p: abs(p.y - HINGE_Y) < 1e-5, Vector((0, -1, 0)))
    T.cap_cut(parts['lower jaw'][o.name], lambda p: abs(p.y - HINGE_Y) < 1e-5, Vector((0, 1, 0)))
...
for o, thin in ((auth, thickness), (puppet, puppet_thickness)):
    ...
    for v in o.data.vertices:
        w = relaxed[v.index]
        # The two copies of the throat cut must have the same deformation at
        # their shared rim. Diffusion may not change only the body's copy.
        if abs(v.co.y - HINGE_Y) < 1e-5 and v.co.z < seam(v.co.y):
            w = weights(v.co, THIN * 2)
        counts.append(len(w))
...
for o in parts['lower jaw'].values():
    for n in B:
        o.vertex_groups.new(name=n)
    for v in o.data.vertices:
        # The jaw becomes the pouch at its posterior attachment, rather than
        # rotating away from a throat carried by a different bone field.
        blend = T.smooth((HINGE_Y - v.co.y) / .045)
        w = {n: value * (1. - blend) for n, value in weights(v.co, THIN * 2).items()}
        w['jaw'] = w.get('jaw', 0.) + blend
        top = sorted(w.items(), key=lambda item: -item[1])[:4]
        total = sum(value for _, value in top)
        for n, value in top:
            if value > 1e-8: o.vertex_groups[n].add([v.index], value / total, 'REPLACE')
        v.co = tx(v.co)
    for p in o.data.polygons:
        p.use_smooth = True
...
def mouth_section(y):
    """There is no measured cavity to size the lining from, so it is sized off the **head's own**
    measured radius profile, drawn in at both ends so it closes rather than ending in a ring
    standing in open flesh. Dinocephalosaurus does the same and for the same reason.

    Then it is *fitted*: the ring is shrunk station by station until every point on it is inside
    the closed intake surface. That is only safe because this generation has no modelled mouth --
    where Placodus has one, a point in the lumen is outside the closed shell and this test would
    read backwards, which is why Cymbospondylus' lining is sized from its measured cavity instead.
    """
    w, h = _raw_section(y)
...
ANCHOR_POINTS = {
    ...
    'anchor_mouth_inside': ('skull', (cx(HINGE_Y - .08), HINGE_Y - .08, seam(HINGE_Y - .08)), 'swallow'),
    # **Not the skull.** This animal has `noBite`: its light and heavy attacks are a gape and a
    # gulp, and what delivers the blow is the open mouth closing, which is the mandible. So the
    # attack anchor rides `jaw` and travels with the gape rather than with the braincase.
    'anchor_attack_primary': ('jaw', (cx(SNOUT_Y), SNOUT_Y - .004, seam(SNOUT_Y) - .004), 'attack'),
}
...
CHAIN = ['neck', 'chest', 'body'] + ['tail_%02d' % i for i in range(7)]
...
# The stiffest of the four, and deliberately so: the armoured trunk contributes **nothing** to
# the wave, the first two caudal joints barely more, and the last three carry the whole of it.
# That is the research's "stiff armoured trunk, undulation of the tail", and it is why this animal
# reads slow beside Mixosaurus even at the same tail amplitude.
WAVE_SPAN = 2.6          # radians of phase from the neck to the last caudal joint
GAIN = [0., 0., .01, .05, .16, .34, .58, .82, .96, 1.0]
LAG = [WAVE_SPAN * max(0., (i - 2)) / 7 for i in range(10)]
...
for clip, duration in CLIPS.items():
    ...
    for f in range(last + 1):
        ...
        amp = {'Idle': .26, 'Swim': 1.0, 'Sprint': 1.45, 'Eat': .34, 'Guard': .20, 'Grab': .30,
               'Breath': .34, 'Breathe': .30, 'Growth': .24, 'Dodge': 1.10, 'Gulp': .55, 'Ability': .40,
        ...
        GULPS = ('Attack', 'Heavy', 'Gulp', 'Ability', 'Eat')
        cock = spike(u, .00, .36, 1.4) if clip in ('Attack', 'Heavy', 'Gulp') else 0.
        drive = ramp(u, .26, .44, 2.2) * (1 - ramp(u, .62, 1., 1.)) if clip in ('Attack', 'Heavy', 'Gulp') else 0.
        snap = spike(u, .34, .58, 2.6) if clip in ('Attack', 'Heavy', 'Gulp') else 0.
        # **The pouch**, which is what this animal feeds with. It fills fast behind the open jaws
        # and is squeezed out slowly, which is the asymmetry that makes a gulp read as a gulp
        # rather than as a yawn. `Ability` is the roster's 2.4 s mobile version and fills twice.
        if clip in ('Attack', 'Heavy', 'Gulp'):
            pouch = ramp(u, .22, .40, 1.8) * (1 - ramp(u, .52, .90, 1.))
        elif clip == 'Ability':
            pouch = max(ramp(u, .08, .22, 1.8) * (1 - ramp(u, .34, .52, 1.)),
                        ramp(u, .52, .66, 1.8) * (1 - ramp(u, .78, .96, 1.)))
        elif clip == 'Eat':
            pouch = .45 * (1 - cos(p * 2)) / 2
        else:
            pouch = 0.
        cstart = launch = 0.
        ...
        if clip == 'Bite':
            ...
            gape = .30 * cock + .70 * ramp(u, .20, .40, 1.6) * (1 - ramp(u, .52, .76, 1.2))
        elif clip == 'Ability':
            # The roster's mobile filter gulp: the pouch does the work and the jaws simply stay
            # wide through the pass, which is what separates it from a bite.
            gape = .74 * ramp(u, .06, .20, 1.4) * (1 - ramp(u, .80, .96, 1.2))
        elif clip == 'Grab':
            gape = .10 + .05 * haul
        elif clip == 'Eat':
        ...
        if clip == 'Gulp':
            body.location.y = .10 * cock - .52 * drive
            body.rotation_euler.x = .16 * cock - .12 * drive
        if clip == 'Ability':
            # mobileAbility: it swims *through* the shoal rather than stopping to feed
            body.location.y = -.40 * ramp(u, .05, .55, 1.) - .10 * pouch
            body.rotation_euler.x = -.06 * pouch
        if clip == 'Ability':
        ...
        for i, n in enumerate(CHAIN):
            q = pb[n]
            z = .230 * GAIN[i] * amp * wave(i, beat)
            z += turn * (.018 + i * .009)
            z += .040 * dead * sin(i * .8)
            if clip in ('Gulp', 'Ability'):
                # The tail keeps working through the lunge: this animal has no bite and no reach,
                # so what carries it onto the shoal is the beat, not a neck or a jaw.
                z += .22 * GAIN[i] * amp * wave(i, beat) * (drive + pouch)
            if clip in ('Attack', 'Heavy'):
        ...
        if clip in ('Gulp', 'Ability'):
            # The head tips up as the pouch fills, which is what a pelican does and what makes the
            # throat read as a bag rather than as an open jaw.
            pb['skull'].rotation_euler.x += -.22 * pouch
            pb['neck'].rotation_euler.x += -.10 * pouch
        if clip == 'Ability':
        ...
        # The pouch. A pelican's gulp is the floor of the mouth dropping, so this joint swings
        # and drops rather than the jaw opening further, and it is the one bone here with a
        # location channel. It fills on the lunge and squeezes out on the recovery.
        pb['pouch'].rotation_euler.x = .55 * pouch
        pb['pouch'].location.z = -.055 * pouch * SCALE
        pb['pouch'].location.y = .010 * pouch * SCALE
        ...
        for key, names in LIMB_NAMES.items():
            ...
            if clip in ('Gulp', 'Ability'):
                up.rotation_euler.x += .22 * pouch
                up.rotation_euler.z += s * .16 * pouch
            if clip == 'Guard':
...
meta = {
    'id': ID, 'name': NAME, 'species': 'Hupehsuchus nanchangensis',
    'provenance': 'Early Triassic · Jialingjiang Formation, Hubei',
    'description': 'Armoured ichthyosauromorph with rows of dorsal osteoderms over a stiff trunk, '
                   'a long toothless snout, short paddles and a tapering tail. Authored Tripo body '
                   'and measured procedural volume twin share one armature, one set of inverse '
                   'binds, one set of sockets and one set of actions.',
    'modelLength': BODY_LENGTH, 'lengthMeters': 1.0, 'locomotion': 'Swim',
    'clips': list(CLIPS), 'looping': LOOPS, 'anchors': [a['name'] for a in anchors],
    'puppet': ID + '.puppet.glb',
    'sources': ['docs/triassic/canonical/hupehsuchus.png',
                'tools/triassic/creatures/hupehsuchus/tripo-raw/hupehsuchus.raw.glb'],
    'notes': [
        'The armoured trunk does not undulate. The gain curve is zero through the osteoderm '
        'rows and everything is behind them, which is the research\'s "stiff armoured trunk, '
        'undulation of the tail" and is why this animal reads slow beside Mixosaurus.',
        'The osteoderm rows are the generation\'s own plates. They are found by measurement, they '
        'get no bone of their own, and they ride the trunk -- which is what a stiff armoured back '
        'is. No plate was modelled, moved or added.',
        'The feeding reading is Motani, Pyenson & Jiang 2025, not Fang et al. 2023: a pelican, '
        'not a bowhead. So the mouth is a pouch that fills and empties, the clips are lunges '
        'rather than a cruise with the mouth held open, and there is no baleen and no sieve.',
        'The pouch is animation, not geometry: one joint under the throat carries the '
        'generation\'s own skin and is swung and translated. Nothing was modelled for it.',
        'This generation has **no modelled mouth**: Placodus\' geometric method finds nothing on '
        'the rostrum, so the mouth line was read off the albedo as Dinocephalosaurus\' was. The '
        'animal is toothless in life and the generation carries no dentition, which agree.',
...
profile_report = {
    ...
    'appendageRootSeatingRaw': {**LIMB_SEATING, 'pouch': POUCH_SEATING},
    'appendageRootSeatingFractionOfBodyLength':
        {k: v * SCALE / BODY_LENGTH for k, v in LIMB_SEATING.items()},
    'dorsalArmour': {'patches': len(armour), 'vertices': ARMOUR_VERTICES,
                     'span': ARMOUR_SPAN,
                     'spanAsFractionOfBody': [(ARMOUR_SPAN[0] - Y0), (ARMOUR_SPAN[1] - Y0)],
                     'patchRows': [{k: c[k] for k in ('count', 'yRange', 'xRange', 'zRange')}
                                   for c in armour]},
...
print('HUPE_FRAME', json.dumps({k: v for k, v in frame.items() if k != 'perStation'}))
print('HUPE_REPORT', json.dumps({k: report[k] for k in
      ('authoredTriangles', 'twinTriangles', 'twinTriangleFraction', 'bones', 'maxInfluences')}))
print('HUPE_ENVELOPE', json.dumps(report['envelope']))
print('HUPE_MOUTH', json.dumps({k: report['mouth'][k] for k in
...
print('HUPE_POSE', json.dumps({'spine': {k: v for k, v in POSE_DEVIATION['spine'].items() if k != 'perStation'},
      'tail': {k: v for k, v in POSE_DEVIATION['tail'].items() if k != 'perStation'},
      'limbAsymmetry': LIMB_ASYMMETRY.get('allPairs')}))
print('HUPE_SEAMS', json.dumps({k: round(v, 9) for k, v in seams.items()}))
