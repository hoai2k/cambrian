/**
 * Package the Hybodus family, prove the authored body and its twin are the same rig playing the
 * same samples, then play every clip through Three.js and measure the things this animal is
 * supposed to do: a tail beat that grows backwards, a head that holds the line of travel, a lunge
 * that actually commits, a jaw that opens, and a roll-and-shake that rolls.
 *
...
for (const suffix of ['', '.puppet']) {
  ...
  if (!suffix) {
    ...
    // The shark's own move: Shake has to roll the trunk and swing the head, more than once.
    {
      const rows = track('Shake', ['skull', 'body'], 120);
      const roll = rows.map(r => r.bodyRoll);
      let crossings = 0;
      for (let i = 1; i < roll.length; i++) if ((roll[i] > 0) !== (roll[i - 1] > 0)) crossings++;
      report.shake = {
        rollAmplitude: (Math.max(...roll) - Math.min(...roll)) / 2,
        rollReversals: crossings,
        headSwing: spread(rows, 'skull', 0),
      };
    }
    // The spine brace: both dorsal fins have to pitch, and the animal has to hold it.
    {
      const rows = track('SpineBrace', ['dorsal_1', 'dorsal_2', 'body'], 60);
      const pitch = (n) => {
        const q = new THREE.Quaternion();
        return rows.map(r => r[n]);
      };
      report.spineBrace = {
        dorsal1Travel: spread(rows, 'dorsal_1', 1) + spread(rows, 'dorsal_1', 2),
        dorsal2Travel: spread(rows, 'dorsal_2', 1) + spread(rows, 'dorsal_2', 2),
      };
    }
    report.jaw = [];
...
need(report.shake.rollAmplitude > 0.35, `Shake must roll the trunk (${report.shake.rollAmplitude.toFixed(3)} rad)`);
need(report.shake.rollReversals >= 4, `Shake must roll more than once (${report.shake.rollReversals})`);
need(report.spineBrace.dorsal1Travel > 0.02 && report.spineBrace.dorsal2Travel > 0.02, 'SpineBrace must move both dorsal fins');
...
for (const name of ['Attack', 'Heavy', 'Eat', 'Grab', 'Shake']) {
