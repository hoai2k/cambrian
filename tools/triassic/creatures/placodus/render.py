...
LOCAL=ROOT/'local/triassic-authoring/placodus';SUFFIX='.puppet' if '--puppet' in sys.argv else ''
REVIEW=LOCAL/('puppet-review' if SUFFIX else 'authored-review');REVIEW.mkdir(parents=True,exist_ok=True)
bpy.ops.object.select_all(action='SELECT');bpy.ops.object.delete(use_global=False)
SOURCE=(LOCAL/('placodus'+SUFFIX+'.unpacked.glb')) if '--decoded' in sys.argv else (OUT/('placodus'+SUFFIX+'.glb'))
...
GAIT=[('Crawl',t) for t in [0.,.25,.5,.75,1.,1.25,1.5,1.75]]
if '--gait-only' in sys.argv:
 for clip,t in GAIT:
  pose(clip,t);render(REVIEW/('%s-%s-gside.png'%(clip,t)),900,560,(7,0,.6),(0,0,.2),6.4)
  pose(clip,t);render(REVIEW/('%s-%s-gtop.png'%(clip,t)),900,560,(0,0,8),(0,0,0),6.4)
 sys.exit(0)
pose('Idle',0)
if SUFFIX and '--review-only' not in sys.argv:
 render(OUT/'placodus.puppet.png',1200,900)
...
if not SUFFIX and '--review-only' not in sys.argv:
 for suffix,w,h in [('select.png',1600,1200),('card.png',800,600),('thumb.png',256,192),('png',1200,900)]:
  render(OUT/('placodus.'+suffix),w,h)
 if '--portrait-only' in sys.argv:sys.exit(0)
POSES=[('Idle',0),('Swim',0),('Swim',.45),('Swim',.9),('Swim',1.35),('Sprint',0),('Sprint',.3),('Sprint',.6),('Sprint',.9),
 ('TurnLeft',.8),('TurnRight',.8),('Dive',.7),('Rise',.7),('Attack',.14),('Attack',.4),('Attack',.7),('Bite',.25),
 ('Heavy',.15),('Heavy',.45),('Heavy',.8),('Hit',.3),('Death',1.6),('Guard',.5),('Parry',.2),('Dodge',.25),('Eat',.4),
 ('Stagger',.6),('Ability',.45),('Grab',.6),('Breath',1.2),('Growth',.75),
 ('Crawl',.1),('Crawl',.5),('Crawl',1.),('Crawl',1.5),('Pry',.4),('Pry',1.),('Pry',1.9),
 ('CrushBite',.2),('CrushBite',.8),('CrushBite',1.25),('Breathe',.9),('Breathe',2.1)]
for clip,t in POSES:
 pose(clip,t);render(REVIEW/('%s-%s.png'%(clip,t)))
for name,loc,tgt,scale in [('side',(7,0,.1),(0,0,0),6.8),('top',(0,0,8),(0,0,0),6.8),
 ('front',(0,-8,.9),(0,0,.1),3.4),('belly',(0,0,-8),(0,0,0),6.8)]:
 pose('Idle',0);render(REVIEW/(name+'.png'),1000,750,loc,tgt,scale)
for name,clip,t in [('mouth-closed','Idle',0),('mouth-Bite','Bite',.25),('mouth-CrushBite','CrushBite',.25)]:
 pose(clip,t);render(REVIEW/(name+'.png'),1000,750,(4.5,-4.5,.7),(0,-2.2,-.12),1.9)
for clip,t in GAIT:
 pose(clip,t);render(REVIEW/('%s-%s-gside.png'%(clip,t)),900,560,(7,0,.6),(0,0,.2),6.4)
 pose(clip,t);render(REVIEW/('%s-%s-gtop.png'%(clip,t)),900,560,(0,0,8),(0,0,0),6.4)
