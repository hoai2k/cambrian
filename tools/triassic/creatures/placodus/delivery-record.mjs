...
const files=fs.readdirSync(DIR).filter(f=>f.startsWith('placodus.')&&!f.includes('.preview.')).sort();
const out={};
...
fs.writeFileSync('tools/triassic/creatures/placodus/delivery-files.json',JSON.stringify(out,null,2)+'\n');
