...
const files=fs.readdirSync(DIR).filter(f=>f.startsWith('dinocephalosaurus.')&&!f.includes('.preview.')).sort();
const out={};
...
fs.writeFileSync('tools/triassic/creatures/dinocephalosaurus/delivery-files.json',JSON.stringify(out,null,2)+'\n');
