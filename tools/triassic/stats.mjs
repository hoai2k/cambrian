...
// Two files, because two of these animals are not on the roster. Archelon and Mosasaurus are Late
// Cretaceous and live in src/content/triassic/guests.ts, where they reach the game as standing
// visitors and nothing else; their movement fields are generated by exactly the same rule as the
// roster's, which is the point of keeping them here rather than typing six numbers by hand.
const FILES = ['src/content/triassic/creatures.ts', 'src/content/triassic/guests.ts'];
...
// Four-flipper underwater flight, which is `flight: true` on the creature definition: high
// agility and a short glide, because a body carried by two hydrofoils answers at once and
// stops as soon as they do. Archelon is the second of them and is off the roster.
...
for (const row of rows) {
  // A guest carries its adultLength in expansion.json rather than in its own definition, so its
  // block has no `adultLength:` line to rewrite; both shapes are accepted and one of them has to
  // match somewhere.
  const withLength = new RegExp(`(id: '${row.id}',[\\s\\S]*?adultLength: )[0-9.]+([\\s\\S]*?speed: )[0-9.]+, burst: [0-9.]+, agility: [0-9.]+, turnRate: [0-9.]+, glide: [0-9.]+`);
  const plain = new RegExp(`(from\\('${row.id}'\\),)([\\s\\S]*?speed: )[0-9.]+, burst: [0-9.]+, agility: [0-9.]+, turnRate: [0-9.]+, glide: [0-9.]+`);
  const stats = `${row.cruise}, burst: ${row.burstMul}, agility: ${row.agility}, turnRate: ${row.turn}, glide: ${row.glide}`;
  // **Whether the pattern matched, not whether the text changed.** Rerunning this script on an
  // unchanged roster rewrites every block with the numbers it already had, so a change test reads
  // as "no stat block found" for all 25 of them.
  let hit = false;
  for (const [file, text] of sources) {
    if (withLength.test(text)) {
      sources.set(file, text.replace(withLength, (_, a, b) => `${a}${row.L}${b}${stats}`));
    } else if (plain.test(text)) {
      sources.set(file, text.replace(plain, (_, a, b) => `${a}${b}${stats}`));
    } else continue;
    hit = true;
    break;
  }
  if (!hit) throw new Error(`no stat block found for ${row.id}`);
}
for (const [file, text] of sources) fs.writeFileSync(file, text);
// A guest's adultLength lives in expansion.json, and the same rule has to produce it: a length
// typed there that the formula does not agree with is the one way these two can drift.
{
  const ex = JSON.parse(fs.readFileSync('src/content/triassic/expansion.json', 'utf8'));
  for (const s of ex.subjects) {
    const want = r2(K * Math.pow(s.lengthMeters, EXP));
    if (Math.abs(want - s.adultLength) > 1e-9) {
      throw new Error(`${s.id}: expansion.json says adultLength ${s.adultLength}, the era's rule says ${want}`);
    }
  }
}
