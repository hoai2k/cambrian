"""Selectively map the authorised original pigment swatch onto accepted clay-02.
...
Image pixels contribute colour only, never a second normal/height field.
"""
...
SWATCH=LOCAL/'material-sources/gemuendina-skin-imagegen-01.png'
SWATCH_SHA='6820a68305bfd1265510934d3df653e0e80f2768158722b54684fbab0cdf0210'
PROVENANCE=HERE/'imagegen-skin-01.json'
PROVENANCE_SHA='f41ca645788c2a1b28a5fc57512f559125ccdee96a69d67e58c8bf5920aa5315'
OUT=LOCAL/'material-03'
sys.dont_write_bytecode=True
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
if sha(SOURCE)!=SOURCE_SHA:raise RuntimeError('Accepted clay-02 hash mismatch')
if sha(SWATCH)!=SWATCH_SHA or sha(PROVENANCE)!=PROVENANCE_SHA:raise RuntimeError('Authorised swatch/provenance hash mismatch')
if OUT.exists() and any(OUT.iterdir()):raise RuntimeError('material-03 already contains evidence')
...
swatch_image=bpy.data.images.load(str(SWATCH),check_existing=False)
swatch_image.colorspace_settings.name='sRGB';swatch_image.pack()
...
class Field:
    ...
    def finish(self,color,rough,height,distance=.010):
        ...
        self.set(bump,'Strength',.25);self.set(bump,'Distance',distance)
...
base=field.mix((.015,.034,.012,1),(.107,.134,.040,1),field.smooth(macro,.35,.65))
base=field.mix(base,(.157,.154,.060,1),field.mul(.28,field.smooth(cloud,.58,.77)))
base=field.mix(base,(.018,.039,.017,1),field.mul(.35,field.sub(1,field.smooth(cloud,.33,.51))))
# Subtle pale organic mottles use their own nonperiodic spatial scale.
mottle=field.noise(organic,8.3,4.1,.74)
base=field.mix(base,(.168,.168,.076,1),field.mul(.17,field.smooth(mottle,.65,.79)))
...
base=field.mix(base,(.017,.030,.009,1),field.mul(field.mul(edge,mosaic_strength),.02))
base=field.mix(base,(.145,.148,.053,1),field.mul(field.mul(celltone.outputs[0],mosaic_strength),.06))
...
base=field.mix(base,(.130,.131,.043,1),field.mul(.17,patchinside))
base=field.mix(base,(.025,.038,.013,1),field.mul(.12,patchline))
# Fin ornament has the same pigment family but much quieter tubercle relief.
base=field.mix(base,(.108,.127,.045,1),field.mul(.10,fin))
under=field.mix((.120,.130,.055,1),(.240,.215,.100,1),macro)
color=field.mix(under,base,dorsal)
# Swatch mapping is independent of packed bake UVs: continuous object-space
# coordinates maintain uniform cell density over the raised cranial/core body.
# Approximately 0.34 source tiles per authoring unit means roughly 20–30 small
# tesserae per unit, not a handful of large reptile/rock cells. The source's
# requested seamlessness is unproven, so its own boundary pixels are never
# trusted: four half-tile phase samples crossfade through every tile edge.
sp=field.nodes.new('ShaderNodeSeparateXYZ');field.set(sp,'Vector',organic)
sx,sy,sz=[sp.outputs[k]for k in 'XYZ']
# A modest oblique rotation prevents a body-aligned texture lattice.
su=field.add(.53,field.mul(.34,field.add(field.add(field.mul(.94,sx),field.mul(.342,sy)),field.mul(.20,sz))))
sv=field.add(.77,field.mul(.34,field.add(field.sub(field.mul(.94,sy),field.mul(.342,sx)),field.mul(.14,sz))))
fu=field.math('FRACT',su);fv=field.math('FRACT',sv)
wu=field.sub(1,field.smooth(field.math('MINIMUM',fu,field.sub(1,fu)),.020,.075))
wv=field.sub(1,field.smooth(field.math('MINIMUM',fv,field.sub(1,fv)),.020,.075))
def swatch_sample(du,dv):
    uv=field.nodes.new('ShaderNodeCombineXYZ')
    field.set(uv,'X',field.math('FRACT',field.add(fu,du)))
    field.set(uv,'Y',field.math('FRACT',field.add(fv,dv)))
    tx=field.nodes.new('ShaderNodeTexImage');tx.image=swatch_image
    tx.interpolation='Linear';tx.extension='EXTEND';field.set(tx,'Vector',uv.outputs['Vector'])
    return tx.outputs['Color']
swatch_color=field.mix(field.mix(swatch_sample(0,0),swatch_sample(.5,0),wu),
                       field.mix(swatch_sample(0,.5),swatch_sample(.5,.5),wu),wv)
# Lift the darkest source boundaries slightly and compress pale mineral spots.
# Neither source luminance nor its small apparent relief enters the bump chain.
swatch_color=field.mix(swatch_color,(.039,.054,.016,1),.14)
swatch_amount=field.add(.035,field.mul(.72,field.mul(dorsal,field.sub(1,field.mul(.78,fin)))))
color=field.mix(color,swatch_color,swatch_amount)
# No ornamental eye rings: soft local pigment subtly follows the real eye bed.
eye_radius=field.ellipse(ax,y,.55,-1.30,.21,.23,2)
eye_field=field.mul(.08,field.gauss(eye_radius))
color=field.mix(color,(.046,.067,.023,1),eye_field)
oralbed=field.gauss(field.ellipse(x,y,0,-1.75,.46,.105,4))
color=field.mix(color,(.145,.124,.069,1),field.mul(.20,oralbed))
rough=field.add(.49,field.add(field.mul(.050,grain),field.mul(.020,mosaic_strength)))
rough=field.add(rough,field.mul(.055,field.sub(1,dorsal)))
microtops=field.voronoi(organic,95,'F1')
micrograin=field.sub(1,field.smooth(microtops.outputs['Distance'],.18,.44))
relief=field.add(field.mul(.22,grain),field.mul(.13,field.mul(micrograin,mosaic_strength)))
relief=field.sub(relief,field.mul(.020,patchline))
relief=field.mul(relief,field.sub(1,field.mul(.75,oralbed)))
field.finish(color,rough,relief,.0042)
...
angle=eye.math('ARCTAN2',eye.sub(ey,.5),eye.sub(ex,.5))
fibres=eye.add(.5,eye.mul(.5,eye.math('SINE',eye.add(eye.mul(angle,74),eye.mul(iristone,2.3)))))
iriscol=eye.mix((.029,.051,.015,1),(.127,.124,.034,1),eye.add(eye.mul(.78,iristone),eye.mul(.22,fibres)))
eyecolor=eye.mix((.008,.015,.010,1),iriscol,eye.mul(iris,upper))
eyecolor=eye.mix(eyecolor,(.002,.006,.004,1),eye.mul(pupil,upper))
eye_roughness=eye.add(.285,eye.mul(.03,eye.sub(1,eye.mul(iris,upper))))
eye.finish(eyecolor,eye_roughness,eye.noise(ep,76,1,.5),.00010)
eye.bs.inputs['Coat Weight'].default_value=.020
...
for obj in [body]+eyes:
    ...
    obj['material_version']='Gemuendina original regional material-03'
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'gemuendina-procedural-material-03.blend'))
...
def bake_family(obj,fields,family,size):
    ...
    bs.inputs['Specular IOR Level'].default_value=.32;bs.inputs['Coat Weight'].default_value=.020 if family=='eye' else 0
...
report={'phase':'material review pending','accepted_clay_sha256':SOURCE_SHA,
        'script_sha256':sha(HERE/'materials_03.py'),'geometry_sha256':initial_geometry,
        ...
        'provenance':'Authorised original ImageGen swatch mixed selectively with anatomical procedural pigment; no user-reference paleoart pixels',
        'swatch':{'path':str(SWATCH),'sha256':SWATCH_SHA,'size':list(swatch_image.size),
                  'prompt_provenance':str(PROVENANCE),'provenance_sha256':PROVENANCE_SHA,
                  'use':'Base colour only; four-phase edge crossfade; no source-derived height/normal',
                  'tiles_per_authoring_unit':.34,'dorsal_core_mix':.755,'fin_mix_max':.1934,'ventral_mix':.035},
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'gemuendina-material-03.blend'))
report['blend_sha256']=sha(OUT/'gemuendina-material-03.blend')
