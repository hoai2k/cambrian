...
SRC=LOCAL/name;OUT=LOCAL/('audit-'+name+'-02')
...
report={'candidate':name,'method':'Actual exported eye-polyhedron volume rejection; continuous body three-direction triangle parity; canonical Wilson/conservative bounds. Actual exported channels and skin inverse binds evaluated.',
        'source_sha256':sha(__file__),'shared_audit_sha256':sha(shared),'records':[],
 ...
 def evaluated(clip=None,phase=0.):
  ...
  if clip:
   ...
   for ch in action['channels']:
    sampler=action['samplers'][ch['sampler']];assert sampler.get('interpolation','LINEAR')=='LINEAR'
    times=data(sampler['input'])[:,0];values=data(sampler['output']);t=phase*CLIPS[clip]
    j=max(0,min(len(times)-2,int(np.searchsorted(times,t,side='right')-1)))
    f=float(np.clip((t-times[j])/(times[j+1]-times[j]),0,1));a,b=values[j],values[j+1];prop=ch['target']['path']
    if prop=='rotation':
     q=Quaternion((a[3],*a[:3])).slerp(Quaternion((b[3],*b[:3])),f);value=[q.x,q.y,q.z,q.w]
    else:value=((1-f)*a+f*b).tolist()
...
print('GEMUENDINA_ACTUAL_EYE_ATTACHMENT_AUDIT_OK',OUT)
