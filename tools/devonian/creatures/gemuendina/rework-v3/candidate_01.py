...
if OUT.exists() and any(OUT.iterdir()):raise RuntimeError('candidate-01 already contains evidence')
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'gemuendina-production-01.blend'))
...
meta['notes']+=['V3 living mass and regional pigmentation follow user art direction as explicit interpretations; exact accepted clay-02 volume is preserved.',
...
report={'phase':'local candidate; visual and post-rework audit pending','input_material_sha256':MATERIAL_SHA,
        'source_sha256':{n:sha(HERE/n)for n in ('candidate_01.py','rig_actions_01.py','sculpt_spec_02.py','render_candidate_01.py','check_candidate_01.py')},
        'metadata_input_sha256':METADATA_SHA,
        ...
        'body_bind_geometry_sha256':body_hash,'accepted_body_unchanged':True,'bones':len(B),'clips':CLIPS,
...
print('GEMUENDINA_CANDIDATE_GROUP_OK '+str(OUT/'candidate-report.json'))
