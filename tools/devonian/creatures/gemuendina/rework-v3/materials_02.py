...
if OUT.exists() and any(OUT.iterdir()):raise RuntimeError('material-02 already contains evidence')
...
base=field.mix(base,(.017,.030,.009,1),field.mul(field.mul(edge,mosaic_strength),.12))
base=field.mix(base,(.145,.148,.053,1),field.mul(field.mul(celltone.outputs[0],mosaic_strength),.22))
...
rough=field.add(.47,field.add(field.mul(.07,grain),field.mul(.045,field.mul(edge,mosaic_strength))))
rough=field.add(rough,field.mul(.055,field.sub(1,dorsal)))
relief=field.add(field.mul(.38,grain),field.mul(.56,field.mul(tubercles,mosaic_strength)))
relief=field.sub(relief,field.mul(.025,field.mul(edge,mosaic_strength)))
relief=field.sub(relief,field.mul(.055,patchline))
relief=field.mul(relief,field.sub(1,field.mul(.75,oralbed)))
field.finish(color,rough,relief,.0085)
...
eye.finish(eyecolor,.31,eye.noise(ep,76,1,.5),.00012)
...
for obj in [body]+eyes:
    ...
    obj['material_version']='Gemuendina original regional material-02'
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'gemuendina-procedural-material-02.blend'))
...
report={'phase':'material review pending','accepted_clay_sha256':SOURCE_SHA,
        'script_sha256':sha(HERE/'materials_02.py'),'geometry_sha256':initial_geometry,
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'gemuendina-material-02.blend'))
report['blend_sha256']=sha(OUT/'gemuendina-material-02.blend')
