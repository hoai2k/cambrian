...
ROOT=HERE.parents[4]
OUT=ROOT.parent/'devonian-authoring/gemuendina/rework-v3/clay-02'
sys.dont_write_bytecode=True
sys.path.insert(0,str(HERE))
from sculpt_spec_02 import make_mesh, eye_specs, VERSION, VIEWS
...
eye=material('Eye_clay_landmark',(.071,.075,.067),.40)
...
# Dorsal branchial sulci are now sculpted into the same cranial skin.
...
report={
    'version':VERSION,'phase':'clay review only','source_sha256':{p.name:sha(p) for p in [HERE/'sculpt_spec_02.py',HERE/'build_clay_02.py',HERE/'render_clay_02.py']},
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'gemuendina-clay-02.blend'))
report['blend_sha256']=sha(OUT/'gemuendina-clay-02.blend')
