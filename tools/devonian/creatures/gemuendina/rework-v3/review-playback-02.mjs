...
const candidate=path.resolve('../devonian-authoring/gemuendina/rework-v3/candidate-02');
...
const out=path.resolve('../devonian-authoring/gemuendina/rework-v3/playback-02');
...
try{
 ...
 const report=await page.evaluate(async()=>{
  ...
  const gltf=await new GLTFLoader().loadAsync('/assets/devonian/creatures/gemuendina.glb?review=rework-v3-candidate-02');document.body.replaceChildren();document.body.style.margin='0';
  ...
  const bindBox=new T.Box3().setFromObject(gltf.scene), bindSize=bindBox.getSize(new T.Vector3()), bindCenter=bindBox.getCenter(new T.Vector3());
  const visibleLength=Math.max(bindSize.x,bindSize.z,.01);
  const recolor=makeRecolor(gltf.scene);
  const cam=new T.PerspectiveCamera(35,4/3,.01,420);const mixer=new T.AnimationMixer(gltf.scene);let action, lowModel;
  ...
  window.reviewLod=async()=>{mixer.stopAllAction();scene.remove(gltf.scene);const low=await new GLTFLoader().loadAsync('/assets/devonian/creatures/gemuendina.lod1.glb?review=rework-v3-candidate-02');scene.add(low.scene);lowModel=low.scene;const lm=new T.AnimationMixer(low.scene);lm.clipAction(low.animations.find(a=>a.name==='Idle')).play();lm.update(0);cam.position.set(5,5.0,6);cam.lookAt(0,0,-.5);r.render(scene,cam);window.reviewLodDefault=()=>{makeRecolor(low.scene).setScheme(DEFAULT_SCHEME);r.render(scene,cam);};};
  window.reviewTransition=(level,ratio,fov)=>{
   if(!lowModel)throw Error('LOD must be loaded before transition review');
   gltf.scene.removeFromParent();lowModel.removeFromParent();scene.add(level==='full'?gltf.scene:lowModel);
   const distance=visibleLength/ratio;cam.fov=fov;cam.updateProjectionMatrix();
   cam.position.copy(bindCenter).add(new T.Vector3(5,5,6).normalize().multiplyScalar(distance));cam.lookAt(bindCenter);r.render(scene,cam);
   return {level,ratio,fov,distance,visibleLength,viewport:[1000,750],basis:'engine length/distance thresholds .05 .06 .075; FOV55/64 cover base range; direct GLB with default palette, no water haze'};
  };
 ...
 report.transitions=[];
 for(const fov of [55,64])for(const ratio of [.05,.06,.075])for(const level of ['full','lod']){
  report.transitions.push(await page.evaluate(({level,ratio,fov})=>window.reviewTransition(level,ratio,fov),{level,ratio,fov}));
  await saveCanvas('transition-'+level+'-ratio-'+ratio+'-fov-'+fov+'.png');
 }
