"""Read-only raw GLB material/skin/action validation for the authored Gemuendina exports."""
import json,struct,hashlib,math
from pathlib import Path
here=Path(__file__).resolve().parent;root=here.parents[3];out=root.parent/'devonian-authoring/gemuendina/candidate';reports=[]
for suffix in ['', '.lod1']:
 path=out/('gemuendina'+suffix+'.glb');raw=path.read_bytes();n=struct.unpack_from('<I',raw,12)[0];g=json.loads(raw[20:20+n]);blob=raw[28+n:]
 def data(ai):
...
(here/'export-review-v2.json').write_text(json.dumps({'id':'gemuendina','fullColorPolicy':'White COLOR_0 multiplied by UV albedo; no duplicate pigment darkening.','lodColorPolicy':'Texture-free, atlas-sampled linear vertex pigment.','exports':reports},indent=2));print('PASS',[(r['file'],r['bytes'],r['distinctMotions'])for r in reports])
