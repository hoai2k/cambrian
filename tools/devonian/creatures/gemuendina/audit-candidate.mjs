...
const root=path.resolve(import.meta.dirname,'../../../..');const local=path.resolve(root,'../devonian-authoring/gemuendina');const io=new NodeIO();
for(const level of ['full','lod']){
 const file=path.join(local,'candidate',`gemuendina${level==='lod'?'.lod1':''}.glb`);const bytes=await fs.readFile(file);const doc=await io.readBinary(bytes);const meshes=[];
 ...
 const out=path.join(local,'eye-audit-'+level);await fs.mkdir(out,{recursive:true});await fs.writeFile(path.join(out,'gemuendina.geometry.json'),JSON.stringify({id:'gemuendina',revision:'local-v2-'+level,asset:file,sha256:createHash('sha256').update(bytes).digest('hex'),coordinateSystem:'glTF +Y up +Z forward',meshes}));console.log(out);
}
