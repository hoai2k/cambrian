...
Usage: blender --background --threads 2 --python THIS -- candidate-01|candidate-05
"""
...
for suffix in ('','.lod1'):
 ...
 for clip,phase in samples:
  ...
  # Actual exported anchors define the prey path into the terminal lumen.
  # A free segment plus outside-material samples proves this bounded passage
  # stays open; closure alone would not detect a pinched or inverted throat.
  ...
  for fraction in (.08,.20,.40,.60,.80,.95):
   ...
   corridor.append({'fraction':fraction,'inside_tissue_votes':sum(inside(tree,point,d)for d in directions),
                    'clearance':tree.find_nearest(point)[3]})
