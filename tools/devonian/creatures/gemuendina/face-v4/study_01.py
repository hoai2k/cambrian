...
OUT = ROOT.parent/'devonian-authoring/gemuendina/face-v4/study-01'
...
def deform(p,region='dorsal'):
    ...
    return Vector((x,-1.70+math.cos(a)*dy-math.sin(a)*dz-.18*weight,
                   .15+math.sin(a)*dy+math.cos(a)*dz-.075*weight))
...
for side,sign in [('L',1),('R',-1)]:
    ...
    eyes.append({'side':side,'center':list(obj.location),'surface':list(p),'normal':list(normal),
                 'centerInset':.029,'finalContainmentAudit':'pending after shape approval'})
...
views=[('front',(0,-5,.30),(0,-1.55,.13),1.9),
       ('face-oblique',(2.1,-3.9,1.55),(0,-1.51,.18),2.5),
       ('side',(6,-1.25,.20),(0,-1.40,.10),2.35),
...
report={'source':str(SOURCE),'sourceSha256':EXPECTED,'scriptSha256':sha(__file__),
        'scope':'face sculpt study only; no production or final anatomy acceptance',
        'changedVertices':changed,'posteriorPreservedFromY':-1.04,'nonmanifoldEdges':nonmanifold,
        'volume':volume,'eyes':eyes,'renders':[]}
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'gemuendina-face-study-01.blend'))
report['blendSha256']=sha(OUT/'gemuendina-face-study-01.blend')
...
print('GEMUENDINA_FACE_STUDY_01_COMPLETE',flush=True)
