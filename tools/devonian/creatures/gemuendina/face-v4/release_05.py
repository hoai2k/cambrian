...
SOURCE = LOCAL / 'candidate-05'
DEST = ROOT / 'public/assets/devonian/creatures'
audit = json.loads((LOCAL / 'audit-candidate-05-03/audit.json').read_text())
...
report = {'status': 'preview', 'candidate': 'candidate-05', 'files': {},
          'metadata_source_sha256': sha(SOURCE / 'gemuendina.json'),
          'eye_audit_sha256': sha(LOCAL / 'audit-candidate-05-03/audit.json'),
          'remaining': 'Terminal-snout face preview; broader final art review, small LOD chin creases/pigment cleanup and controller playtest remain.'}
...
(LOCAL / 'release-05.json').write_text(json.dumps(report, indent=2) + '\n')
