...
MATERIAL=LOCAL/'study-05/gemuendina-face-study-05.blend'
MATERIAL_SHA='147a90e3233d6af75dcd06b3dd6ac92829bfffe45ac3f4020c6a87e0f7b9b40c'
METADATA=ROOT.parent/'devonian-authoring/backups/gemuendina-final-pre-face-refinement-2026-09-08/public/gemuendina.json'
METADATA_SHA='27ad05fc69089fed186820c5c28f9bdd6e7daf6d6fdf33fea0a64d2c15a49cb3'
OUT=LOCAL/'candidate-05'
sys.dont_write_bytecode=True;sys.path.insert(0,str(HERE.parent/'rework-v3'));sys.path.insert(0,str(HERE))
from rig_spec_05 import bones,build_weights,pose,CLIPS,LOOPS,ANCHORS
from sculpt_spec_02 import make_mesh,APERTURE_Y
from shape_05 import deform
...
if OUT.exists() and any(OUT.iterdir()):raise RuntimeError('candidate-05 already contains evidence')
...
if len(spec_regions)!=len(body.data.vertices):raise RuntimeError('Face study topology mismatch')
# Study05 is already the reviewed closed terminal envelope. Apply eye object transforms
# before binding; do not reapply the obsolete dorsal orbital correction.
for eye in eyes:
    bpy.ops.object.select_all(action='DESELECT');eye.select_set(True);bpy.context.view_layer.objects.active=eye
    bpy.ops.object.transform_apply(location=False,rotation=True,scale=True)
changed=[(v.index,(v.co-Vector(spec_vertices[v.index])).length)for v in body.data.vertices if (v.co-Vector(spec_vertices[v.index])).length>1e-7]
...
W=build_weights(spec_vertices,spec_regions,make_mesh()[4])
...
B=bones()
# Pivots and anchors come from the explicit terminal lumen spec, not a warp.
...
def patch_export(path,lod=False):
    ...
    # Keep all authored actions in both full and LOD face candidates.
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'gemuendina-face-production-05.blend'))
meta=json.loads(METADATA.read_text())
meta.update(modelLength=max(v.co.y for v in body.data.vertices)-min(v.co.y for v in body.data.vertices),clips=list(CLIPS),looping=list(LOOPS),anchors=[a['name']for a in ANCHORS],
    description='Broad rhenanid with a leading mouth, paired anterior eyes, small mosaic armour and a long finless tail; tail-driven swimming with restrained pectoral trim.')
meta['notes']+=['V4 study05 forms a true terminal snout: the front upper and lower lips are the leading body edge, with closely paired eyes immediately above them. The oral lining connects continuously into an internal throat; accepted V3 posterior silhouette and pigmentation remain.',
...
report={'phase':'local candidate; visual and post-rework audit pending','input_material_sha256':MATERIAL_SHA,
        'source_sha256':{str(p):sha(p)for p in [HERE/'candidate_05.py',HERE/'shape_05.py',HERE/'rig_spec_05.py',HERE.parent/'rework-v3/rig_actions_01.py',HERE.parent/'rework-v3/sculpt_spec_02.py',HERE.parent/'rework-v3/filter_lod_pigment_02.py']},
        ...
        'body_changes':'Reviewed terminal snout study05; explicit lower jaw and oral cavity, close anterior eyes; posterior preserved',
...
print('GEMUENDINA_FACE_CANDIDATE_05_GROUP_OK '+str(OUT/'candidate-report.json'))
