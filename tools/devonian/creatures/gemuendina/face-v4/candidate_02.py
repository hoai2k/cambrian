...
MATERIAL=LOCAL/'study-02/gemuendina-face-study-02.blend'
MATERIAL_SHA='5d5ce60cabd2956dbf3528b10b3bdd39fb033d8c47c3a8e6fec9024556ded106'
...
# Study02 is already the reviewed closed envelope. Apply eye object transforms
# before binding; do not reapply the obsolete dorsal orbital correction.
for eye in eyes:
    eye.location -= eye.rotation_quaternion @ Vector((0,0,.010))
...
W=[weights(spec_vertices[v.index],oral=v.index in oral_ids)for v in body.data.vertices]
...
B=bones()
for name in ('jaw','throat'):
    old=Vector(B[name]['head']);delta=deform(old,'oral')-old
    B[name]['head']=tuple(old+delta);B[name]['tail']=tuple(Vector(B[name]['tail'])+delta)
ANCHORS=[{**a,'point':list(deform(a['point'],'oral'))}for a in ANCHORS]
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'gemuendina-face-production-02.blend'))
...
meta['notes']+=['V4 user-directed face refinement moves the oral recess and paired eyes to the leading front. Accepted V3 posterior silhouette and regional pigmentation retained; living orientation is interpretive.',
...
report={'phase':'local candidate; visual and post-rework audit pending','input_material_sha256':MATERIAL_SHA,
        'source_sha256':{str(p):sha(p)for p in [HERE/'candidate_02.py',HERE/'shape_02.py',HERE.parent/'rework-v3/rig_actions_01.py',HERE.parent/'rework-v3/sculpt_spec_02.py',HERE.parent/'rework-v3/filter_lod_pigment_02.py']},
        ...
        'body_changes':'Reviewed face study02; anterior mouth rotation and eye relocation; posterior preserved',
...
print('GEMUENDINA_FACE_CANDIDATE_02_GROUP_OK '+str(OUT/'candidate-report.json'))
