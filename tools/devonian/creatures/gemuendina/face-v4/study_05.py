...
OUT = ROOT.parent/'devonian-authoring/gemuendina/face-v4/study-05'
...
from shape_05 import deform, apply_oral
...
for side,sign in [('L',1),('R',-1)]:
    ...
    eyes.append({'side':side,'center':list(obj.location),'surface':list(p),'normal':list(normal),
                 'centerInset':.022,'finalContainmentAudit':'pending after shape approval'})
...
views=[('side',(6,-1.65,.10),(0,-1.65,.10),1.9),
       ('front',(0,-5,.15),(0,-1.80,.07),1.9),
...
report={'source':str(SOURCE),'sourceSha256':EXPECTED,'scriptSha256':sha(__file__),
        'scope':'Terminal rim and independent shallow belly; explicit recessed oral rings; visual study only, no production acceptance',
        'changedVertices':changed,'shapeSha256':sha(HERE/'shape_05.py'),'posteriorPreservedFromY':-1.04,'nonmanifoldEdges':nonmanifold,
        'volume':volume,'eyes':eyes,'renders':[]}
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'gemuendina-face-study-05.blend'))
report['blendSha256']=sha(OUT/'gemuendina-face-study-05.blend')
...
print('GEMUENDINA_FACE_STUDY_05_COMPLETE',flush=True)
