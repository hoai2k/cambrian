"""Read-only structural export checks for a candidate_06.py build.

`GEMUENDINA_OUT` names the output directory to check (required).

Adapted from check_candidate_05.py. Two relaxations from that original, both purely about
material fidelity (see candidate_06.py's module docstring — the imagegen swatch this
creature's real materials are baked from is not available in this environment, so this
build uses flat placeholder materials): the FULL export is not required to carry a
baseColorTexture for the body/eyes slots, and the LOD body is not required to show pigment
*variation* (a flat material bakes to a uniform vertex colour). Everything else — finite
accessors, normalized weights, clip durations/seams, anchor graph, skeleton/bind-matrix
parity between full and LOD, true LOD triangle reduction, no scale/root channels — is
unchanged.
"""
import json, struct, hashlib, math, sys, os, re
from pathlib import Path
HERE = Path(__file__).resolve().parent
OUT = Path(os.environ['GEMUENDINA_OUT'])
sys.dont_write_bytecode = True
sys.path.insert(0, str(HERE.parent / 'rework-v3')); sys.path.insert(0, str(HERE))
from rig_spec_05 import CLIPS, LOOPS, bones
from rig_spec_05 import ANCHORS as ANCHORS_05

def sha(p): return hashlib.sha256(Path(p).read_bytes()).hexdigest()
report = json.loads((OUT / 'candidate-report.json').read_text())
for name, digest in report['source_sha256'].items():
    assert sha(Path(name)) == digest, 'Changed candidate source ' + name
...
for suffix in ('', '.lod1'):
    ...
    for mesh in g['meshes']:
        for p in mesh['primitives']:
            ...
            if not suffix:
                assert max(abs(v - 1) for v in rgb) < 1e-6  # full export: white COLOR_0, colour comes from the material
            else:
                assert not any(k.endswith('Texture') for k in pbr)
                assert 'normalTexture' not in mat
                # relaxed vs candidate_05: placeholder materials are flat, so LOD pigment has
                # no variation to preserve (see module docstring)
