...
const out=path.resolve('../devonian-authoring/gemuendina/viewer-v2');fs.mkdirSync(out,{recursive:true});for(const name of fs.readdirSync(out))if(name.endsWith('.png'))fs.unlinkSync(path.join(out,name));
...
try{
 await page.route('**/assets/devonian/creatures/gemuendina*.glb*',async route=>{const file=new URL(route.request().url()).pathname.split('/').pop();await route.fulfill({body:fs.readFileSync(path.resolve('../devonian-authoring/gemuendina/candidate',file)),contentType:'model/gltf-binary'});});
 ...
 const report=await page.evaluate(async()=>{
  ...
  const gltf=await new GLTFLoader().loadAsync('/assets/devonian/creatures/gemuendina.glb?review=v2');document.body.replaceChildren();document.body.style.margin='0';
 ...
 for(const name of ['Swim','Dodge','Ability','Bite','Death','TurnLeft','Heavy','Hit','Stagger','Attack','Parry','Growth'])for(const phase of [.2,.75]){await page.evaluate(({name,phase})=>window.reviewPose(name,phase,name==='Bite'||name==='Ability'?'mouth':'threequarter'),{name,phase});await saveCanvas(name+'-phase-'+phase+'.png');}
