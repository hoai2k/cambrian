"""V3 port of finalize.py: points at the v3-candidate/ GLBs and checks anchor sockets against
anchors-v3.json (the moved mouth/attack anchors) instead of the shipped anchors.json. Otherwise
identical to finalize.py. Remove exporter-created identity scale tracks; audit decoded
geometry/rig/clips."""
import json,struct,math,hashlib
from pathlib import Path
import numpy as np
H=Path(__file__).resolve().parent;R=H.parents[3];O=R.parent/'devonian-authoring/rhinodipterus/v3-candidate'
...
results=[]
for suffix in ['','.lod1']:
 p=O/('rhinodipterus'+suffix+'.glb');d,bin=read(p);removed=0
 ...
 manifest=json.loads((H/'anchors-v3.json').read_text())['rhinodipterus']
 for s in manifest:
...
meta=json.loads((O/'rhinodipterus.json').read_text());meta['modelLength']=results[0]['bounds'][2];meta['clips']=[x['name'] for x in results[0]['clips']];(O/'rhinodipterus.json').write_text(json.dumps(meta,indent=2)+'\n')
(H/'validation-v3.json').write_text(json.dumps({'checks':'finite transforms, normalized weights, root stable, identity-scale removal, unique motion, seamless loops, socket bind alignment, true LOD reduction','models':results,'lodTriangleRatio':results[1]['triangles']/results[0]['triangles']},indent=2)+'\n')
