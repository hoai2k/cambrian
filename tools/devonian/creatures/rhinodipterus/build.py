"""Rhinodipterus V2: independently shaped Gogo lungfish; candidate-only Blender authoring."""
...
H=Path(__file__).resolve().parent;R=H.parents[3];L=R.parent/'devonian-authoring/rhinodipterus';C=L/'v2-candidate';C.mkdir(parents=True,exist_ok=True)
...
HEAD=[(-2.32,0,-.060,-.060),(-2.285,.075,.052,-.047),(-2.20,.143,.106,-.062),(-1.99,.174,.163,-.070),(-1.73,.202,.221,-.083),(-1.46,.274,.306,-.099),(-1.17,.352,.365,-.123),(-.91,.405,.365,-.115),(-.68,.418,.350,.006)]
def hp(y,a):
 ...
 if sn>0:z+=.010*sn**7*sin(pi*np.clip((y+2.32)/1.64,0,1))
...
JAW=[(-2.30,0,-.075,-.075),(-2.265,.070,-.062,-.104),(-2.18,.135,-.081,-.132),(-1.93,.17,-.112,-.177),(-1.65,.225,-.139,-.191),(-1.36,.298,-.16,-.215),(-1.13,.345,-.163,-.215),(-.94,.371,-.099,-.155),(-.79,.365,-.005,-.061),(-.70,0,-.015,-.015)]
def jp(y,a):
...
# Fin/body proportions follow the explicitly comparative outline in Clement2012 Fig7G.
...
# Shared palate/mandible boundaries form the posterior oral lining and outer cheeks.
v=[];f=[];ww=[];UV=[];NL=64;AL=80
for j in range(NL):
 t=j/(NL-1);y=-1.18+.41*t;blend=np.clip((t-.52)/.48,0,1);blend=blend*blend*(3-2*blend)
 for k in range(AL+1):
...
for sign in [1,-1]:
 ...
 for j in range(NL):
  t=j/(NL-1);y=-1.62+.85*t;upper=hp(y,0);lower=jp(y,0);back=np.clip((t-.83)/.17,0,1);back=back*back*(3-2*back)
  ...
  for k in range(NK+1):
   ...
   x+=.002*sin(pi*q)*sin(pi*t);vv.append((sign*x,yy,z));uv.append(((yy+2.32)/1.64,1-.020*(1-q) if sign>0 else .5+.020*(1-q)));weights.append({'jaw':(1-back)*(1-q),'skull':(1-back)*q,'body':back})
 for j in range(NL-1):
...
# One continuous tapered fleshy fin surface, never a cylindrical lobe on a separate paddle.
...
# Median fins preserve local body-chain attachment; distal lobes have independent timing.
...
eyes=[]
for sign in [1,-1]:
 cy=-1.43;a=.27;surface=hp(cy,a);normal=Vector((1,0,.30)).normalized();center=surface-normal*.026;center.x*=sign;basis=[Vector((0,1,0)),Vector((-.287*sign,0,.958)),Vector((.958*sign,0,.287))];radii=(.070,.050,.049);vv=[];ff=[]
 for j in range(33):
  lat=pi*j/32
...
if '--clay' not in sys.argv:exec(compile((H/'anatomy-v2.py').read_text(),str(H/'anatomy-v2.py'),'exec'))
...
s.frame_set(0);bpy.ops.wm.save_as_mainfile(filepath=str(L/'rhinodipterus-v2-clay.blend'))
if '--clay' not in sys.argv:
 exec(compile((H/'materials-v2.py').read_text(),str(H/'materials-v2.py'),'exec'))
 exec(compile((H/'motion-v2.py').read_text(),str(H/'motion-v2.py'),'exec'))
 exec(compile((H/'export-v2.py').read_text(),str(H/'export-v2.py'),'exec'))
