"""Actual GLB fin-root immersion and finite skin checks through every exported action."""
...
for suffix in ['', '.lod1']:
 p=L/'v2-candidate'/('rhinodipterus'+suffix+'.glb');sha=hashlib.sha256(p.read_bytes()).hexdigest()
...
(H/'attachment-validation-v2.json').write_text(json.dumps(results,indent=2));print(json.dumps([{'file':r['file'],'maxRootOutwardDistance':r['maxRootOutwardDistance'],'clips':len(r['clips'])}for r in results]))
