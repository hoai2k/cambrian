...
if '--profile-only' in sys.argv:
 ...
 render('Idle',1,L/'full-profile-v2.png',(6,.56,.24),(0,.56,0),6.3)
 render('Heavy',12,L/'gape-profile-v2.png',(6,.56,.24),(0,.56,0),6.3)
 ...
 render('Heavy',12,L/'gape-opposite-v2.png',(-1.1,-3,-.35),(0,-1.57,-.11),1.65)
 render('Bite',10,L/'closing-opposite-v2.png',(-1.2,-3,-.45),(0,-1.64,-.12),1.65)
 render('Heavy',12,L/'mouth-gape-front-v2.png',(0,-4,-.24),(0,-1.80,-.08),1.35)
 (H/'profile-validation-v2.json').write_text(json.dumps({'sourceGlbSha256':loaded_sha,'renders':render_records},indent=2))
 sys.exit(0)
if '--gape-only'in sys.argv:
 ...
 render('Heavy',12,L/'mouth-gape-v2.png',(.17,-4,-.55),(0,-1.80,-.08),1.35)
 sys.exit(0)
if '--oblique-only'in sys.argv:
 s.render.resolution_x=1000;s.render.resolution_y=750
 render('Idle',1,L/'portrait-preview-v2.png')
 bpy.ops.object.light_add(type='AREA',location=(.1,-3,-.25));fill=bpy.context.object;fill.data.energy=45;fill.data.size=1.2;fill.rotation_euler=(Vector((0,-1.2,-.1))-fill.location).to_track_quat('-Z','Y').to_euler()
 render('Bite',10,L/'mouth-closing-oblique-v2.png',(1.2,-3,-.45),(0,-1.64,-.12),1.65)
 render('Heavy',12,L/'mouth-gape-oblique-v2.png',(1.1,-3,-.35),(0,-1.57,-.11),1.65)
...
for label,action,frame in [('gape','Heavy',12),('closing','Bite',10),('rest','Idle',1),('eat','Eat',29)]:render(action,frame,L/('mouth-'+label+'-v2.png'),(.17,-4,-.55),(0,-1.80,-.08),1.35)
