"""V3 port of motion-v2.py: the opercular weight-paint band (0 at y=-1.01, full at y=-.73, in the
old head's coordinates) is carried through the same stretchY() the head loft uses, so the gill
patch stays seated at the same anatomical spot on the now-longer head. Otherwise identical to
motion-v2.py. Restrained dipnoan feeding and buoyant, lobed-fin motion; no gameplay claims."""
GILL_LO=stretchY(-1.01);GILL_HI=stretchY(-.73)
for sign,side in [(1,'L'),(-1,'R')]:
 g=head.vertex_groups.new(name='gill'+side)
 for i,v in enumerate(head.data.vertices):
  if sign*v.co.x<=0:continue
  w=.40*np.clip((v.co.y-GILL_LO)/(GILL_HI-GILL_LO),0,1)*np.clip(abs(v.co.x)/.38,0,1)**4
