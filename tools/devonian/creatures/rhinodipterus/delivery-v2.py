...
H=Path(__file__).resolve().parent;L=H.parents[3].parent/'devonian-authoring/rhinodipterus';C=L/'v2-candidate'
...
for folder,name,digest in [('v2-eye-audit','eye-audit-full-v2.json',full),('v2-eye-audit-lod','eye-audit-lod-v2.json',lod)]:
...
for file,digest in [('render-validation-v2.json',full),('render-validation-lod-v2.json',lod),('profile-validation-v2.json',full)]:
...
play=json.loads((L/'playback-validation-v2.json').read_text());assert play['sourceGlbSha256']==full and len(play['audit'])==18 and not play['errors'];assert all(r['finite']and r['rootStable']for r in play['audit']);assert sha(L/'motion-review-v2.webm')==play['videoSha256'];shutil.copy2(L/'playback-validation-v2.json',H/'playback-validation-v2.json')
attach=json.loads((H/'attachment-validation-v2.json').read_text());assert [r['sourceGlbSha256']for r in attach]==[full,lod];assert all(r['maxRootOutwardDistance']<0 for r in attach)
...
for i,name in enumerate(names):
 p=L/f'webgl-{name}-v2.png';im=ImageOps.contain(Image.open(p).convert('RGB'),(395,278));x=(i%4)*400;y=(i//4)*290;out.paste(im,(x,y));draw.text((x+8,y+265),name,fill='white')
out.save(L/'webgl-action-contact-v2.jpg',quality=94);shutil.copy2(L/'webgl-action-contact-v2.jpg',H/'action-review-v2.jpg')
files={p.name:{'sha256':sha(p),'bytes':p.stat().st_size}for p in sorted(C.glob('rhinodipterus*'))};source=L/'rhinodipterus-v2.blend'
report={'candidateOnly':True,'species':'Rhinodipterus kimberleyensis','files':files,'originalBlend':{'path':str(source),'sha256':sha(source),'bytes':source.stat().st_size},'validation':validation,'reviewReports':{p.name:sha(p)for p in H.glob('*validation*.json')},'eyeReports':{p.name:sha(p)for p in H.glob('eye-audit-*-v2.json')},'sourceScripts':{p.name:sha(p)for p in sorted(H.glob('*.py'))},'notes':['Parent performs independent viewer review and public promotion/commit.','Source anatomy and pigment frozen before matching final renders, full/LOD eye audits and all-action playback.','Full source and every original/intermediate remain in the authoring directory; V1 was preserved before editing.']}
(H/'delivery-v2.json').write_text(json.dumps(report,indent=2));print(json.dumps({'full':full,'lod':lod,'blend':report['originalBlend']['sha256'],'files':len(files)},indent=2))
