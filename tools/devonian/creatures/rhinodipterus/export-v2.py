...
anchors=[('anchor_mouth','jaw',(0,-2.255,-.05),'mouth'),('anchor_mouth_inside','skull',(0,-1.16,-.11),'swallow'),('anchor_attack_primary','skull',(0,-2.29,.032),'attack')]
...
bpy.context.view_layer.update();(H/'anchors.json').write_text(json.dumps({'rhinodipterus':[{'name':n,'bone':b,'point':p,'role':r}for n,b,p,r in anchors]},indent=2))
sourcepath=L/'rhinodipterus-v2.blend';bpy.ops.wm.save_as_mainfile(filepath=str(sourcepath))
def export(path):
...
(C/'rhinodipterus.json').write_text(json.dumps(meta,indent=2));(L/'build-stats-v2.json').write_text(json.dumps({'fullTriangles':full,'lodTriangles':lod,'ratio':lod/full,'clips':clips,'source':str(sourcepath)},indent=2))
print('RHINODIPTERUS_V2_CANDIDATE',full,lod,str(C),flush=True)
