...
const root=path.resolve(import.meta.dirname,'../../../..'),local=path.resolve(root,'../devonian-authoring/rhinodipterus'),out=path.join(local,lod?'v2-eye-audit-lod':'v2-eye-audit');await fs.mkdir(out,{recursive:true});
const file=path.join(local,'v2-candidate/rhinodipterus'+(lod?'.lod1':'')+'.glb'),bytes=await fs.readFile(file),doc=await new NodeIO().readBinary(bytes),meshes=[];
...
await fs.writeFile(path.join(out,'rhinodipterus.geometry.json'),JSON.stringify({id:'rhinodipterus',revision:'V2 candidate',sha256:createHash('sha256').update(bytes).digest('hex'),meshes}));
