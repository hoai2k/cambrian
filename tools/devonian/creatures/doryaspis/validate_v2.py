...
import numpy as np
H=Path(__file__).resolve().parent;R=H.parents[3];O=(R/'public/assets/devonian/creatures')if os.environ.get('DORY_PUBLISH')=='1'else R.parent/'devonian-authoring/doryaspis/v2/candidate'
...
results=[]
for suffix in ['','.lod1']:
 p=O/('doryaspis'+suffix+'.glb');d,bin=read(p);removed=0
 ...
 manifest=json.loads((H/'anchors.json').read_text())['doryaspis']
 for s in manifest:
 ...
 for a in d['animations']:
  ...
  for c in a['channels']:
   ...
   if target.startswith('oral_') and c['target']['path']=='translation' and a['name'] in ['Attack','Bite','Heavy','Eat','Ability']:
    amount=float(np.linalg.norm(v-v[0],axis=1).max());jawproof.append({'clip':a['name'],'bone':target,'maxTranslation':amount});assert amount>1e-4
 ...
 results.append({'file':p.name,'bytes':p.stat().st_size,'vertices':verts,'triangles':tri,'bounds':span.tolist(),'bones':len(d['skins'][0]['joints']),'sockets':sockets,'clips':motions,'oralSoftTissueArticulation':jawproof,'removedIdentityScaleChannels':removed,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()})
assert results[1]['triangles']/results[0]['triangles']<.4
meta=json.loads((O/'doryaspis.json').read_text());meta['modelLength']=results[0]['bounds'][2];meta['clips']=[x['name'] for x in results[0]['clips']];(O/'doryaspis.json').write_text(json.dumps(meta,indent=2)+'\n')
