"""Linux copy of build_clay01.py (Blender 5.2.1 at /opt/blender/blender).

Deltas from the frozen macOS script, and nothing else:
  * render/build thread count 2 -> 1, because this environment's determinism
    convention is --threads 1.
  * the recorded inputHashes name this copy and render_clay01-linux.py.
  * `.blend` files are not byte-reproducible across saves, so the frozen
    blend-hash gate is satisfied *by value*: construction.json records the
    geometry invariants (vertex/face counts, components, manifoldness, volume,
    eye placeholder seats) and the render stage re-checks the blend hash it
    itself just recorded, within the one run.
The geometry module `geometry_clay01.py` is imported unchanged.
"""
import bpy
import bmesh
import json
...
report={'stage':'unreviewed clay01','coordinates':'world Y-up, +Z-forward',
        ...
        'inputHashes':{str(HERE/n):sha(HERE/n) for n in ('geometry_clay01.py','build_clay01-linux.py','render_clay01-linux.py')},
