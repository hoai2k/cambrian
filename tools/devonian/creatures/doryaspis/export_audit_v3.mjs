 ...
 * V3 counterpart of export_audit.mjs (added, never edited): reads v3-candidate,
...
const H=import.meta.dirname, R=path.resolve(H,'../../../..'), L=path.resolve(R,'../devonian-authoring/doryaspis');
await Promise.all([MeshoptDecoder.ready, MeshoptEncoder.ready]);
const io=new NodeIO().registerExtensions(ALL_EXTENSIONS).registerDependencies({'meshopt.decoder':MeshoptDecoder,'meshopt.encoder':MeshoptEncoder});
const selectors=JSON.parse(await fs.readFile(path.join(L,'v3-candidate','audit-selectors.json'),'utf8'));
for(const suffix of ['', '.lod1']){
 const out=path.join(L, suffix?'v3-audit-lod':'v3-audit-full');
 await fs.mkdir(out,{recursive:true});
 const asset=path.join(L,'v3-candidate',`doryaspis${suffix}.glb`);
 ...
 await fs.writeFile(path.join(out,'doryaspis.geometry.json'),JSON.stringify({id:'doryaspis',
