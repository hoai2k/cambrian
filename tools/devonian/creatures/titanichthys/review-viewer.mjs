/** Actual exported GLB review through Three.js; writes only Titanichthys local review outputs. */
import {chromium} from 'playwright-core';
import fs from 'node:fs';
import path from 'node:path';
const out=path.resolve('../devonian-authoring/titanichthys/viewer-v2');fs.mkdirSync(out,{recursive:true});for(const name of fs.readdirSync(out))if(name.endsWith('.png'))fs.unlinkSync(path.join(out,name));
...
try{
 ...
 const report=await page.evaluate(async()=>{
  ...
  const {auditDevonian}=await import('/tools/devonian/runtime-audit.ts');const audit=await auditDevonian(['titanichthys']);
  const gltf=await new GLTFLoader().loadAsync('/assets/devonian/creatures/titanichthys.glb?review=v2');document.body.replaceChildren();document.body.style.margin='0';
  ...
  window.reviewPose=(clip,phase,view)=>{mixer.stopAllAction();action=mixer.clipAction(gltf.animations.find(a=>a.name===clip));action.reset().play();mixer.update(0);action.time=action.getClip().duration*phase;mixer.update(0);gltf.scene.updateMatrixWorld(true);
   const views={threequarter:[[7,4.4,7.2],[0,0,-.3]],side:[[10,1,0],[0,0,-.3]],front:[[0,1.4,10],[0,0,1]],dorsal:[[0,12,-.1],[0,0,-.1]],eye:[[3.6,1.7,4.3],[1.25,.17,2.03]],mouth:[[1.6,1.1,6.7],[0,-.15,2.1]]};const[p,t]=views[view];cam.position.set(...p);cam.lookAt(...t);r.render(scene,cam);return {clip,phase,view};};
  window.reviewLod=async()=>{mixer.stopAllAction();scene.remove(gltf.scene);const low=await new GLTFLoader().loadAsync('/assets/devonian/creatures/titanichthys.lod1.glb?review=v2');scene.add(low.scene);const lm=new T.AnimationMixer(low.scene);lm.clipAction(low.animations.find(a=>a.name==='Idle')).play();lm.update(0);cam.position.set(7,4.4,7.2);cam.lookAt(0,0,-.3);r.render(scene,cam);};
  return {audit,clips:gltf.animations.map(a=>({name:a.name,duration:a.duration}))};
 });
