"""Read-only raw GLB material/skin/action validation for the authored Titanichthys exports."""
import json,struct,hashlib,math
from pathlib import Path
here=Path(__file__).resolve().parent;root=here.parents[3];out=root/'public/assets/devonian/creatures';reports=[]
for suffix in ['', '.lod1']:
 path=out/('titanichthys'+suffix+'.glb');raw=path.read_bytes();n=struct.unpack_from('<I',raw,12)[0];g=json.loads(raw[20:20+n]);blob=raw[28+n:]
 def data(ai):
 ...
 for mesh in g['meshes']:
  for primitive in mesh['primitives']:
   ...
   else:assert max(rgb)-min(rgb)>.0001 or 'eyes'in mat['name'];assert not any(k.endswith('Texture')for k in pbr)
 ...
 reports.append({'file':str(path.relative_to(root)),'sha256':hashlib.sha256(raw).hexdigest(),'bytes':len(raw),'clips':names,'distinctMotions':len(signatures),'materials':primitiveReports,'rootStable':True,'scaleChannels':False})
assert len(reports[0]['clips'])==18;assert set(reports[1]['clips'])=={'Idle','Swim','Death'}
(here/'export-review-v2.json').write_text(json.dumps({'id':'titanichthys','fullColorPolicy':'White COLOR_0 multiplied by UV albedo; no duplicate pigment darkening.','lodColorPolicy':'Texture-free, atlas-sampled linear vertex pigment.','exports':reports},indent=2));print('PASS',[(r['file'],r['bytes'],r['distinctMotions'])for r in reports])
