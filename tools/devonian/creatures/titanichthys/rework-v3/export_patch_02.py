...
def patch_export(path,lod=False):
    ...
    # Full and LOD retain the complete authored gameplay action contract.
    names=[a['name']for a in g['animations']]
    if len(names)!=len(CLIPS)or set(names)!=set(CLIPS):raise RuntimeError('Export action set mismatch')
