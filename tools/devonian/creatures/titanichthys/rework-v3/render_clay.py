"""Frozen multiview recipe for the unreviewed Titanichthys clay-01.
...
SOURCE = OUT/'titanichthys-clay-01.blend'
...
if hashlib.sha256((HERE/'build_clay.py').read_bytes()).hexdigest() != report['builder_sha256']:
...
VIEWS = [
    ...
    ('02-front', (0, -12, .12), (0, -.5, .10), 8.1, 0., 80),
    ('03-dorsal', (0, .60, 13), (0, .60, 0), 9.8, 0., 0),
    ('04-three-quarter', (8.4, -9.4, 4.7), (0, .10, .08), 9.4, 0., 35),
    ('05-ventral-oblique', (7.6, -7.8, -4.4), (0, .05, -.20), 9.4, 0., 75),
    ('06-head-sculpt', (6.6, -7.2, 2.7), (0, -1.82, .10), 4.6, 0., 45),
    ('07-oral-front', (0, -8, -.10), (0, -1.80, -.12), 3.65, 1., 170),
    ('08-gape-side', (9, -1.8, .02), (0, -1.78, -.12), 4.7, 1., 100),
]
scene = bpy.context.scene
camera = scene.camera
body = bpy.data.objects['Titanichthys_new_continuous_sculpt']
gape = body.data.shape_keys.key_blocks['Gape study 24 degrees']
...
for name, location, target, scale, value, power in VIEWS:
    ...
    for ob, original in followers:
        rotation = Matrix.Translation(neck) @ Matrix.Rotation(math.radians(-3)*value*ob['gape_skull_weight'], 4, 'X') @ Matrix.Translation(-neck)
