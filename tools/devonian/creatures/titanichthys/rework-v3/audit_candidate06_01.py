...
LOCAL=ROOT.parent/'devonian-authoring/titanichthys/rework-v3';SRC=LOCAL/'candidate-06';OUT=LOCAL/'audit-candidate06-01'
...
report={'source_sha256':sha(__file__),'inputs':HASHES,'method':'Actual exported eye-polyhedron rejection volume; actual deformed full/LOD body parity in three directions; conservative Wilson bounds; distant closure certificate only',
...
for name,digest in HASHES.items():
    ...
    for clip,phase in [(None,0.),('Ability',.5)]:
        ...
        report['records'].append(record);save();assert topology['valid'],'Invalid continuous head topology'
        ...
        for side in ('L','R'):
            ...
            eye_tree=BVHTree.FromPolygons(ep,ef,all_triangles=True);a=np.array(ep);lo=a.min(0);hi=a.max(0)
            rng=np.random.default_rng(719061+(side=='R'));nbox=120000 if clip is None else 24000
            accepted=[p for p in rng.uniform(lo,hi,(nbox,3))if core['inside'](eye_tree,p,core['DIRECTIONS'][0])]
            votes=np.array([[core['inside'](tree,p,d)for d in core['DIRECTIONS']]for p in accepted],dtype=np.uint8).sum(1);n=len(votes);assert n>1000
            lower=core['wilson'](int((votes==3).sum()),n)[0];upper=core['wilson'](int((votes>0).sum()),n)[1]
            ...
            er={'side':side,'actual_globe_volume_samples':n,'inside_percent':100*float((votes>=2).mean()),'conservative95_percent':[lower,upper],
                'ray_disagreement_count':int(((votes>0)&(votes<3)).sum()),'closure_clearance_lower_bounds':clearance,'minimum50_pass':passed,'target65':lower>=65 and verified}
