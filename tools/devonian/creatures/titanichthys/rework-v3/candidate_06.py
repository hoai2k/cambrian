...
LOCAL=REPO.parent/'devonian-authoring/titanichthys/rework-v3';OUT=LOCAL/'candidate-06'
...
from atlas_pigment_04 import sample_object,assert_pigment,stats,transfer_export_colors
...
if OUT.exists():raise RuntimeError('Preserve existing candidate-06')
    ...
    for mat in ob.data.materials:
        for node in mat.node_tree.nodes:
            ...
            if name not in export_images:
                image=source.copy();image.name='Candidate06 '+source.name
...
kwargs=dict(export_format='GLB',use_selection=True,export_animations=True,export_animation_mode='ACTIONS',
    ...
    export_vertex_color_name='Color',export_all_vertex_colors=False,export_yup=True,export_extras=True,export_morph=False)
...
# Retain the exact dense, region/normal-compatible filtered body field. Its
# values never enter the decimator; the final body samples its source surface.
...
color_transfer=transfer_export_colors(OUT/'titanichthys.lod1.glb',parts)
(OUT/'lod-pigment-transfer.json').write_text(json.dumps({'stages':pigment_stages,'export':color_transfer},indent=2)+'\n')
...
blend=OUT/'titanichthys-production-06.blend';bpy.ops.wm.save_as_mainfile(filepath=str(blend))
...
source_names=('candidate_06.py','rig_actions_01.py','export_patch_02.py','metadata_seed_01.json','render_candidate_06.py','check_candidate_06.py','atlas_pigment_04.py','atlas_samples_02.json','lod_pigment_03.py')
report={'phase':'animated candidate; actual playback and final eye/general audits pending',
    ...
    'pigment_transfer':color_transfer,'textures':texture_records,'palette_slots':['body','underside','accent','fins','eyes'],
    'lod_pigment':'Neutral Color during decimation; body convex source-surface interpolation within material role preserves dense region/normal filter (.055 body/.028 oral); fins and eyes sample accepted atlas at final UV; no colour clamp',
