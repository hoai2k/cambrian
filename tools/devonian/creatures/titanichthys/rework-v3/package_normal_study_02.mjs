/** Local normal-resolution candidate06 comparison study. No LOD stripping, quantization or public writes. */
...
const SRC=path.join(LOCAL,'candidate-06'),OUT=path.join(LOCAL,'packaging-normal-study-02');
const HASHES={'titanichthys.glb':'e2c69eab63e50805c7d3980ee5e65e8b328b8d88e190a944e3f26db2b8ef36ad'};
...
await mkdir(OUT);
const report={scope:'Local packaging/normal-map comparison only. 18 PNG maps recompressed with exact decoded RGB except body normal2048 vector-filtered derivative. Art review required; no public writes.',
...
for(const [name,digest]of Object.entries(HASHES)) {
 ...
 const untouched=snapshot(doc);
 const pngReport=JSON.parse(await readFile(path.join(LOCAL,'png-lossless-study-01/report.json'),'utf8'));
 const normalReport=JSON.parse(await readFile(path.join(LOCAL,'body-normal-study-02/report.json'),'utf8'));
 report.imageChanges=[];
 for(const texture of doc.getRoot().listTextures()) {
   const original=hash(texture.getImage());const row=pngReport.images.find(x=>x.originalSha256===original);
   assert(row,'Unexpected image');
   const reduced=row.name==='body-normal';
   const filename=reduced?path.join(LOCAL,'body-normal-study-02/body-normal-2048.png'):path.join(LOCAL,'png-lossless-study-01',row.file);
   const bytes=await readFile(filename);assert.equal(hash(bytes),reduced?normalReport.outputSha256:row.outputSha256);
   texture.setImage(bytes);
   report.imageChanges.push({name:row.name,original,output:hash(bytes),decodedPixelsExact:!reduced});
 }
 function withoutImageHashes(value) {
   if(Array.isArray(value))return value.map(withoutImageHashes);
   if(value&&typeof value==='object')return Object.fromEntries(Object.entries(value).map(([k,v])=>[k,k==='image'&&'mime'in value?'IMAGE':withoutImageHashes(v)]));
   return value;
 }
 assertSnapshot(withoutImageHashes(snapshot(doc)),withoutImageHashes(untouched),'Texture replacement changed scene data');
 ...
 report.outputs.push({file:target,bytes:bytes.length,sha256:hash(bytes),beforeBytes:input.length,before,after,
  under25MiB:bytes.length<25*1024*1024,validation:{decodedNumbersExact:true,orientedTrianglesExact:true,nodeTransformsExact:true,
   materialImagesMatchReviewedStudy:true,allOtherSceneNumbersUnchanged:true,skinAndAnchorsExact:true,all18AnimationsExact:true}});
 await writeFile(path.join(OUT,'package-report.json'),JSON.stringify(report,null,2)+'\n');
 console.log('TITANICHTHYS_NORMAL_STUDY_PACKAGE_FILE',name,input.length,'->',bytes.length,'18 clips exact');
}
...
console.log('TITANICHTHYS_NORMAL_STUDY_PACKAGE_OK',path.join(OUT,'package-report.json'));
