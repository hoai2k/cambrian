...
LOCAL=REPO.parent/'devonian-authoring/titanichthys/rework-v3';OUT=LOCAL/'candidate-02'
...
if OUT.exists():raise RuntimeError('Preserve existing candidate-02')
    ...
    for mat in ob.data.materials:
        for node in mat.node_tree.nodes:
            ...
            if name not in export_images:
                image=source.copy();image.name='Candidate02 '+source.name
...
for ob,source in zip(parts,meshes):
    ...
    before=np.array([v.color[:]for v in ob.data.color_attributes['Color'].data]);assert_pigment(before,source.name+' pre-decimate')
    pigment_stages[source.name]['pre_decimate']=stats(before)
    ...
    after=np.array([v.color[:]for v in ob.data.color_attributes['Color'].data]);assert_pigment(after,source.name+' post-decimate')
    pigment_stages[source.name]['post_decimate']=stats(after)
    materials=[]
...
blend=OUT/'titanichthys-production-02.blend';bpy.ops.wm.save_as_mainfile(filepath=str(blend))
...
source_names=('candidate_02.py','rig_actions_01.py','export_patch_01.py','metadata_seed_01.json','render_candidate_02.py','check_candidate_02.py','atlas_pigment_02.py','atlas_samples_02.json')
