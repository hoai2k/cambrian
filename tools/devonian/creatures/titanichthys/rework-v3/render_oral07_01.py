...
SRC=ROOT.parent/'devonian-authoring/titanichthys/rework-v3/candidate-07';OUT=SRC/'oral-renders-01'
...
sweep=json.loads((SRC/'oral-sweep-01/result.json').read_text());assert sweep['passed'] and sweep['sampled_pose_count']==58
...
manifest={'script_sha256':sha(__file__),'candidate_report_sha256':sha(SRC/'candidate-report.json'),
 'sweep_sha256':sha(SRC/'oral-sweep-01/result.json'),'renders':[],'scope':'Actual imported GLB materials and animation slots; oral correction review only; eyes unchanged for separate root study'}
def imported(name):
