...
No blend save, public files or final audit; all images belong to candidate05.
"""
import bpy,sys,json,hashlib
from pathlib import Path
from mathutils import Vector
HERE=Path(__file__).resolve().parent;ROOT=HERE.parents[4]
OUT=ROOT.parent/'devonian-authoring/titanichthys/rework-v3/candidate-05'
...
blend=OUT/'titanichthys-production-05.blend'
...
lod_rigs=[obj for obj in bpy.data.objects if obj.type=='ARMATURE']
if len(lod_rigs)!=1:raise RuntimeError('Expected exactly one imported LOD skeleton')
lod_rig=lod_rigs[0]
if not lod_rig.animation_data:raise RuntimeError('Imported LOD has no animation data')
tracks=lod_rig.animation_data.nla_tracks
if set(t.name for t in tracks)!=set(CLIPS):raise RuntimeError('Imported LOD must contain all18 animation tracks')
for track in tracks:track.mute=True
# glTF import stashes each clip with its precise layered action slot. Use that
# pair, rather than selecting an old production action with a similar name.
def lod_bind():
    lod_rig.animation_data.action_slot=None;lod_rig.animation_data.action=None
    for pb in lod_rig.pose.bones:pb.matrix_basis.identity()
    scene.frame_set(0);bpy.context.view_layer.update()
def lod_pose(clip,phase):
    track=next(t for t in tracks if t.name==clip)
    if len(track.strips)!=1:raise RuntimeError('Unexpected imported clip strip layout '+clip)
    strip=track.strips[0]
    lod_rig.animation_data.action=strip.action;lod_rig.animation_data.action_slot=strip.action_slot
    scene.frame_set(round(CLIPS[clip]*30*phase));bpy.context.view_layer.update()
    manifest.setdefault('lod_action_binding',[]).append({'clip':clip,'phase':phase,'track':track.name,
        'action':strip.action.name,'slot':strip.action_slot.identifier})
lod_bind();view((8.4,-9.4,4.7),(0,.10,.08),9.4)
render(RENDERS/'LOD-oblique.png',1400,1050,False,'bind',0.,'actual imported texture-free LOD pigment and silhouette')
for name,clip,phase,pos,target,scale,oral in [
 ('LOD-Attack-oblique','Attack',.46,(8.4,-9.4,4.7),(0,.10,.08),9.6,False),
 ('LOD-Eat-oral','Eat',.5,(0,-8,-.16),(0,-1.65,-.16),2.8,True),
]:
    lod_pose(clip,phase);view(pos,target,scale,oral)
    render(RENDERS/(name+'.png'),1400,1050,False,clip,phase,'actual imported LOD action/slot: jaw, floor, corners, fins and tail review')
lod_bind();view((8.4,-9.4,4.7),(0,.10,.08),9.4)
