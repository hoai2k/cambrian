...
OUT=HERE/'candidate07-pixel-classification-01.json'
...
for version in ('06','07'):
    ...
    def posed(ids):
        ...
        return r[:,:3]
    # Source mouth_rim(a), build_clay04.py205-213. Used only to SELECT actual
    # boundary vertices. Deformation uses actual exported joints/weights/keys.
    rim=[];selection_errors=[];duplicate_spreads=[]
    for j in range(192):
        a=2*math.pi*j/192;c,s=math.cos(a),math.sin(a)
        expected=np.array((.91*c,-.080+.064*s-.020*max(0,s)**3-.014*max(0,-s)**8,2.60-.70*abs(c)**2.6))
        distance=np.linalg.norm(ps-expected,axis=1);ids=np.flatnonzero(distance<1e-6);assert len(ids)>0,('Missing exact source rim',j,float(distance.min()))
        actual=posed(ids);spread=float(np.max(np.linalg.norm(actual-actual[0],axis=1)))
        assert spread<2e-6,'Split aperture vertices disagree'
        selection_errors.append(float(distance[ids].max()));duplicate_spreads.append(spread);rim.append(actual[0])
    rim=np.array(rim)
    if version=='06':
    ...
    for row in rays:
        ...
        heights=[]
        for p,q in zip(rim,np.roll(rim,-1,axis=0)):
            if (p[0]<=x<q[0])or(q[0]<=x<p[0]):heights.append(float(p[1]+(q[1]-p[1])*(x-p[0])/(q[0]-p[0])))
        heights.sort();assert len(heights)==2,('Aperture projection not a simple vertical interval',row['pixel'],heights)
    ...
    report['records'].append({'asset':str(path),'sha256':sha(path),'evidence':str(evidence),'evidence_sha256':sha(evidence),
        'rim_selection_max_error':max(selection_errors),'rim_duplicate_pose_spread':max(duplicate_spreads),'actual_posed_rim':rim.tolist(),
