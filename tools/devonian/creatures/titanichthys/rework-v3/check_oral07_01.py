...
LOCAL=ROOT.parent/'devonian-authoring/titanichthys/rework-v3';SRC=LOCAL/'candidate-07';OUT=SRC/'oral-sweep-01'
...
assert (SRC/'export-structural-review.json').exists(),'Run structural gate first'
assert not OUT.exists(),'Preserve existing sweep';OUT.mkdir()
PLANES=[-.68,-.60,-.52,-.451,-.40,-.30,-.18,-.003,.003,.18,.30,.40,.455,.52,.60,.68]
report={'script_sha256':sha(__file__),'candidate_report_sha256':sha(SRC/'candidate-report.json'),
...
def svg(path,sets,x):
    ...
    path.write_text('\n'.join(rows+['</svg>']))
def visible_roles(body):
    p=body['positions'];f=np.asarray(body['indices']).reshape(-1,3)
    tree=BVHTree.FromPolygons(p,f.tolist(),all_triangles=True);counts={};bad=[]
    for cx in (474,927):
        for py in range(535,581,5):
            for px in range(cx-40,cx+41,5):
                ...
                if role=='underside':bad.append({'pixel':[px,py],'triangle':int(index),'point':list(hit)})
    return counts,bad
...
for name in ('titanichthys.glb','titanichthys.lod1.glb'):
    ...
    _,evaluate=read_glb(old);meshes,_=evaluate('Ability',.5);body=meshes['Titanichthys_new_continuous_sculpt_export']
    rows=[{'x':x,'crossings':crossings(segments(body,x))}for x in (-.451,.455)]
    report['baseline'].append({'file':name,'sha256':sha(old),'sections':rows});save()
    assert all(row['crossings']for row in rows),'Known oral inversion did not reproduce in unrounded baseline'
poses=[(None,0.)]+[('Ability',float(p))for p in np.linspace(0.,1.,25)]+[('Bite',.35),('Eat',.5),('Death',1.)]
for name in ('titanichthys.glb','titanichthys.lod1.glb'):
    _,evaluate=read_glb(SRC/name)
    for clip,phase in poses:
        ...
        if (clip,phase)==('Ability',.5):
            counts,bad=visible_roles(body);record['original_patch_ray_counts']=counts;record['underside_first_rays']=bad
        ...
        if 'underside_first_rays'in record:assert not record['underside_first_rays'],'Actual underside still occludes maximum Ability floor'
