"""Bespoke Tiktaalik sp. reconstruction; Blender 4.5+ standalone builder."""
...
OUT=os.path.join(LOCAL,'v2-candidate')
...
# Dedicated Tiktaalik V2 anatomy and original mapped PBR materials.
...
if os.environ.get('TIKTAALIK_QUICK'):
 render(os.path.join(LOCAL,'v2-silhouette.png'),1000,750,False)
 cam.location=(9,0,.55);cam.rotation_euler=(Vector((0,1.6,0))-cam.location).to_track_quat('-Z','Y').to_euler();render(os.path.join(LOCAL,'v2-side.png'),1000,750,False)
...
for clip,phase,view in [('Idle',0,'side'),('Swim',.35,'side'),('Eat',.125,'front'),('Bite',.5,'side'),('Heavy',.20,'threequarter'),('Ability',.38,'front'),('Guard',.5,'threequarter'),('Dodge',.5,'side'),('Death',1,'threequarter')]:
 rig.animation_data.action=bpy.data.actions[clip];scene.frame_set(round(CLIPS[clip]*30*phase));cam.location={'side':(9,0,1.2),'front':(0,-10,1),'threequarter':(7,-6,6)}[view];cam.rotation_euler=(Vector((0,1.6,0))-cam.location).to_track_quat('-Z','Y').to_euler();render(os.path.join(LOCAL,clip+'-'+view+'.png'),900,675,False)
