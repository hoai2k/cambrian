...
for(const level of ['full','lod']){
 const file=path.join(local,'v2-candidate',`tiktaalik${level==='lod'?'.lod1':''}.glb`);const bytes=await fs.readFile(file);const doc=await io.readBinary(bytes);const meshes=[];
 ...
 const out=path.join(local,'eye-audit-'+level);await fs.mkdir(out,{recursive:true});await fs.writeFile(path.join(out,'tiktaalik.geometry.json'),JSON.stringify({id:'tiktaalik',revision:'local-v2-'+level,asset:file,sha256:createHash('sha256').update(bytes).digest('hex'),coordinateSystem:'glTF +Y up +Z forward',meshes}));console.log(out);
}
