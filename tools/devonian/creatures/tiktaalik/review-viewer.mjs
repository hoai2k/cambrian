/** Actual exported GLB review through Three.js; writes only Tiktaalik local review outputs. */
import {chromium} from 'playwright-core';
import fs from 'node:fs';
import path from 'node:path';
const out=path.resolve('../devonian-authoring/tiktaalik/viewer-v2');fs.mkdirSync(out,{recursive:true});for(const name of fs.readdirSync(out))if(name.endsWith('.png'))fs.unlinkSync(path.join(out,name));
...
try{
 await page.route('**/assets/devonian/creatures/tiktaalik*.glb*',async route=>{const file=new URL(route.request().url()).pathname.split('/').pop();await route.fulfill({body:fs.readFileSync(path.resolve('../devonian-authoring/tiktaalik/v2-candidate',file)),contentType:'model/gltf-binary'});});
 await page.route('**/tiktaalik-qa.html',route=>route.fulfill({body:'<!doctype html><html><body></body></html>',contentType:'text/html'}));
 await page.goto((process.env.QA_BASE_URL||'http://127.0.0.1:5173')+'/tiktaalik-qa.html',{waitUntil:'domcontentloaded',timeout:60000});
 const report=await page.evaluate(async()=>{
  ...
  const {auditDevonian}=await import('/tools/devonian/runtime-audit.ts');const audit=await auditDevonian(['tiktaalik']);
  const gltf=await new GLTFLoader().loadAsync('/assets/devonian/creatures/tiktaalik.glb?review=v2');document.body.replaceChildren();document.body.style.margin='0';
  ...
  const oralFill=new T.PointLight(0xffe0cc,2.8,8,2);oralFill.position.set(0,-.28,3.25);oralFill.visible=false;scene.add(oralFill);
  ...
  window.reviewPose=(clip,phase,view)=>{mixer.stopAllAction();action=mixer.clipAction(gltf.animations.find(a=>a.name===clip));action.reset().play();mixer.update(0);action.time=action.getClip().duration*phase;mixer.update(0);gltf.scene.updateMatrixWorld(true);
   ...
   const views={threequarter:[[8,6.2,9],[0,0,-1.65]],side:[[12,.65,0],[0,.02,-1.65]],front:[[0,2,12],[0,0,-.5]],dorsal:[[0,16,-1.65],[0,0,-1.65]],eye:[[1.10,1.30,2.60],[.20,.19,1.66]],mouth:[[1.55,.55,4.8],[0,-.22,1.9]],mouthleft:[[-1.55,.55,4.8],[0,-.22,1.9]],palate:[[.4,-.25,4.5],[0,.03,1.6]],eyedorsal:[[0,2.6,1.6],[0,.19,1.6]]};const[p,t]=views[view];cam.zoom=['eye','mouth','mouthleft','palate','eyedorsal'].includes(view)?0.84:view==='front'?1.55:1.17;cam.updateProjectionMatrix();cam.position.set(...p);cam.lookAt(...t);r.render(scene,cam);return {clip,phase,view};};
  window.reviewLod=async()=>{oralFill.visible=false;mixer.stopAllAction();scene.remove(gltf.scene);const low=await new GLTFLoader().loadAsync('/assets/devonian/creatures/tiktaalik.lod1.glb?review=v2');scene.add(low.scene);cam.zoom=1.17;cam.updateProjectionMatrix();const lm=new T.AnimationMixer(low.scene);lm.clipAction(low.animations.find(a=>a.name==='Idle')).play();lm.update(0);cam.position.set(8,6.2,9);cam.lookAt(0,0,-1.65);r.render(scene,cam);};
 ...
 for(const view of ['front','side','dorsal','eye','eyedorsal','mouth','mouthleft','palate']){await page.evaluate(v=>window.reviewPose(['mouth','mouthleft','palate'].includes(v)?'Heavy':'Idle',['mouth','mouthleft','palate'].includes(v)?.30:0,v),view);await saveCanvas('neutral-'+view+'.png');}
 ...
 if(!process.env.TIKTAALIK_QA_QUICK){const movie=await page.evaluate(async()=>{
