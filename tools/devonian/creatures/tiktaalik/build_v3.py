"""Tiktaalik V3: ports the approved shape study (anatomy_v3.py) over the V2 body, standalone
Blender builder. V2 is kept beside this untouched — build.py still runs V2 by default; this file
is the whole way this candidate is produced. Only anatomy_v3/materials_v3 are swapped in for
anatomy_v2/materials_v2; the skeleton, rig, actions_v2 animation and everything below the mesh
build are identical to build.py. Candidate output goes to v3-candidate/, never v2-candidate/.
"""
...
LOCAL=os.environ.get('DEVONIAN_AUTHORING',os.path.abspath(os.path.join(ROOT,'../devonian-authoring/tiktaalik')))
OUT=os.path.join(LOCAL,'v3-candidate')
os.makedirs(LOCAL,exist_ok=True);os.makedirs(OUT,exist_ok=True)
ID='tiktaalik';CLIPS={'Idle':2.4,'Swim':2.4,'TurnLeft':1.6,'TurnRight':1.6,'Dive':1.4,'Rise':1.4,'Attack':1.,'Bite':.5,'Heavy':1.1,'Hit':.6,'Death':1.6,'Guard':1.,'Parry':.35,'Dodge':.4,'Eat':1.6,'Stagger':1.2,'Ability':2.4,'Growth':1.5}
...
bone('root',(0,0,0),None);bone('body',(0,0,0),'root');bone('neck',(0,-.55,.015),'body');bone('skull',(0,-1.02,.025),'neck');bone('jaw',(0,-1.24,-.045),'skull');bone('throat',(0,-.62,-.10),'skull')
for i,y in enumerate([.45,1.25,2.05,2.75,3.4,3.95]):bone('tail%d'%i,(0,y,0),'body' if i==0 else 'tail%d'%(i-1))
for side in [-1,1]:
 s='L' if side==1 else 'R';bone('cheek'+s,(side*.54,-1.12,.0),'skull')
 bone('pectoral'+s,(side*.43,-.70,-.13),'body');bone('elbow'+s,(side*.83,-.46,-.22),'pectoral'+s);bone('distal'+s,(side*1.08,-.10,-.29),'elbow'+s);bone('web'+s,(side*1.22,.31,-.30),'distal'+s)
 bone('pelvic'+s,(side*.33,1.47,-.12),'tail1');bone('pelvicDistal'+s,(side*.63,1.75,-.24),'pelvic'+s);bone('pelvicWeb'+s,(side*.85,2.15,-.27),'pelvicDistal'+s)
...
# Dedicated Tiktaalik V3 anatomy (the ported shape study) and its matching materials sampling.
sys.path.insert(0,HERE)
from anatomy_v3 import make
make(globals())
from materials_v3 import build_materials
...
anchors=[{'name':'anchor_mouth','bone':'jaw','point':[0,-2.78,-.03],'role':'mouth'}, {'name':'anchor_mouth_inside','bone':'skull','point':[0,-1.37,-.025],'role':'swallow'},{'name':'anchor_attack_primary','bone':'skull','point':[0,-2.8,0],'role':'attack'}]
open(os.path.join(HERE,'anchors-v3.json'),'w').write(json.dumps({ID:anchors},indent=2))
...
sources=[{'title':'Daeschler, Shubin & Jenkins 2006, Devonian tetrapod-like fish','url':'https://doi.org/10.1038/nature04639'},{'title':'Shubin, Daeschler & Jenkins 2006, Tiktaalik pectoral fin','url':'https://doi.org/10.1038/nature04637'},{'title':'Lemberg, Daeschler & Shubin 2021, feeding system','url':'https://knowledge.uchicago.edu/records/s34bv-j5m06'},{'title':'Stewart et al. 2024, axial skeleton','url':'https://knowledge.uchicago.edu/records/mvb59-qth34'},{'title':'Stewart et al. 2020, fin ray patterns','url':'https://doi.org/10.1073/pnas.1915983117'}]
notes=['Original Tiktaalik roseae reconstruction of a representative 2 m animal, not a fossil scan or species maximum.','Broad flattened skull with posterior dorsal orbits, short mobile neck and no bony opercular cover; the rigid central skull is not stretched during cheek expansion.','Four robust fins retain continuous lepidotrichial webs; the pectoral skeleton has shoulder, elbow and distal support zones, not tetrapod fingers. Pelvic fins are substantial but restricted hip retraction does not imply normal terrestrial walking.','Rhombic narrowly overlapping scales and dermal roof texture are fossil-informed. Pigmentation, soft tissues and the low inferred caudal outline remain artistic reconstruction.','Animations represent aquatic undulation, paddling, bracing, neck aiming, gape and modest cheek expansion. Growth is a relaxed extension display, with no body scaling. Runtime labels do not define Devonian gameplay.']
meta={'id':ID,'name':'Tiktaalik','species':'Tiktaalik roseae','provenance':'Late Devonian, early Frasnian Fram Formation, Ellesmere Island, Nunavut, Canada','description':'Broad-headed finned tetrapodomorph with dorsal eyes, a mobile neck, rib-supported low trunk and robust jointed paired fins retaining fin-ray webs.','lengthMeters':2.,'modelLength':max(p[1]for p in V)-min(p[1]for p in V),'locomotion':'Swim','clips':list(CLIPS),'looping':LOOPS,'anchors':[a['name']for a in anchors],'sources':sources,'notes':notes}
open(os.path.join(OUT,ID+'.json'),'w').write(json.dumps(meta,indent=2))
report={'vertices':len(V),'fullTriangles':fulltris,'lodTriangles':lodtris,'reductionRatio':lodtris/fulltris,'bones':len(B),'clips':CLIPS,'loopSeams':seams,'boundsAtEveryFrame':bounds,'actionMetrics':actionMetrics,'weightNormalization':True,'rootStable':True,'noScaleChannels':True,'anchorCount':3,'loftExcursion':globals().get('loftExcursion'),'fullBytes':os.path.getsize(os.path.join(OUT,ID+'.glb')),'lodBytes':os.path.getsize(os.path.join(OUT,ID+'.lod1.glb'))}
open(os.path.join(HERE,'validation-v3.json'),'w').write(json.dumps(report,indent=2))
...
d=bpy.data.cameras.new('Camera');cam=bpy.data.objects.new('Camera',d);bpy.context.collection.objects.link(cam);scene.camera=cam;d.type='ORTHO';d.ortho_scale=10.4
...
cam.location=(7,-6,6);cam.rotation_euler=(Vector((0,1.6,0))-cam.location).to_track_quat('-Z','Y').to_euler();rig.animation_data.action=bpy.data.actions['Idle'];scene.frame_set(0);scene.frame_start=0;scene.frame_end=72
...
if os.environ.get('TIKTAALIK_QUICK'):
 render(os.path.join(LOCAL,'v3-silhouette.png'),1000,750,False)
 cam.location=(9,0,.55);cam.rotation_euler=(Vector((0,1.6,0))-cam.location).to_track_quat('-Z','Y').to_euler();render(os.path.join(LOCAL,'v3-side.png'),1000,750,False)
 print('TIKTAALIK_QUICK_COMPLETE',flush=True);sys.exit(0)
...
for clip,phase,view in [('Idle',0,'side'),('Swim',.35,'side'),('Eat',.125,'front'),('Bite',.5,'side'),('Heavy',.20,'threequarter'),('Ability',.38,'front'),('Guard',.5,'threequarter'),('Dodge',.5,'side'),('Death',1,'threequarter')]:
 rig.animation_data.action=bpy.data.actions[clip];scene.frame_set(round(CLIPS[clip]*30*phase));cam.location={'side':(9,0,1.2),'front':(0,-10,1),'threequarter':(7,-6,6)}[view];cam.rotation_euler=(Vector((0,1.6,0))-cam.location).to_track_quat('-Z','Y').to_euler();render(os.path.join(LOCAL,'v3-'+clip+'-'+view+'.png'),900,675,False)
print('TIKTAALIK_COMPLETE',json.dumps(report),flush=True)
