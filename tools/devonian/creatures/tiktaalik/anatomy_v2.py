...
SEC=[(-2.80,.63,.012,-.015),(-2.63,.645,.103,.015),(-2.35,.68,.148,.020),(-1.9,.725,.188,.025),(-1.45,.74,.205,.035),(-1.05,.64,.25,.018),(-.68,.57,.30,0),(-.2,.61,.36,0),(.45,.585,.345,0),(1.1,.52,.31,0),(1.65,.43,.26,0),(2.25,.26,.225,0),(2.85,.12,.27,0),(3.4,.048,.34,0),(3.85,.023,.30,0),(4.23,.009,.145,0),(4.43,.0004,.003,0)]
def smooth(t):
 if np.isscalar(t):
...
def surf(y,a):
 ...
 yy=y+1.46*np.abs(c)**1.85*(1-smooth((y+2.8)/2.5));x=w*c;head=1-smooth((y+1.10)/.65);power=1.28;bodyZ=z+h*np.sign(s)*np.abs(s)**.88;headZ=-.015+(h+z+.015)*np.maximum(0,s)**power-.44*h*np.maximum(0,-s)**power;zz=bodyZ*(1-head)+headZ*head
...
def bw(y,a):
 if y<-.50:
  s=sin(a);c=cos(a);yy=y+1.46*abs(c)**1.85*(1-smooth((y+2.8)/2.5));jaw=float(smooth((-s-.05)/.50)*(1-smooth((yy+1.55)/.37)));cheek=float(.48*abs(c)**4*(1-smooth((y+1.15)/.57))*(1-jaw));head=float(1-smooth((y+1.07)/.57));return {'jaw':jaw*head,'cheek'+('L'if c>0 else'R'):cheek*head,'skull':(1-jaw-cheek)*head,'neck':1-head}
 if y<.45:
...
def oralPoint(t,a):
 p=Vector(surf(-2.8,a));s=sin(a);q=Vector((.63*cos(a)*(1-.95*t),p.y+(-.37-p.y)*t,-.015+(.012+(.11 if s>=0 else .022)*sin(pi*t)**1.2)*s))
...
def make(g):
 ...
 def fin(side,pelvic=False):
  ...
  else:centers=[(.33,-.78,-.10),(.74,-.48,-.20),(1.04,-.10,-.28),(1.23,.32,-.29),(1.25,.68,-.25)];widths=[.23,.28,.38,.32,.0008];depths=[.15,.13,.073,.023,.0005];names=['pectoral','elbow','distal','web'];points=[0,.27,.53,.78]
  def param(t,a):
