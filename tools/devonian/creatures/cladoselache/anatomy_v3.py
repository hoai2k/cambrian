...
V3: ports the approved fin-outline study (docs/reference/Chondrichthyes.webp reading) onto the
unchanged v2 body/mouth/gills/materials. Only the five `fin()` calls at the bottom of `make()`
change: the pectoral becomes a delta wing whose root runs further back along the body to a far
apex with a concave trailing edge; the pelvic gains the same base-sweep/concave-edge treatment;
the dorsal gets a straight leading edge and a sharper apex with a concave trailing edge; the rear
fin's apex moves further aft and higher; the caudal's lobes draw out further from the peduncle.
`spine` is untouched (queue finding only asked for the five named outlines).

Two of the ported points -- the pectoral's rootB (which is also its edge's last control point,
the two are literally the same authored vertex) and the pelvic's equivalent -- land outside the
trunk section ellipse once the root is swept back that far (see the seating check the build
reports before this module is even imported). `seat()` below is the same radial pull-in
`tools/devonian/creatures/cheirolepis/build_v3.py` uses: it rescales a point's (x, z-zc) pair
about the section centreline just enough to bring it back to -0.05 depth, changing nothing about
points that are already seated. It is applied only to those two points; the other three fins and
every other point in these two need no correction (see the reported numbers)."""
...
def seat(p,target):
 # Radial pull-in about the section ellipse centreline, ported unchanged from cheirolepis's
 # build_v3.py seat(): a no-op once the point already reads at or inside `target`.
 p=Vector(p);w,h,zc=section(p.y);d=(p.x/w)**2+((p.z-zc)/h)**2-1
 if d<=target:return p
 s=math.sqrt((target+1)/(d+1));return Vector((p.x*s,p.y,zc+(p.z-zc)*s))
def bw(y,a=0):
 if y<-.65:
...
def make(g):
 ...
 for side in [-1,1]:
  ...
  # Delta wing: root swept back to a far apex, concave trailing edge. The apex is also rootB
  # (the study authors it as one point); seat() pulls it back inside the trunk (was +0.15 deep).
  pectApex=seat(Vector(cv((.29,.62,-.24))),-.05)
  fin('pectoral',cv((.26,-.64,-.21)),tuple(pectApex),[cv(p)for p in[(.29,-.64,-.21),(.95,-.30,-.30),(1.40,.22,-.40),(1.62,.72,-.42),(1.40,.86,-.38),(.95,.80,-.31)]]+[tuple(pectApex)],'pectoral'+s,'pectoralTip'+s,thick=.058)
  # Same base-sweep/concave-edge treatment as the pectoral. rootB/edge apex was +0.04 deep.
  pelvApex=seat(Vector(cv((.105,1.69,-.13))),-.05)
  fin('pelvic',cv((.16,1.01,-.17)),tuple(pelvApex),[cv(p)for p in[(.16,1.01,-.17),(.52,1.20,-.26),(.74,1.58,-.28),(.56,1.72,-.24)]]+[tuple(pelvApex)],'pelvic'+s,thick=.033)
 # Straight leading edge, sharp apex, concave trailing edge.
 fin('dorsal',(0,-.35,.32),(0,.73,.28),[(0,-.35,.32),(0,-.06,.86),(0,.14,1.04),(0,.30,.64),(0,.52,.40),(0,.73,.28)],'dorsal',normal=(1,0,0),thick=.042)
 fin('rear',(0,1.00,.22),(0,1.84,.14),[(0,1.00,.22),(0,1.22,.58),(0,1.40,.66),(0,1.52,.40),(0,1.84,.14)],'dorsalRear',normal=(1,0,0),thick=.028)
 # Anterior spine is a stout recurved blade with a buried broad base, not a thin horn. Unchanged.
 fin('spine',(0,-.48,.29),(0,-.01,.36),[(0,-.48,.29),(0,-.37,.68),(0,-.08,.94),(0,-.13,.60),(0,-.01,.36)],'dorsal',normal=(1,0,0),thick=.040)
 # Lunate tail with lobes drawn out further from the peduncle than v2. Roots unchanged.
 fin('caudal',(0,2.52,.10),(0,2.51,-.025),[(0,2.52,.10),(0,2.80,.55),(0,3.36,1.14),(0,3.52,1.18),(0,3.20,.56),(0,2.96,.08),(0,3.12,-.44),(0,3.50,-1.00),(0,3.36,-.94),(0,2.80,-.43),(0,2.51,-.025)],'caudal',normal=(1,0,0),thick=.029)
