...
H=Path(__file__).resolve().parent;L=H.parents[3].parent/'devonian-authoring/eldredgeops/v2';Q=L/'renders';Q.mkdir(exist_ok=True)
imported=os.environ.get('ELD_IMPORT')
if imported:
 bpy.ops.wm.read_factory_settings(use_empty=True);bpy.context.scene.render.fps=30;bpy.ops.import_scene.gltf(filepath=str(L/'candidate'/('eldredgeops.lod1.glb'if imported=='lod'else'eldredgeops.glb')));Q=L/('review-export-'+imported);Q.mkdir(exist_ok=True)
else:bpy.ops.wm.open_mainfile(filepath=str(L/'eldredgeops-v2.blend'))
S=bpy.context.scene;rig=bpy.data.objects['Eldredgeops'];S.render.engine='CYCLES';S.cycles.samples=16;S.cycles.use_denoising=True;S.render.resolution_x=1200;S.render.resolution_y=900;S.render.resolution_percentage=100;S.render.image_settings.file_format='PNG';S.render.image_settings.color_mode='RGBA';S.render.film_transparent=True;S.view_settings.view_transform='AgX'
...
def render(clip,fr,file,loc=(4,-4,3.8),target=(0,0,.1),scale=4.8):
...
mode=os.environ.get('ELD_RENDER','clay')
if mode in ['clay','all']:
 render('Idle',1,'clay-threequarter');render('Idle',1,'clay-side',(5,0,.5),(0,0,.05),4.8);render('Idle',1,'clay-dorsal',(0,0,6),(0,0,0),5.5);render('Idle',1,'clay-front',(0,-5,.8),(0,-.7,.15),2.7);render('Idle',1,'clay-eye',(2,-1.4,1.1),(.68,-.59,.30),.70);render('Idle',1,'clay-ventral',(3,-2,-5),(0,.1,0),4.9);render('Ability',30,'clay-enrolled',(4,-2,1.8),(0,-.5,-.4),3.4);render('Ability',30,'clay-enrolled-side',(5,-.4,-.3),(0,-.4,-.3),3.2)
if mode=='quick':render('Idle',1,'clay-threequarter');render('Ability',30,'clay-enrolled-side',(5,-.4,-.3),(0,-.4,-.3),3.2)
if mode=='preview':
 render('Idle',1,'threequarter');render('Idle',1,'side',(5,0,.5),(0,0,.05),4.8);render('Idle',1,'dorsal',(0,0,6),(0,0,0),6.4);render('Idle',1,'front',(0,-5,.6),(0,-.7,.05),2.7);render('Idle',1,'eye',(2,-1.4,1.1),(.70,-.59,.25),.75)
...
if mode=='portrait':
 S.render.resolution_x=1600;S.render.resolution_y=1200;S.cycles.samples=32;render('Idle',1,'selection',scale=5.0)
if mode=='lod':
 for clip,fr in [('Idle',1),('Crawl',19),('Death',49)]:render(clip,fr,'lod-'+clip)
 render('Idle',1,'lod-eye',(2,-1.4,1.1),(.70,-.59,.25),.75)
print('ELDREDGEOPS_RENDER_COMPLETE',mode,flush=True)
