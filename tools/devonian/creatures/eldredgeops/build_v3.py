"""Independently authored Eldredgeops rana: rigid shell, compound eyes, biramous limbs."""
import bpy,bmesh,math,json,os,random
import numpy as np
from pathlib import Path
from math import sin,cos,pi,exp,sqrt
from mathutils import Vector
H=Path(__file__).resolve().parent;R=H.parents[3];L=R.parent/'devonian-authoring/eldredgeops/v3';O=L/'candidate';O.mkdir(parents=True,exist_ok=True)
...
for n,c,r in [('body',(.29,.32,.30),.46),('underside',(.22,.25,.23),.53),('legs',(.25,.29,.27),.48),('gills',(.26,.32,.29),.54),('eyes',(.018,.026,.026),.24),('lens',(.022,.032,.034),.16),('oral',(.19,.12,.105),.47)]:
 m=bpy.data.materials.new('Eldredgeops_'+n);m.diffuse_color=(*c,1);m.use_nodes=True;bs=m.node_tree.nodes.get('Principled BSDF');bs.inputs['Base Color'].default_value=(*c,1);bs.inputs['Roughness'].default_value=r;bs.inputs['IOR'].default_value=1.46;bs.inputs['Specular IOR Level'].default_value=.30;M[n]=m
if os.environ.get('ELD_PBR')=='1':
...
a=bpy.data.armatures.new('Eldredgeops_segmental_rig');rig=bpy.data.objects.new('Eldredgeops',a);S.collection.objects.link(rig);rig.select_set(True);bpy.context.view_layer.objects.active=rig;bpy.ops.object.mode_set(mode='EDIT')
for n,p,pa in spec:
...
def headz(x,y):
 ...
 g=.415*exp(-(x/gw)**4-((y+.89)/.47)**4);ocular=.265*exp(-((abs(x)-.635)/.132)**2-((y+.59)/.282)**4)
...
for i in range(260):
 ...
 bumps.append((x,y,rg.uniform(.018,.034),rg.uniform(.004,.012)))
...
def plate(i):
 y=-.04+i*STEP;bone=f'thorax_{i+1:02d}';w=.91-.022*i-.0012*i*i;v=[];weights=[];NX=57;NY=11
 # The occipital joint (cephalon/thorax_01) is a real articulation, not a fused seam: every other
 # inter-tergite joint is rigid-to-rigid because neighbouring segments share nearly the same pivot
 # and stay close together through the whole flexion range. thorax_01's front margin does not: it
 # is pulled by an extra compensating rotation (see animations.py) so its *absolute* angle
 # matches the rest of the arc, but that only matches angle, not position, since the cephalon and
 # thorax_01 pivot about different points. Rigidly weighting the front two rows to thorax_01 alone
 # lets that margin swing past the cephalon's own rear margin at deep flexion (Ability/Guard),
 # opening a visible gap. Blending those rows partly onto 'cephalon' bridges the joint the way the
 # coaptation membrane would, closing under flexion; at rest (curl=0) both bones are unrotated so
 # this changes nothing about the bind pose.
 JOINT_BLEND=[.55,.275] if i==0 else []
 for lower in [False,True]:
  for j in range(NY):
   t=j/(NY-1)
   for k in range(NX):
    ...
    v.append((x,yy,z));wc=JOINT_BLEND[j]if j<len(JOINT_BLEND)else 0;weights.append({'cephalon':wc,bone:1-wc}if wc else{bone:1})
 n=NX*NY;f=[]
 for off in [0,n]:
  for j in range(NY-1):
 ...
 o=mesh(f'thoracic_tergite_{i+1:02d}',v,f,bone=bone,sub=1,weights=weights);o['rigidAnatomy']=True
...
anchors=[('anchor_mouth','cephalon',(0,-.525,.067),'mouth'),('anchor_mouth_inside','oral_pump',(0,-.635,.12),'swallow'),('anchor_attack_primary','cephalon',(0,-1.375,headz(0,-1.375)-.012),'attack')]
...
S.frame_set(1);bpy.context.view_layer.update();(L/'eye-lens-layout.json').write_text(json.dumps({'ocularVolumes':EYES,'lenses':LENSES},indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(L/'eldredgeops-v3.blend'));print('ELDREDGEOPS_CLAY_READY',len(objects),len(spec),flush=True)

if os.environ.get("ELD_EXPORT")=="1":exec(compile((H/"export.py").read_text(),str(H/"export.py"),"exec"))
