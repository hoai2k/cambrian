...
H=Path(__file__).resolve().parent;R=H.parents[3];L=R.parent/'devonian-authoring/eldredgeops/v2';O=L/'candidate';O.mkdir(parents=True,exist_ok=True)
...
S.frame_set(1);bpy.context.view_layer.update();(L/'eye-lens-layout.json').write_text(json.dumps({'ocularVolumes':EYES,'lenses':LENSES},indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(L/'eldredgeops-v2.blend'));print('ELDREDGEOPS_CLAY_READY',len(objects),len(spec),flush=True)
