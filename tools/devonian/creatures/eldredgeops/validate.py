...
import numpy as np
H=Path(__file__).resolve().parent;R=H.parents[3];O=(R/'public/assets/devonian/creatures')if os.environ.get('ELD_PUBLISH')=='1'else R.parent/'devonian-authoring/eldredgeops/v2/candidate'
...
results=[]
for suffix in ['','.lod1']:
 p=O/('eldredgeops'+suffix+'.glb');d,bin=read(p);removed=0
 ...
 manifest=json.loads((H/'anchors.json').read_text())['eldredgeops']
 for s in manifest:
...
meta=json.loads((O/'eldredgeops.json').read_text());meta['modelLength']=results[0]['bounds'][2];meta['clips']=[x['name'] for x in results[0]['clips']];(O/'eldredgeops.json').write_text(json.dumps(meta,indent=2)+'\n')
