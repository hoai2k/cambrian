"""Bespoke Cheirolepis trailli reconstruction; Blender 4.5+ standalone builder."""
...
OUT=os.path.abspath(os.path.join(ROOT,'../devonian-authoring/cheirolepis/v2-candidate'))
...
HEAD=[(-2.35,.085,.018,.012,.010),(-2.22,.15,.115,.045,.003),(-1.98,.23,.265,.13,-.015),(-1.70,.28,.35,.235,-.025),(-1.38,.305,.395,.30,-.028),(-1.16,.32,.40,.33,0)]
def hd(y):
...
for side in [-1,1]:
 eye=Vector((side*.165,-1.99,.115));ell(eye,(.050,.080,.073),(.002,.004,.003),'skull')
 nostril=Vector((side*.105,-2.22,.092));ell(nostril,(.012,.020,.010),(.006,.014,.009),'skull',4)
...
for side in [-1,1]:
 ...
 fin('pectoral',(side*.29,-.67,-.22),[(side*x,y,z)for x,y,z in[(.28,-.79,-.18),(.49,-.65,-.30),(.76,-.27,-.44),(.78,-.06,-.44),(.65,.13,-.40),(.46,.22,-.29),(.27,-.19,-.23)]],'pectoral'+ss,'pectoralTip'+ss,thickness=.010,rays=20)
 fin('pelvic',(side*.215,.87,-.19),[(side*x,y,z)for x,y,z in[(.24,.76,-.21),(.43,1.04,-.37),(.53,1.34,-.41),(.48,1.48,-.38),(.28,1.39,-.24),(.19,1.12,-.20)]],'pelvic'+ss,thickness=.008,rays=13)
# One posterior dorsal with a short base, paired visually with the anal fin.
fin('dorsal',(0,1.29,.27),[(0,.99,.285),(0,1.08,.64),(0,1.24,.80),(0,1.43,.72),(0,1.71,.40),(0,1.88,.19)],'dorsal',thickness=.012,rays=24)
fin('anal',(0,1.50,-.16),[(0,1.24,-.19),(0,1.33,-.47),(0,1.54,-.63),(0,1.70,-.54),(0,1.95,-.21),(0,2.02,-.08)],'anal',thickness=.010,rays=19)
# The scaled axial lobe is strongly extended upwards; a broad ventral ray fan originates
# below it. This is deliberately not a symmetrical shark crescent or teleost tail.
fin('caudal',(0,2.52,.07),[(0,2.42,.10),(0,2.89,.31),(0,3.46,.59),(0,3.54,.57),(0,3.28,.30),(0,3.05,.035),(0,3.04,-.28),(0,3.00,-.54),(0,2.85,-.56),(0,2.67,-.34),(0,2.46,-.10)],'caudal',thickness=.012,rays=31)
