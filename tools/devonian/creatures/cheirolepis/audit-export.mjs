...
const root=path.resolve(import.meta.dirname,'../../../..'),local=path.resolve(root,'../devonian-authoring/cheirolepis'),out=path.join(local,lod?'v2-eye-audit-lod':'v2-eye-audit');await fs.mkdir(out,{recursive:true});
const file=path.join(local,'v2-candidate/cheirolepis'+(lod?'.lod1':'')+'.glb'),bytes=await fs.readFile(file),doc=await new NodeIO().readBinary(bytes),meshes=[];
...
await fs.writeFile(path.join(out,'cheirolepis.geometry.json'),JSON.stringify({id:'cheirolepis',revision:'V2 candidate',sha256:createHash('sha256').update(bytes).digest('hex'),meshes}));
await fs.writeFile(path.join(out,'selectors.json'),JSON.stringify({cheirolepis:{headMesh:'cheirolepis cheirolepis head',eyeMeshes:['cheirolepis cheirolepis eyes']}}));
