...
ROOT=Path(__file__).resolve().parents[4];LOCAL=ROOT.parent/'devonian-authoring/cheirolepis';OUT=LOCAL/'v3-candidate';REVIEW=LOCAL/'final-review-v2';REVIEW=LOCAL/('lod-review-v3' if '--lod' in sys.argv else 'final-review-v3');REVIEW.mkdir(exist_ok=True)
bpy.ops.object.select_all(action='SELECT');bpy.ops.object.delete(use_global=False);bpy.ops.import_scene.gltf(filepath=str(OUT/('cheirolepis.lod1.glb' if '--lod' in sys.argv else 'cheirolepis.glb')))
...
def render(file,w,h,loc=(7,-6,4.2),target=(0,.5,0),scale=6.7):
...
pose('Idle',0)
for suffix,w,h in ([] if '--lod' in sys.argv else [('select.png',1600,1200),('card.png',800,600),('thumb.png',256,192),('png',1200,900)]):render(OUT/('cheirolepis.'+suffix),w,h)
for clip,t in [('Idle',0),('Swim',.8),('TurnLeft',.8),('TurnRight',.8),('Dive',.7),('Rise',.7),('Attack',.34),('Bite',.25),('Heavy',.38),('Hit',.23),('Death',1.6),('Guard',.5),('Parry',.16),('Dodge',.2),('Eat',.42),('Stagger',.44),('Ability',.83),('Growth',.7)]:
 if '--lod' in sys.argv and clip not in ['Idle','Swim','Death']:continue
 pose(clip,t);render(REVIEW/(clip+'.png'),1000,750)
for clip,t in ([] if '--lod' in sys.argv else [('Bite',.25),('Eat',.4),('Heavy',.35)]):pose(clip,t);render(REVIEW/('mouth-'+clip+'.png'),1000,750,(1,-5,.7),(0,-1.9,.04),1.25)
