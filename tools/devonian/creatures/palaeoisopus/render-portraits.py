...
here=os.path.dirname(os.path.abspath(__file__));root=os.path.abspath(os.path.join(here,'../../../..'));local=os.environ.get('DEVONIAN_AUTHORING',os.path.abspath(os.path.join(root,'../devonian-authoring/palaeoisopus')));out=os.path.join(local,'v1-candidate')
bpy.ops.wm.open_mainfile(filepath=os.path.join(local,'palaeoisopus-v1.blend'));scene=bpy.context.scene;cam=scene.camera;rig=bpy.data.objects['palaeoisopus_rig'];rig.animation_data.action=bpy.data.actions['Idle'];scene.frame_set(0);cam.location=(7,-6,6);cam.rotation_euler=(Vector((0,.4,0))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=9.2;scene.cycles.samples=12
