...
here=os.path.dirname(os.path.abspath(__file__));local=os.path.abspath(os.path.join(here,'../../../../../devonian-authoring/palaeoisopus'));ANATOMY=os.environ.get('PAL_ANATOMY','v1');path=os.path.join(local,'palaeoisopus-'+ANATOMY+'.blend');bpy.ops.wm.open_mainfile(filepath=path);rig=bpy.data.objects['palaeoisopus_rig'];obj=bpy.data.objects['palaeoisopus'];scene=bpy.context.scene
chains={}
