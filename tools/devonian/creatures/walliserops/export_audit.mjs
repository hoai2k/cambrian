...
const H=import.meta.dirname,R=path.resolve(H,'../../../..'),L=path.resolve(R,'../devonian-authoring/walliserops/v2');
const io=new NodeIO();
for(const suffix of ['', '.lod1']){
 const out=path.join(L,suffix?'audit-lod':'audit-full');await fs.mkdir(out,{recursive:true});
 const asset=path.join(L,'candidate',`walliserops${suffix}.glb`),bytes=await fs.readFile(asset),doc=await io.readBinary(bytes),meshes=[];
 ...
 await fs.writeFile(path.join(out,'walliserops.geometry.json'),JSON.stringify({id:'walliserops',revision:'local-v2-candidate'+suffix,asset,sha256:createHash('sha256').update(bytes).digest('hex'),coordinateSystem:'glTF +Y up +Z forward; exported world-space rest geometry',meshes}));
 console.log(out);
}
