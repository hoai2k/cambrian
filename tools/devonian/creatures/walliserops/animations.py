"""Species-specific appendage-led actions. Executed inside the Walliserops builder."""
...
for clip,F in CLIPS.items():
 ...
 for fr in range(F+1):
  ...
  elif clip=='Guard':curl=.12*(.5-.5*cos(ph))**.55;brace=.5*curl;antenna=.045*(1-curl)
  elif clip=='Ability':
   q=pulse(t,.06,.43,.95);curl=.09*q;brace=.60*q;antenna=.11*en;bodyz=.075*q;body.rotation_euler.x=-.13*q;headb.rotation_euler.x=-.11*q;body.location.y=-.12*q
  elif clip=='Attack':
   recoil=pulse(t,0,.22,.48);reach=pulse(t,.23,.45,.94);body.location.y=.08*recoil-.20*reach;body.rotation_euler.x=-.045*recoil+.075*reach;walk=.8*en;phase=ph*2.2;feed=.65*pulse(t,.25,.48,.76);brace=.35*recoil;antenna=.15*en
  elif clip=='Bite':feed=pulse(t,.02,.33,.78);headb.rotation_euler.x=.045*pulse(t,0,.30,.92);brace=.22*en;antenna=.08*en
  elif clip=='Heavy':
   recoil=pulse(t,.01,.24,.50);press=pulse(t,.27,.52,.98);body.location.y=.11*recoil-.24*press;body.rotation_euler.x=.065*recoil-.13*press;headb.rotation_euler.x=.065*recoil-.085*press;brace=.7*recoil+.3*press;walk=.65*en;phase=ph*2;feed=.8*pulse(t,.31,.56,.82);antenna=.19*en
  ...
  for j,side,base,pts,size,pa in limbs:
   phasej=phase-j*.81+(0 if side>0 else pi);swing=max(0,sin(phasej));stance=cos(phasej);effort=walk*(1+side*asym*.35);fold=curl
   for k in range(7):
    ...
    if k==0:pb.rotation_euler.z=side*.20*stance*effort;pb.rotation_euler.y-=side*.24*swing*effort
