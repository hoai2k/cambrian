...
import numpy as np
H=Path(__file__).resolve().parent;R=H.parents[3];O=(R/'public/assets/devonian/creatures')if os.environ.get('WAL_PUBLISH')=='1'else R.parent/'devonian-authoring/walliserops/v2/candidate'
...
results=[]
for suffix in ['','.lod1']:
 p=O/('walliserops'+suffix+'.glb');d,bin=read(p);removed=0
 ...
 manifest=json.loads((H/'anchors.json').read_text())['walliserops']
 for s in manifest:
 ...
 for a in d['animations']:
  ...
  for c in a['channels']:
   ...
   if target=='oral_pump' and c['target']['path']=='rotation' and a['name'] in ['Attack','Bite','Heavy','Eat']:
    amount=float(np.linalg.norm(v-v[0],axis=1).max());jawproof.append({'clip':a['name'],'bone':target,'maxQuaternionDelta':amount});assert amount>1e-4
...
meta=json.loads((O/'walliserops.json').read_text());meta['modelLength']=results[0]['bounds'][2];meta['clips']=[x['name'] for x in results[0]['clips']];(O/'walliserops.json').write_text(json.dumps(meta,indent=2)+'\n')
