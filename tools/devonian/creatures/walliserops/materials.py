...
base=raw*.50+np.array([.28,.325,.35])*.50
# Regionally lighter genae/pleurae, darker axial field; pigment continuity uses shared dorsal UVs.
edge=np.clip((abs(x-.5)-.22)/.27,0,1);base+=edge[:,:,None]*np.array([.06,.055,.035]);base-=np.exp(-((x-.5)/.12)**2)[:,:,None]*np.array([.025,.023,.014]);base=np.clip(base*1.16,0,1)
...
for key,col in [('body',body),('underside',base*.30+np.array([.34,.35,.33])*.70),('legs',base*.51+np.array([.27,.31,.315])*.49),('gills',base*.30+np.array([.31,.35,.335])*.70)]:
 save(key+'-albedo.png',col);save(key+'-normal.png',normal*.5+.5);save(key+'-roughness.png',np.clip(.46+(grain-.5)*.065+(lum-.3)*.07,0,1))
print('Walliserops original UV materials ready')
