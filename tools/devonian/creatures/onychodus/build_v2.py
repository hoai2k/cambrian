"""Onychodus jandemarrai V2: the approved head study ported onto the shipped body, Blender 5.2
standalone builder.

build.py is kept beside this untouched and stays the shipped reproduction. Everything here is
build.py except the head-study changes from the approved pass-2 study (queue finding: paired lower
tusk apparatus, mapped dermal cranial bones, coherent oral/jaw motion) — a deeper nose station that
closes the blunt snout (HEAD_Y0=-2.56 instead of -2.5), a lip ridge and suture-groove relief carried
as one multiplier in hp() (SUTURES/groove() copied verbatim from the study), the tusk whorls moved
...
# Dermal cranial bones, mapped as straight-ish suture segments in (y, angle) space on the upper
# shell; copied verbatim from the approved head study.
SUTURES=[
    (-2.45,pi/2,-1.30,pi/2),           # midline of the roof
    (-1.95,.55,-1.95,pi-.55),          # transverse: frontals/parietals
    (-1.55,.45,-1.55,pi-.45),          # transverse: parietals/postparietals
    (-2.30,.62,-1.70,.62),(-2.30,pi-.62,-1.70,pi-.62),   # lateral roof bones
    (-1.85,.25,-1.35,.15),(-1.85,pi-.25,-1.35,pi-.15),   # cheek: preopercular
    (-2.20,.95,-1.75,.40),(-2.20,pi-.95,-1.75,pi-.40),   # postorbital, behind the eye down to the cheek
]
def groove(y,a):
 g=0.0
 for y0,a0,y1,a1 in SUTURES:
  L=math.hypot(y1-y0,(a1-a0)*.42);t=max(0,min(1,((y-y0)*(y1-y0)+(a-a0)*(a1-a0)*.42*.42)/(L*L)))if L>0 else 0
  d=math.hypot(y-(y0+(y1-y0)*t),(a-(a0+(a1-a0)*t))*.42);g+=math.exp(-(d/.022)**2)
 return min(1.0,g)
def hp(y,a):
 w,up,lo,z=hd(y);top=sin(a)>=0;ht=up if top else lo
 # Lip ridge along the mouth line from the nose station back to y~-1.45, fading over .25; suture
 # grooves recess the upper shell behind the brow. Never applied to the throat.
...
for low in [False,True]:
 ...
 for step in range(1,9):
  ...
  for j in range(57):
   ...
   col=tuple(C[outer[0][j]][k]*(1-t)+C[inner[0][j]][k]*t for k in range(3));row.append(vertex(p,col,jw(HEAD_Y0)if low else 'skull',(j/56,0),False))
  front.append(row)
...
for side in [-1,1]:
 for low in [False,True]:
  for k,y in enumerate(np.linspace(-2.45,-1.40,22)):
...
# Anchors moved to the new snout tip (HEAD_Y0=-2.56), the way cheirolepis' build_v3 did: mouth at
# the lip seam just behind the nose station, attack exactly at the tip. anchor_mouth_inside is
# unchanged. Written only into the candidate dir (OUT), never into this tracked directory (HERE) —
# copying the result onto the tracked anchors.json is the parent's job.
anchors=[{'name':'anchor_mouth','bone':'jaw','point':[0,-2.54,.085],'role':'mouth'}, {'name':'anchor_mouth_inside','bone':'skull','point':[0,-1.70,.055],'role':'swallow'},{'name':'anchor_attack_primary','bone':'jaw','point':[0,-2.56,.10],'role':'attack'}]
