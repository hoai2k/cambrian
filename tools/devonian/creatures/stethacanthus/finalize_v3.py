"""Finalize the V3 candidate GLBs in place (adapted from rhinodipterus/finalize-v3.py): strip the
constant identity scale tracks the Blender 5.2.1 exporter keeps per bone (a one-ULP tail_tip scale
of (1, 0.99999994, 1) made check.mjs refuse the export), drop root channels, then audit the decoded
geometry, rig, clips and anchor sockets against the candidate's own anchors.json (the snout anchors
moved with NOSE_SHIFT). Run after build_v3.py and before package.mjs."""
import json,struct,math,hashlib
from pathlib import Path
import numpy as np
H=Path(__file__).resolve().parent;R=H.parents[3];O=R.parent/'devonian-authoring/stethacanthus/v3-candidate'
...
for suffix in ['','.lod1']:
 ...
 manifest=json.loads((O/'anchors.json').read_text())['stethacanthus']
 for s in manifest:
