...
import numpy as np
H=Path(__file__).resolve().parent;R=H.parents[3];O=(R/'public/assets/devonian/creatures')if os.environ.get('STETH_PUBLISH')=='1'else R.parent/'devonian-authoring/stethacanthus/v2/candidate'
...
results=[]
for suffix in ['','.lod1']:
 p=O/('stethacanthus'+suffix+'.glb');d,bin=read(p);removed=0
 ...
 manifest=json.loads((H/'anchors.json').read_text())['stethacanthus']
 for s in manifest:
 ...
 for a in d['animations']:
  ...
  for c in a['channels']:
   ...
   if target=='jaw' and c['target']['path']=='rotation' and a['name'] in ['Attack','Bite','Heavy','Eat','Ability']:
...
meta=json.loads((O/'stethacanthus.json').read_text());meta['modelLength']=results[0]['bounds'][2];meta['clips']=[x['name'] for x in results[0]['clips']];(O/'stethacanthus.json').write_text(json.dumps(meta,indent=2)+'\n')
