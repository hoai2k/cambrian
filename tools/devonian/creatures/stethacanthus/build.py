"""Bespoke Stethacanthus sp. reconstruction; Blender 4.5+ standalone builder."""
...
LOCAL=os.environ.get('DEVONIAN_AUTHORING',os.path.abspath(os.path.join(ROOT,'../devonian-authoring/stethacanthus')))
os.makedirs(LOCAL,exist_ok=True);os.makedirs(OUT,exist_ok=True)
ID='stethacanthus';CLIPS={'Idle':2.4,'Swim':2.4,'TurnLeft':1.6,'TurnRight':1.6,'Dive':1.4,'Rise':1.4,'Attack':1.,'Bite':.5,'Heavy':1.1,'Hit':.6,'Death':1.6,'Guard':1.,'Parry':.35,'Dodge':.4,'Eat':1.6,'Stagger':1.2,'Ability':2.4,'Growth':1.5}
...
bone('dorsal',(0,-.62,.30),'body');bone('dorsalRear',(0,1.32,.28),'tail2');bone('caudal',(0,2.62,.02),'tail5')
for side in [-1,1]:
 ss='L' if side==1 else 'R';bone('whip'+ss,(side*.97,.31,-.20),'pectoralTip'+ss);bone('whipTip'+ss,(side*1.12,1.14,-.23),'whip'+ss)
...
# CMNH 8988-informed Stethacanthus sp.: body reconstruction is comparative,
# not a scan of a complete Devonian articulated animal.
sections=[(-2.15,.173,.039,-.095),(-2.01,.225,.165,-.008),(-1.77,.295,.26,.015),(-1.40,.33,.295,.017),(-.88,.33,.32,0),(-.26,.31,.325,0),(.38,.26,.275,0),(.98,.205,.235,-.008),(1.55,.151,.18,-.014),(2.13,.085,.118,-.005),(2.65,.047,.074,.035),(3.12,.014,.028,.15)]
def section(y):
...
def pigment(y,a):
 ...
 # Earthy ochre countershading; sparse soft mottling, no all-over shark denticle relief.
 saddle=max(0,cos(y*5.5+1.2*cos(a)))**8*t;band=math.exp(-((sin(a)+.03)/.17)**2)
 return (.48*(1-t)+.19*t-.032*saddle+.035*band,.40*(1-t)+.095*t-.021*saddle+.018*band,.27*(1-t)+.059*t-.014*saddle+.004*band)
def torso(i,j):
 y=-2.15+5.27*i/170;a=j*2*pi/88;return vertex(surf(y,a),pigment(y,a),bw(y,a),(j/88,(y+2.15)*.7))
rows=grid(171,88,torso,0,True);face(rows[-1],0)
# Recessed terminal opening with a continuous flexible jaw/throat transition.
def mouth(i,j):
 t=i/20;a=2*pi*j/88;p=Vector((.173*(1-.78*t)*cos(a),-2.151+.72*t,-.10+.039*(1-.60*t)*sin(a)))
...
for upper in [False,True]:
 ...
 tube(pts,[.009]*65,(.37,.27,.16),[bw(-2.15,aa+pi*i/64)for i in range(65)],2,8)
...
for upper in [False,True]:
 ...
 for side in [-1,1]:
  for i in range(7):
   t=(i+.5)/7;x=side*(.154-.084*t);y=-2.12+.33*t;z=-.10+(-sign)*.035
   base=Vector((x,y,z));height=.033+.012*sin(pi*t)
   for shift,mul in [(0,1),(-.018,.7),(.018,.7),(-.009,.32),(.009,.32)]:
...
for side in [-1,1]:
 eye=Vector((side*.269,-1.79,.11));ell(eye,(.070,.125,.125),(.002,.004,.005),'skull')
 ring=[eye+Vector((side*.025,.131*cos(i*2*pi/64),.130*sin(i*2*pi/64)))for i in range(65)]
 tube(ring,[.015]*65,(.24,.145,.080),'skull',2,8)
 # Nostrils close to front of head: dark recessed marks, not an operculum.
 p=Vector((side*.16,-2.018,.103));ell(p,(.025,.018,.012),(.007,.014,.014),'skull',4)
 ...
 for k in range(5):
  ...
  tube(lips,[.004]*28,(.31,.20,.12),'gill'+('L'if side==1 else'R'),2,6)
 ...
 tube(pts,[.0028]*90,(.30,.20,.11),[bw(p.y)for p in pts],2,5)
def fin(name,origin,boundary,base,tip=None,thickness=.012,rays=9):
 ...
 for side in [-1,1]:
  def fv(i,j):
   t=i/steps;p=point(t,j)+normal*(side*thickness*.4*(.15+.85*sin(pi*t)));col=(.28-.18*t,.18-.105*t,.095-.048*t);return vertex(p,col,weight(t),(j/(N-1),t))
 ...
 for j in np.linspace(1,N-2,rays).astype(int):
  ...
  tube(pts,[.0030*(1-t)+.0007 for t in ts],(.41,.29,.16),[weight(t)for t in ts],2,5)

# Broad fin bases meet the trunk; posterior metapterygial extensions are slender,
# separately skinned whips, not over-sized modern shark pectoral wings.
for side in [-1,1]:
 ss='L'if side==1 else'R'
 fin('pectoral',(side*.275,-.50,-.18),[(side*x,y,z)for x,y,z in[(.29,-.70,-.17),(.57,-.50,-.19),(.85,-.19,-.23),(1.04,.21,-.22),(1.01,.38,-.23),(.77,.39,-.22),(.51,.29,-.18),(.28,.01,-.17)]],'pectoral'+ss,'pectoralTip'+ss,thickness=.016,rays=12)
 fin('pelvic',(side*.165,1.16,-.15),[(side*x,y,z)for x,y,z in[(.18,1.02,-.16),(.39,1.17,-.20),(.53,1.51,-.21),(.46,1.67,-.22),(.25,1.55,-.17),(.13,1.34,-.15)]],'pelvic'+ss,thickness=.010,rays=9)
 pts=[];ww=[]
 for t in np.linspace(0,1,58):
  pts.append((side*(.985+.11*sin(pi*t)),.29+1.91*t,-.22-.025*sin(pi*t)+.045*t*t));q=max(0,min(1,(t-.22)/.65));ww.append({'whip'+ss:1-q,'whipTip'+ss:q})
 tube(pts,[.030*(1-i/57)**.7+.002 for i in range(58)],(.235,.132,.069),ww,5,12)
 # Shallow longitudinal folds on fin whip preserve long metapterygial silhouette.
 for dx in [-.009,.009]:
  pp=[(x+dx,y,z+.019*(1-i/57))for i,(x,y,z)in enumerate(pts)]
  tube(pp,[.0027*(1-i/57)+.0004 for i in range(58)],(.34,.24,.13),ww,2,5)
# The spine and brush are one coherent raised complex: a narrow leading spine,
# expanding membranous body and broad flat denticle-bearing crown. It is not hair.
# Whole complex remains rigid relative to the dedicated basal articulation bone.
def brush(i,j):
 t=i/30;a=2*pi*j/64
 yc=-.47+.60*t;ry=.15+.43*t;rx=.045+.20*t**1.5;z=.295+1.02*t
 return vertex((rx*cos(a),yc+ry*sin(a),z),(.19+.11*t,.10+.07*t,.059+.035*t),'dorsal',(j/64,t))
brushrows=grid(31,64,brush,1,True);face(reversed(brushrows[0]),1)
# Convex gently flattened cap rather than a paper-thin rectangular block.
def cap(i,j):
 r=i/14;a=2*pi*j/64;return vertex((.245*r*cos(a),.13+.58*r*sin(a),1.315+.027*(1-r*r)),(.29,.20,.112),'dorsal',(j/64,r))
grid(15,64,cap,2,True)
pts=[(0,-.63+.18*t, .31+1.015*t)for t in np.linspace(0,1,40)]
tube(pts,[.032*(1-i/39)+.020 for i in range(40)],(.38,.29,.17),'dorsal',2,12)
# Fine parallel ridges on the membranous flank, interpreted surface detail.
for side in [-1,1]:
 for k in range(11):
  aa=-1.12+2.24*k/10;pts=[]
  for t in np.linspace(.12,.96,27):
   rx=.045+.20*t**1.5;ry=.15+.43*t;pts.append((side*(rx+.003)*cos(aa),-.47+.60*t+ry*sin(aa),.295+1.02*t))
  tube(pts,[.0035]*27,(.29,.19,.105),'dorsal',2,6)
# Large crowns on brush point anteriorly; cranial field points posteriorly.
# This opposition is anatomical; defensive/inflatable/courtship interpretations
# are deliberately NOT baked into the model as proven mechanisms.
for row in range(7):
 y=-.31+row*.145
 for col in range(-3,4):
  x=col*.058
  if (x/.245)**2+((y-.13)/.58)**2>.86:continue
  base=Vector((x,y,1.34));tip=base+Vector((0,-.105,.105))
  tube([base,base.lerp(tip,.45)+Vector((0,.007,.02)),tip],[.030,.020,.001],(.47,.35,.19),'dorsal',2,9)
for row in range(7):
 y=-1.86+row*.11
 for col in range(-3,4):
  a=pi/2+col*.20;p=surf(y,a,.004);tip=p+Vector((0,.075,.061))
  tube([p,p.lerp(tip,.5)+Vector((0,-.005,.01)),tip],[.020,.013,.0006],(.40,.29,.155),'skull',2,8)
# Posterior dorsal is small and spineless; no Cladoselache keel plates.
fin('dorsalRear',(0,1.40,.19),[(0,1.09,.21),(0,1.35,.61),(0,1.52,.62),(0,1.72,.38),(0,1.92,.145)],'dorsalRear',thickness=.013,rays=10)
# Long epicercal tail axis with broad lobes, softer sweep than a tuna-like crescent.
fin('caudal',(0,2.66,.027),[(0,2.58,.08),(0,2.94,.29),(0,3.67,.76),(0,3.89,.70),(0,3.53,.33),(0,3.35,.05),(0,3.65,-.34),(0,3.52,-.47),(0,2.98,-.30),(0,2.58,-.045)],'caudal',thickness=.018,rays=19)
# Texture is reproducible procedural micro-relief; neither a fossil observation nor scraped imagery.
normal=bpy.data.images.new('Stethacanthus skin microrelief',width=512,height=512);yy,xx=np.mgrid[0:512,0:512]/512.;rng=np.random.default_rng(616);grain=rng.normal(size=(512,512));freq=np.fft.fftfreq(512);kernel=np.exp(-((freq[:,None]**2+freq[None,:]**2)*180));height=np.fft.ifft2(np.fft.fft2(grain)*kernel).real*.14;dy,dx=np.gradient(height);arr=np.stack((.5-dx*9,.5-dy*9,np.ones_like(dx),np.ones_like(dx)),axis=-1).astype(np.float32);normal.pixels.foreach_set(arr.ravel());normal.filepath_raw=os.path.join(HERE,'skin-normal.png');normal.file_format='PNG';normal.save();normal.colorspace_settings.name='Non-Color';normal.pack()
mats=[]
...
for clip,duration in CLIPS.items():
 ...
 for f in range(last+1):
  ...
  if clip=='Ability':body.rotation_euler.z=.07*e*sin(p);body.rotation_euler.x=-.045*e;body.location.z=.035*e
  ...
  pb['dorsal'].rotation_euler.x=.009*amp*wave(1.6)+.035*dead
  if clip=='Ability':
   pb['dorsal'].rotation_euler.x=-.09*e;pb['skull'].rotation_euler.x-=.08*e
  if clip=='Guard':pb['dorsal'].rotation_euler.x=-.025*(1-cos(p))
  ...
  for s in [-1,1]:
   ...
   pb['whip'+suffix].rotation_euler.z=s*.020*wave(1.8)+.045*amp*wave(2.0)
   pb['whip'+suffix].rotation_euler.x=.045*wave(2.3)+.09*dead
   pb['whipTip'+suffix].rotation_euler.z=.085*amp*wave(2.9)
   pb['whipTip'+suffix].rotation_euler.x=.04*wave(3.2)+.14*dead
   if clip=='Ability':pb['whipTip'+suffix].rotation_euler.z+=s*.12*e
   if clip=='Dodge':pb['whip'+suffix].rotation_euler.z+=.17*e
...
sources=[{'title':'Ginter and Sun (2007), p.710: CMNH 8988 spine-brush and teeth, upper Famennian Cleveland Shale; questions S. altonensis assignment','url':'https://www.app.pan.pl/archive/published/app52/app52-705.pdf'}, {'title':'Zangerl (1984), spine-brush anatomy and opposing denticle fields; comparative anatomy, functional interpretation hypothetical','url':'https://doi.org/10.1080/02724634.1984.10012016'},{'title':'Lund (1984), stethacanthid spine and paired-fin comparisons, not a Devonian specimen substitution','url':'https://doi.org/10.1016/S0016-6995(84)80095-9'},{'title':'Ginter (2018), Devonian–Permian symmoriiform record and comparative morphology','url':'https://repozytorium.uw.edu.pl/bitstreams/5cc9e710-1bcb-4353-a08d-f1f56bde31ec/download'}]
notes=['Devonian provenance anchor: spine-brush CMNH 8988 and associated teeth from upper Famennian Cleveland Shale. Ginter and Sun (2007, p.710) question its old S. altonensis assignment; consequently labelled Stethacanthus sp., not Akmonistion zangerli.','A comparative whole-animal reconstruction around partial Devonian material, not a complete specimen scan. The selected 0.7 m presentation length is an illustrative small individual, not a measured CMNH 8988 total length or a species maximum.','Oppositely directed enlarged cranial and brush denticles, membranous brush supported by an anterior spine, cladodont five-cusped dentition, long metapterygial pectoral whips and soft epicercal tail distinguish the model. Body proportions, fin whip extent, pigmentation and fine surface detail are reconstructed.','The brush is not hair. Tissue histology and inflation, deterrence or courtship hypotheses do not establish behaviour. This complex-bearing reconstruction does not establish the sex or developmental stage of CMNH 8988, nor claim every individual bore this structure.','Ability is a restrained posture displaying head and brush; only the basal rigid complex rotates. It does not inflate or shed. All animation is artistic compatibility work, not Devonian gameplay specification. Growth is relaxed fin and breathing motion.']
meta={'id':ID,'name':'Stethacanthus','species':'Stethacanthus sp. (CMNH 8988-informed reconstruction)','provenance':'Late Devonian, upper Famennian Cleveland Shale, Ohio, USA','description':'Small early chondrichthyan with opposed cranial and dorsal denticle fields, a spine-supported brush complex, slender pectoral fin whips and a sweeping epicercal tail.','lengthMeters':.7,'modelLength':max(p[1]for p in V)-min(p[1]for p in V),'locomotion':'Swim','clips':list(CLIPS),'looping':LOOPS,'anchors':[a['name']for a in anchors],'sources':sources,'notes':notes}
...
cam.location=(8,-5.6,4.5);cam.rotation_euler=(Vector((0,.65,0))-cam.location).to_track_quat('-Z','Y').to_euler();rig.animation_data.action=bpy.data.actions['Idle'];scene.frame_set(0);scene.frame_start=0;scene.frame_end=72
...
for clip,phase,view in [('Idle',0,'side'),('Swim',.35,'side'),('Eat',.125,'front'),('Bite',.5,'side'),('Heavy',.20,'threequarter'),('Ability',.38,'front'),('Guard',.5,'threequarter'),('Dodge',.5,'side'),('Death',1,'threequarter')]:
 rig.animation_data.action=bpy.data.actions[clip];scene.frame_set(round(CLIPS[clip]*30*phase));cam.location={'side':(9,0,1.2),'front':(0,-10,1),'threequarter':(8,-5.6,4.5)}[view];cam.rotation_euler=(Vector((0,.65,0))-cam.location).to_track_quat('-Z','Y').to_euler();render(os.path.join(LOCAL,clip+'-'+view+'.png'),900,675,False)
print('STETHACANTHUS_COMPLETE',json.dumps(report),flush=True)
