...
const root=path.resolve(import.meta.dirname,'../../../..'),local=path.resolve(root,'../devonian-authoring/manticoceras'),out=path.join(local,lod?'v2-eye-audit-lod':'v2-eye-audit');await fs.mkdir(out,{recursive:true});
const file=path.join(local,'initial-candidate/manticoceras'+(lod?'.lod1':'')+'.glb'),bytes=await fs.readFile(file),doc=await new NodeIO().readBinary(bytes),meshes=[];
...
await fs.writeFile(path.join(out,'manticoceras.geometry.json'),JSON.stringify({id:'manticoceras',revision:'initial preview candidate',sha256:createHash('sha256').update(bytes).digest('hex'),meshes}));
await fs.writeFile(path.join(out,'selectors.json'),JSON.stringify({manticoceras:{headMesh:'head_envelope_closed',eyeMeshes:['eye_globe_L','eye_globe_R']}}));
