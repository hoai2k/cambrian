...
H=Path(__file__).resolve().parent;R=H.parents[3];L=R.parent/'devonian-authoring/manticoceras';C=L/'initial-candidate';lod='--lod'in sys.argv;preview='--preview'in sys.argv
file=C/('manticoceras'+('.lod1'if lod else'')+'.glb');sha=hashlib.sha256(file.read_bytes()).hexdigest();D=L/('review-'+sha[:12]);D.mkdir(exist_ok=True);records=[]
...
def render(label,clip,frame,loc=(4,-5,2.5),target=(0,-.25,.20),scale=4.6,size=(1000,750)):
...
if preview:
 render('silhouette','Idle',0);render('lateral','Idle',0,(6,-.25,.25),scale=4.3);sys.exit(0)
if lod:
 for clip,frame in [('Idle',0),('Swim',18),('Death',48)]:render('LOD-'+clip,clip,frame,size=(700,525))
else:
 render('manticoceras.select','Idle',0,size=(1600,1200));shutil.copy2(D/'manticoceras.select.png',C/'manticoceras.select.png')
 render('lateral','Idle',0,(6,-.25,.25),scale=4.3,size=(800,600))
 for label,loc in [('front',(0,-4,.30)),('side',(3,-1.17,.15)),('dorsal',(0,-1.17,3))]:render('eyes-'+label,'Idle',0,loc,(0,-1.17,.02),.87,size=(600,450))
 bpy.ops.object.light_add(type='AREA',location=(.3,-2.5,-.7));o=bpy.context.object;o.data.energy=35;o.data.size=1.2;o.rotation_euler=(Vector((0,-1.45,.0))-o.location).to_track_quat('-Z','Y').to_euler()
 render('oral-lit-Eat','Eat',18,(.18,-3,.15),(0,-1.46,.006),.53,size=(600,450))
 render('oral-lit-Bite','Bite',5,(-.35,-3,.0),(0,-1.46,.006),.53,size=(600,450))
 render('aperture-Guard','Guard',15,(2.7,-3,-.8),(0,-.63,-.18),2.05,size=(800,600))
(H/('render-validation-lod.json'if lod else'render-validation.json')).write_text(json.dumps({'sourceGlbSha256':sha,'renders':records},indent=2));print('RENDER_COMPLETE',sha,flush=True)
