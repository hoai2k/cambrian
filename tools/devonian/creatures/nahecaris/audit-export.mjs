...
const root=path.resolve(import.meta.dirname,'../../../..'),local=path.resolve(root,'../devonian-authoring/nahecaris'),out=path.join(local,lod?'v2-eye-audit-lod':'v2-eye-audit');await fs.mkdir(out,{recursive:true});
const file=path.join(local,'initial-candidate/nahecaris'+(lod?'.lod1':'')+'.glb'),bytes=await fs.readFile(file),doc=await new NodeIO().readBinary(bytes),meshes=[];
...
await fs.writeFile(path.join(out,'nahecaris.geometry.json'),JSON.stringify({id:'nahecaris',revision:'initial preview candidate',sha256:createHash('sha256').update(bytes).digest('hex'),meshes}));
