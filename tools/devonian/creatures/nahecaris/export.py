...
bpy.context.view_layer.update();(H/'anchors.json').write_text(json.dumps({'nahecaris':[{'name':n,'bone':b,'point':p,'role':r}for n,b,p,r in anchors]},indent=2))
sourcepath=L/'nahecaris-initial.blend';bpy.ops.wm.save_as_mainfile(filepath=str(sourcepath))
def export(path):
...
(C/'nahecaris.json').write_text(json.dumps(meta,indent=2));(L/'build-stats.json').write_text(json.dumps({'fullTriangles':full,'lodTriangles':lod,'ratio':lod/full,'clips':clips,'source':str(sourcepath)},indent=2));print('NAHECARIS_INITIAL_CANDIDATE',full,lod,str(C),flush=True)
