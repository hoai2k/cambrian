"""v2 copy of finalize.py, pointed at v2-candidate instead of initial-candidate. The anchors
manifest read below (anchors.json, not anchors-v2.json) is deliberately the real tracked one:
the v2 port does not move the mouth/mandible anchors, so this is the actual 2e-5 check against
the frozen manifest, not a copy of it. The results write goes to validation-v2.json so the
shipped candidate's own validation.json is never touched."""
import json,struct,math,hashlib
from pathlib import Path
import numpy as np
H=Path(__file__).resolve().parent;R=H.parents[3];O=R.parent/'devonian-authoring/nahecaris/v2-candidate'
...
results=[]
for suffix in ['','.lod1']:
 p=O/('nahecaris'+suffix+'.glb');d,bin=read(p);removed=0
 ...
 manifest=json.loads((H/'anchors.json').read_text())['nahecaris']
 for s in manifest:
...
meta=json.loads((O/'nahecaris.json').read_text());meta['modelLength']=results[0]['bounds'][2];meta['clips']=[x['name'] for x in results[0]['clips']];(O/'nahecaris.json').write_text(json.dumps(meta,indent=2)+'\n')
(H/'validation-v2.json').write_text(json.dumps({'checks':'finite transforms, normalized weights, root stable, identity-scale removal, unique motion, seamless loops, socket bind alignment, true LOD reduction','models':results,'lodTriangleRatio':results[1]['triangles']/results[0]['triangles']},indent=2)+'\n')
