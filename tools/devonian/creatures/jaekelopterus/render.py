...
H=Path(__file__).resolve().parent;R=H.parents[3];L=R.parent/'devonian-authoring/jaekelopterus';C=L/'initial-candidate';lod='--lod'in sys.argv;preview='--preview'in sys.argv
...
if '--finish-basic' in sys.argv:
 ...
 (H/'render-validation.json').write_text(json.dumps({'sourceGlbSha256':sha,'renders':records},indent=2));sys.exit(0)
if preview:
