...
H=Path(__file__).resolve().parent;L=H.parents[3].parent/'devonian-authoring/jaekelopterus';C=L/'initial-candidate'
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
full=sha(C/'jaekelopterus.glb');lod=sha(C/'jaekelopterus.lod1.glb');validation=json.loads((H/'validation.json').read_text());assert [m['sha256']for m in validation['models']]==[full,lod]
...
for suffix,size in [('',(1600,1200)),('.select',(1600,1200)),('.card',(800,600)),('.thumb',(256,192))]:assert Image.open(C/('jaekelopterus'+suffix+'.png')).size==size
play=json.loads((L/'playback-validation-v2.json').read_text());assert play['sourceGlbSha256']==full and len(play['audit'])==20 and not play['errors'];assert all(r['finite']and r['rootStable']for r in play['audit']);assert sha(L/'motion-review-v2.webm')==play['videoSha256'];shutil.copy2(L/'playback-validation-v2.json',H/'playback-validation.json')
...
files={p.name:{'sha256':sha(p),'bytes':p.stat().st_size}for p in sorted(C.glob('jaekelopterus*'))};assert len(files)==7
source=L/'jaekelopterus-initial.blend';report={'candidateOnly':True,'stage':'initial-preview','species':'Jaekelopterus rhenaniae','files':files,'originalBlend':{'path':str(source),'sha256':sha(source),'bytes':source.stat().st_size},'validation':validation,'reviewReports':{p.name:sha(p)for p in H.glob('*validation*.json')},'eyeReports':{p.name:sha(p)for p in H.glob('eye-audit-*.json')},'sourceScripts':{p.name:sha(p)for p in sorted(H.glob('*.py'))},'notes':['Complete initial preview; integration agent owns packaging, viewer review and main commits.','No pre-existing model existed. Original blend and imagegen source preserved.','Five anchor nodes are explicitly supported by shared check.mjs; full and LOD metadata match.','LOD retains both Swim and Crawl because the shared intake requires Swim when the full model supplies it.','Later refinement may add finer compound-eye optics, podomere-level gait detail and improved microcuticle readability. Initial eye volume and export validity are mandatory and passed.']}
