"""Jaekelopterus v2: port of the approved shape study (scratchpad jaek/study.py NEW) onto the
initial reconstruction. Blender +Z up / -Y forward.

Triage finding fixed here: the opisthosoma was a uniform taper of rounded rings; it is now
flattened, with a broad preabdomen (segments 1-7) stepping down to a narrower postabdomen
(8-12). The chelicerae rami were rounded tubes; they are now long, slender, toothed blades.
Changes from build.py, all confined to this file plus materials_v2.py/export_v2.py:
  - `segs` width/height tables re-profiled to a broad flat preabdomen and a stepped-down
    postabdomen (study's NEW['w']/NEW['h']).
  - `shellpoint()`'s ventral term drops from .125 to .08 (study's NEW['vent']) for a flatter
    underside.
  - The cheliceral rami (`fixed`/`free` point lists) are stretched 1.28x along their length from
    their first point (y = y0 - d*1.28, x/z unchanged - study's NEW['stretch']), their tube
    `flat` drops from .77/.78 to .36 (study's NEW['flat'], a blade cross-section instead of a
    round one), their radii tighten to the study's NEW['claw']/NEW['free'], and the denticle row
    goes from 8 fixed teeth to the study's 13-per-ramus scheme (t=.20+.70*j/12, alternating
    .11/.05 sizes scaled by (1-.35*t)).
  - Bone positions (the `claws`/`legs`/`paddles` skeleton, `spec`) are untouched - the stretch
    only moves drawn ramus geometry past each ramus's first (unmoved) point, so the rig and
    motion.py need no changes.
  - `anchor_attack_primary` and `anchor_grasp_left/right` (on cheliceraL2/R2) are moved by the
    same 1.28 stretch, relative to the fixed ramus's own first point (y0=-2.30), so they stay at
    the ramus tips; see export_v2.py.
  - materials_v2.py re-tunes the dorsal/belly world-z shading thresholds for the flatter body;
    see that file's docstring for the numbers.
  - export_v2.py additionally works around a Blender-version export artifact unrelated to this
    shape port (reproduced with unmodified build.py on this Blender too): see its docstring.
Candidate output goes to a new directory (export_v2.py), never the shipped initial-candidate/.
"""
...
H=Path(__file__).resolve().parent;R=H.parents[3];L=R.parent/'devonian-authoring/jaekelopterus';C=L/'build-v2-candidate';C.mkdir(parents=True,exist_ok=True)
...
segs=[]
for i in range(12):
 y=-.18+i*.235;w=[.66,.70,.72,.72,.70,.66,.60,.42,.37,.32,.27,.23][i];h=[.19,.20,.20,.20,.195,.185,.17,.15,.14,.13,.12,.11][i];segs.append((y,w,h));spec.append(('segment%02d'%i,(0,y-.125,0),'body'if i==0 else'segment%02d'%(i-1)))
...
for i,(y,w,h)in enumerate(segs):
 ...
 def shellpoint(yy,a):
  ww,hh,zz=interp(rows,yy);ww+=.075*((yy-rows[0][0])/(rows[-1][0]-rows[0][0]))**3 if i==11 else 0;sn=sin(a);x=ww*cos(a);z=zz+hh*sn**.7 if sn>=0 else zz+.08*sn*(w/.64)**.6
...
for q,pts in claws.items():
 ...
 # Rami stretched 1.28x along their length from their (unmoved) first point, x/z kept, per the
 # approved shape study - long slender blades rather than the shipped rounded, shorter tubes.
 S=1.28
 fixed=[(sign*.80,-2.30,.015),(sign*.88,-2.30-.26*S,.023),(sign*.87,-2.30-.52*S,.017),(sign*.76,-2.30-.85*S,.0),(sign*.685,-2.30-.935*S,-.009)]
 free=[(sign*.58,-2.29,.008),(sign*.485,-2.29-.25*S,.008),(sign*.48,-2.29-.56*S,-.002),(sign*.515,-2.29-.83*S,-.015),(sign*.60,-2.29-.94*S,-.012)]
 tube('Fixed cheliceral ramus '+q,fixed,[.095,.078,.060,.034,.0015],'legs',bone,flat=.36,n=26,steps=48)
 tube('Moveable cheliceral ramus '+q,free,[.066,.056,.045,.028,.0015],'legs','finger'+q,flat=.36,n=26,steps=48)
 ...
 for freepart,curve,bn in [(False,fixed,bone),(True,free,'finger'+q)]:
  n=13
  for j in range(n):
   t=.20+.70*j/(n-1);u=t*(len(curve)-1);k=min(len(curve)-2,int(u));p=Vector(curve[k]).lerp(Vector(curve[k+1]),u-k);inner=sign*(1 if freepart else-1)
   size=(.11 if j%2==0 else .05)*(1-.35*t);p.x+=inner*(.03 if freepart else .04);end=p+Vector((inner*size,-size*.25,0))
   tube(('Free'if freepart else'Fixed')+' denticle '+q+str(j),[p,p.lerp(end,.7)+Vector((0,-.004,.002)),end],[.022 if size>.06 else .012,.012 if size>.06 else .007,.0015],'denticle',bn,n=6,steps=10)
...
exec(compile((H/'materials_v2.py').read_text(),str(H/'materials_v2.py'),'exec'))
exec(compile((H/'motion.py').read_text(),str(H/'motion.py'),'exec'))
exec(compile((H/'export_v2.py').read_text(),str(H/'export_v2.py'),'exec'))
