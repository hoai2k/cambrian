"""Independent Acanthostega initial anatomy: robust skull, polydactyl paddles, deep caudal fin."""
import math,numpy as np
from math import sin,cos,pi,exp
from mathutils import Vector
SEC=[(-2.20,.50,.022,0),(-2.06,.525,.135,.01),(-1.80,.605,.24,.025),(-1.45,.665,.29,.03),(-1.10,.66,.31,.02),(-.75,.51,.325,.005),(-.30,.44,.36,0),(.35,.42,.35,0),(1.1,.40,.33,0),(1.7,.33,.29,0),(2.1,.24,.31,0),(2.5,.16,.47,.015),(3.,.085,.63,.03),(3.8,.035,.64,.06),(4.5,.015,.40,.07),(5.,.0005,.008,.04)]
def smooth(t):
...
def surf(y,a):
 ...
 # Caudal web expands dorsally earlier than ventrally, as in the preserved supports.
...
def make(g):
 ...
 for hind in [False,True]:
  for side in [-1,1]:
   ...
   centers=[(.25,1.48,-.10),(.59,1.78,-.18),(.85,1.78,-.23),(1.08,2.01,-.25)]if hind else[(.26,-.53,-.09),(.62,-.50,-.18),(.84,-.22,-.25),(1.12,-.15,-.27)]
   widths=[.15,.155,.115,.19]if hind else[.15,.16,.11,.18];depths=[.13,.13,.10,.067];centers=[Vector((side*x,y,z))for x,y,z in centers]
   ...
   palm=centers[-1];g['ell'](palm,(.14,.185,.068),(1,1,1),names[2],5);digitPoints=[]
   for k in range(8):
