...
here=os.path.dirname(os.path.abspath(__file__));root=os.path.abspath(os.path.join(here,'../../../..'));local=os.environ.get('DEVONIAN_AUTHORING',os.path.abspath(os.path.join(root,'../devonian-authoring/acanthostega')))
# ACA_ANATOMY mirrors build.py's selection so this points at the matching candidate/.blend pair
# instead of always the shipped v1 one.
anatomy=os.environ.get('ACA_ANATOMY','v1');out=os.path.join(local,'v2-candidate'if anatomy=='v2'else'v1-candidate')
bpy.ops.wm.open_mainfile(filepath=os.path.join(local,'acanthostega-'+anatomy+'.blend'));scene=bpy.context.scene;cam=scene.camera;rig=bpy.data.objects['acanthostega_rig'];rig.animation_data.action=bpy.data.actions['Idle'];scene.frame_set(0);cam.location=(7,-6,6);cam.rotation_euler=(Vector((0,1.1,0))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=8.7;scene.cycles.samples=24
