/** Actual exported GLB review through Three.js; writes only Acanthostega local review outputs. */
import {chromium} from 'playwright-core';
import fs from 'node:fs';
import path from 'node:path';
const out=path.resolve('../devonian-authoring/acanthostega/viewer-v1');fs.mkdirSync(out,{recursive:true});for(const name of fs.readdirSync(out))if(name.endsWith('.png'))fs.unlinkSync(path.join(out,name));
...
try{
 await page.route('**/assets/devonian/creatures/acanthostega*.glb*',async route=>{const file=new URL(route.request().url()).pathname.split('/').pop();await route.fulfill({body:fs.readFileSync(path.resolve('../devonian-authoring/acanthostega/v1-candidate',file)),contentType:'model/gltf-binary'});});
 await page.route('**/acanthostega-qa.html',route=>route.fulfill({body:'<!doctype html><html><body></body></html>',contentType:'text/html'}));
 await page.goto((process.env.QA_BASE_URL||'http://127.0.0.1:5173')+'/acanthostega-qa.html',{waitUntil:'domcontentloaded',timeout:60000});
 const report=await page.evaluate(async()=>{
  ...
  const gltf=await new GLTFLoader().loadAsync('/assets/devonian/creatures/acanthostega.glb?review=v1');document.body.replaceChildren();document.body.style.margin='0';
  ...
  window.reviewPose=(clip,phase,view)=>{mixer.stopAllAction();action=mixer.clipAction(gltf.animations.find(a=>a.name===clip));action.reset().play();mixer.update(0);action.time=action.getClip().duration*phase;mixer.update(0);gltf.scene.updateMatrixWorld(true);
   ...
   const views={threequarter:[[8,6.2,9],[0,0,-1.1]],side:[[12,.65,0],[0,.02,-1.1]],front:[[0,2,12],[0,0,-.5]],dorsal:[[0,13,-1.1],[0,0,-1.1]],eye:[[1.10,1.30,2.60],[.20,.23,1.43]],mouth:[[1.55,.55,4.8],[0,-.22,1.9]],mouthleft:[[-1.55,.55,4.8],[0,-.22,1.9]],palate:[[.4,-.25,4.5],[0,.03,1.6]],eyedorsal:[[0,2.6,1.6],[0,.19,1.6]]};const[p,t]=views[view];cam.zoom=['eye','mouth','mouthleft','palate','eyedorsal'].includes(view)?0.84:view==='front'?1.45:1.30;cam.updateProjectionMatrix();cam.position.set(...p);cam.lookAt(...t);r.render(scene,cam);return {clip,phase,view};};
  window.reviewLod=async()=>{oralFill.visible=false;mixer.stopAllAction();scene.remove(gltf.scene);const low=await new GLTFLoader().loadAsync('/assets/devonian/creatures/acanthostega.lod1.glb?review=v1');scene.add(low.scene);cam.zoom=1.30;cam.updateProjectionMatrix();const lm=new T.AnimationMixer(low.scene);lm.clipAction(low.animations.find(a=>a.name==='Idle')).play();lm.update(0);cam.position.set(8,6.2,9);cam.lookAt(0,0,-1.1);r.render(scene,cam);};
 ...
 if(!process.env.ACANTHOSTEGA_QA_QUICK){const movie=await page.evaluate(async()=>{
