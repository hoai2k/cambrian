...
import numpy as np
H=Path(__file__).resolve().parent;R=H.parents[3];O=(R/'public/assets/devonian/creatures')if os.environ.get('MIC_PUBLISH')=='1'else R.parent/'devonian-authoring/michelinoceras/v1/candidate'
...
results=[]
for suffix in ['','.lod1']:
 p=O/('michelinoceras'+suffix+'.glb');d,bin=read(p);removed=0
 required={'Idle','Swim','Death'}if suffix else{'Idle','Swim','TurnLeft','TurnRight','Dive','Rise','Attack','Bite','Heavy','Hit','Death','Guard','Parry','Dodge','Eat','Stagger','Ability','Growth','Grab'}
 assert {a['name']for a in d['animations']}==required,('Required clip set',p.name)
 ...
 for i,n in enumerate(d['nodes']):
  ...
  for prim in d['meshes'][n['mesh']]['primitives']:
   ...
   if n.get('name','').startswith(('rigid_shell_','internal_septum_','internal_subcentral_')):
    joints=acc(d,bin,prim['attributes']['JOINTS_0']).astype(int);skin=d['skins'][n['skin']]['joints'];assert {d['nodes'][skin[k]]['name']for k in joints[ws>1e-6]}=={'body'},'Shell must stay rigid'
 q=np.concatenate(bounds);span=q.max(0)-q.min(0)
 pigments=np.concatenate([acc(d,bin,pr['attributes']['COLOR_0'])[:,:3]for m in d['meshes']for pr in m['primitives']])
 if suffix:
  assert pigments.max()>.05 and pigments.mean()<.80 and pigments.std()>.003,('LOD pigmentation',pigments.mean(),pigments.std())
  for m in d['meshes']:
   for pr in m['primitives']:assert acc(d,bin,pr['attributes']['COLOR_0'])[:,:3].mean()<.80,('Unpigmented LOD primitive',m['name'])
 else:assert np.allclose(pigments,1),('Full neutral COLOR_0',pigments.mean())
 sockets=[]
 for i,n in enumerate(d['nodes']):
 ...
 manifest=json.loads((H/'anchors.json').read_text())['michelinoceras']
 for s in manifest:
 ...
 for a in d['animations']:
  ...
  for c in a['channels']:
   ...
   if target=='beak_lower' and c['target']['path']=='rotation' and a['name'] in ['Attack','Bite','Heavy','Eat']:
    amount=float(np.linalg.norm(v-v[0],axis=1).max());jawproof.append({'clip':a['name'],'bone':target,'maxTranslationDelta':amount});assert amount>1e-4
   ...
   if a['name']in ['Idle','Crawl','Swim','Guard','Eat']:assert np.allclose(v[0],v[-1],atol=1e-5),a['name']
 ...
 results.append({'file':p.name,'bytes':p.stat().st_size,'vertices':verts,'triangles':tri,'bounds':span.tolist(),'bones':len(d['skins'][0]['joints']),'sockets':sockets,'clips':motions,'jawArticulation':jawproof,'removedIdentityScaleChannels':removed,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()})
assert results[1]['triangles']/results[0]['triangles']<.4
meta=json.loads((O/'michelinoceras.json').read_text());meta['modelLength']=results[0]['bounds'][2];meta['clips']=[x['name'] for x in results[0]['clips']];(O/'michelinoceras.json').write_text(json.dumps(meta,indent=2)+'\n')
