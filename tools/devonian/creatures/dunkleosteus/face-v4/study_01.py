"""Frozen local face study; patch preserved V2, never rebuild or publish the family.
...
MODE = os.environ.get('DUNK_FACE_MODE', 'candidate01')
assert MODE in ('candidate01', 'baseline')
...
def sculpt_face():
    ...
    for v in head.data.vertices:
        ...
        x, y, z = p[:]
        s = 1 if x >= 0 else -1
        outside = smooth(.08, .60, v.normal.x*s)
        ...
        raised = .032 * smooth(-.006, .041, d)
        seam = -.005 * math.exp(-(d/.009)**2)
        browdist = min(segment_distance(y, z, a, b) for a, b in zip(brow, brow[1:]))
        supraorbital = .034 * math.exp(-(browdist/.025)**2)
        ...
        roof = smooth(.25, .75, v.normal.z)
        p.z += roof * .031 * math.exp(-((y+1.39)/.135)**2) * (1.-.38*smooth(.12,.25,abs(x)))
    head.data.update()
...
def blade(name, rows, bone, s):
    ...
    # Piecewise cubic smoothstep keeps shaped landmarks and prevents overshoot.
    sections = []
    ...
    for i, (y, x, zr, ze, w, edge) in enumerate(sections):
        h = ze-zr
        cross = [(x+w,zr,.04), (x+w*.78,zr+h*.38,.34), (x+edge,ze,.94),
                 (x-edge,ze,.94), (x-w*.59,zr+h*.42,.36), (x-w*.66,zr,.04)]
    ...
    obj['faceStudy']='V4 candidate01, sculpted gnathal, awaiting contact and eye QA'
    return obj
...
if MODE == 'candidate01':
    ...
    for s in (-1,1):
        ...
        blade('inferognathal_cutting_blade_'+str(s), [
            ...
            (-1.373,.219,-.173,.016,.028,.0013),
            (-1.349,.230,-.183,.028,.029,.0010),
            (-1.319,.240,-.195,-.029,.029,.0017),
            (-1.271,.253,-.218,-.102,.027,.0020),
            (-1.211,.267,-.241,-.136,.025,.0017),
            (-1.044,.288,-.281,-.171,.021,.0016),
            (-.883,.288,-.285,-.180,.013,.0017)], 'jaw',s)
