...
Q=R.parent/'devonian-authoring/dunkleosteus/face-v4/candidate03/exports-pigment04'
...
proof=json.loads((Q/'pigment-repair-proof.json').read_text())
assert sha(Q/'pigment-repair-proof.json')==manifest['pigmentRepairProofSHA256']
assert proof['binaryChunkUnchanged'] and proof['nonColorDocumentEquality']
assert proof['correctedLODSHA256']==manifest['models'][1]['sha256']
...
for model,row in zip(manifest['models'],summary):
    ...
    carried=audit['carriedForwardFrom']
    assert sha(carried['report'])==carried['reportSHA256']
    assert carried['proofSHA256']==sha(Q/'pigment-repair-proof.json')
...
meta['artVersion']=4;meta['artCandidate']='face-v4-candidate03-pigment04';meta['status']='final'
...
meta['notes'].append('V4 focused cranial and gnathal refinement; preserved V2 body, oral lining, rig, eye globes and anchors. Actual full/LOD eye-volume and 173 feeding-pose checks passed before an exact-geometry COLOR binding repair; audit transfer is hash-proven. Root completed individual visual review including corrected LOD pigment.')
meta['faceRefinementEvidence']={'assets':[{k:m[k] for k in ('path','sha256','triangles','bytes')} for m in manifest['models']],
    ...
    'auditReports':[{'path':r['report'],'sha256':r['reportSHA256'],'kind':'carried-forward exact-geometry audit'} for r in summary],
    'pigmentRepairProofSHA256':sha(Q/'pigment-repair-proof.json'),
