"""Matched actual full/LOD views after RGB color-space correction; geometry unchanged.
...
Q=R.parent/'devonian-authoring/dunkleosteus/face-v4/candidate03/exports-linear05'
manifest=json.loads((Q/'export-evidence.json').read_text())
sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
mode='poses' # Pigment04 rerenders only the eight corrected LOD views.
assert not (Q/('render-evidence-'+mode+'.json')).exists(), 'Refuse to overwrite render evidence'
if mode in ('all','poses'):
    assert all(not (Q/n).exists() or not any((Q/n).iterdir()) for n in ('review-lod','review-full')), 'Refuse to overwrite prior review images'
...
for model in manifest['models']:
    ...
    lod='.lod1.' in p.name
    # Both actual full and LOD are rendered for a matched comparison.
    ...
    if mode in ('all','poses'):
        ...
        for name,clip,frame,loc,target,scale in views:
            if name not in ('rest-oblique','max-front'):continue
            render(out/(name+'.png'),clip,frame,loc,target,scale,1000,800)
