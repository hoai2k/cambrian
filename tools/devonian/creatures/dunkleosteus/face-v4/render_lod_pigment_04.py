"""Actual corrected-pigment LOD views only; full views and portraits are hash-preserved.
...
if mode in ('all','poses'):
    assert all(not (Q/n).exists() or not any((Q/n).iterdir()) for n in ('review-lod',)), 'Refuse to overwrite prior review images'
