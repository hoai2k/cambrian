"""Finalize the V3 candidate GLBs in place (adapted from stethacanthus/finalize_v3.py, itself
adapted from rhinodipterus/finalize-v3.py): strip the constant identity scale tracks the Blender
5.2.1 exporter keeps per bone, drop root channels, then audit the decoded geometry, rig, clips and
anchor sockets against the candidate's OWN anchors.json (written by build_v3.py into the candidate
directory, never into tools/devonian/creatures/dunkleosteus/ — anchor points are unchanged from V2,
only the head surface around them moved). Run after build_v3.py and before package.mjs.
"""
...
assert 'public' not in O.parts, 'V3 candidate finalize must never touch public/'
...
results=[]
for suffix in ['','.lod1']:
 p=O/('dunkleosteus'+suffix+'.glb');d,bin=read(p);removed=0
 ...
 manifest=json.loads((O/'anchors.json').read_text())['dunkleosteus']
 for s in manifest:
 ...
 results.append({'file':p.name,'bytes':p.stat().st_size,'vertices':verts,'triangles':tri,'bounds':span.tolist(),'bones':len(d['skins'][0]['joints']),'boneNames':[d['nodes'][i]['name']for i in d['skins'][0]['joints']],'textures':len(d.get('textures',[])),'sockets':sockets,'clips':motions,'feedingArticulation':jawproof,'removedIdentityScaleChannels':removed,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()})
...
(H/'validation_v3.json').write_text(json.dumps({'checks':'finite transforms, normalized weights, root stable, identity-scale removal, unique motion, seamless loops, socket bind alignment, true LOD reduction','models':results,'lodTriangleRatio':results[1]['triangles']/results[0]['triangles']},indent=2)+'\n')
