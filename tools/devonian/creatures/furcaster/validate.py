...
import numpy as np
H=Path(__file__).resolve().parent;R=H.parents[3];O=(R/'public/assets/devonian/creatures')if os.environ.get('FUR_PUBLISH')=='1'else R.parent/'devonian-authoring/furcaster/v1/candidate'
...
results=[]
for suffix in ['','.lod1']:
 p=O/('furcaster'+suffix+'.glb');d,bin=read(p);removed=0
 ...
 manifest=json.loads((H/'anchors.json').read_text())['furcaster']
 for s in manifest:
 ...
 for a in d['animations']:
  ...
  for c in a['channels']:
   ...
   if target=='oral_pump' and c['target']['path']=='translation' and a['name'] in ['Attack','Bite','Heavy','Eat']:
...
meta=json.loads((O/'furcaster.json').read_text());meta['modelLength']=results[0]['bounds'][2];meta['clips']=[x['name'] for x in results[0]['clips']];(O/'furcaster.json').write_text(json.dumps(meta,indent=2)+'\n')
