...
-linux variant: identical to build_material01.py except for one line. .blend
files are not byte-reproducible across Blender saves (two consecutive saves
of an identical scene, same machine/build, differ in SHA-256 - Blender's file
format carries save-time pointer/session state) so a from-scratch rebuild of
clay02 in this checkout can never match the original Mac-session blend hash
below, regardless of platform or content fidelity. This stage is instead
verified BY VALUE: the numeric invariants build_clay02.py's own source-check.json
reported (vertex/face/triangle counts, Euler characteristic, closure, minimum
triangle area, root-transition normal dot) were confirmed to match the
handoff-recorded clay02 figures before this file was authored or run.
"""
...
# Verified by value, not by blend byte-hash (see module docstring). This is the
# actual sha256 of the clay02.blend rebuilt here from the unmodified frozen
# build_clay02.py, in place of the unreproducible original '55ca3b38...'.
assert hashlib.sha256(BASE.read_bytes()).hexdigest()=='7326976ff9cb561b3f388c79820b3315072a322e87d5cffba4e5064e13514743'
