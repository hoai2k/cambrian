"""Bounded MATERIAL02 fix on exact MATERIAL01 geometry. CPU2; no export/rig.
Explicit sRGB PNG encoding, ventral UV wrap, shared-boundary pigment extension,
and low-amplitude scaleless skin response. All shape-key coordinates preserved.

-linux variant: identical to build_material02.py except EXPECTED below. .blend
files are not byte-reproducible across Blender saves (proven empirically: two
consecutive saves of an identical scene, same machine/build, differ in
SHA-256), so a from-scratch rebuild can never match the original Mac-session
hash regardless of content fidelity. Verified BY VALUE instead: material01's
source-check.json numeric invariants (vertex/face/triangle counts, Euler,
closure, suture displacement, 14 plate paths, unchanged base coordinates)
were confirmed against the handoff-recorded figures before this file was run.
"""
...
from material_fields02 import shield,pectoral,posterior,linear_to_srgb,srgb_to_linear,extend_boundary_colors,PATHS
BASE=REPO.parent/'devonian-authoring/bothriolepis/rework-v3/material01/bothriolepis-material01.blend'
# Actual sha256 of the material01.blend rebuilt here by the unmodified frozen
# build_material01-linux.py, in place of the unreproducible original hash.
EXPECTED='794ebc208fff9b57fad1aeec7087162205cbab430c301ea8f765e52b587eddad'
assert hashlib.sha256(BASE.read_bytes()).hexdigest()==EXPECTED
OUT=REPO.parent/'devonian-authoring/bothriolepis/rework-v3/material02';OUT.mkdir(parents=True,exist_ok=True)
assert not (OUT/'bothriolepis-material02.blend').exists(),'STOP existing MATERIAL02 candidate'
...
body=bpy.data.objects['Bothriolepis V3 MATERIAL01 continuous anatomy'];me=body.data
...
sc,shgt,sr,_,_=shield(uu,vphysical,detail);shield_maps=atlas('shield-material02',sc,shgt,sr,1.775,3.,True)
pc,phgt,pr=pectoral(uu,vtex,detail);pectoral_maps=atlas('pectoral-material02',pc,phgt,pr,1.294,.24,False)
bc,bh,br=posterior(uu,vphysical,detail);body_maps=atlas('posterior-material02',bc,bh,br,3.20,.80,True)
...
def mat(name,imgs=None,vertex=False,oral=False):
 ...
 normal=None
 if vertex:
  attr=nodes.new('ShaderNodeVertexColor');attr.layer_name='AnatomicalPigmentM02';links.new(attr.outputs['Color'],bs.inputs['Base Color'])
 if imgs:
  uv=nodes.new('ShaderNodeUVMap');uv.uv_map='AnatomicalUVM02'
  for im,socket in zip(imgs,['Base Color','Normal','Roughness']):
   ...
   if socket=='Normal':
    normal=nodes.new('ShaderNodeNormalMap');normal.uv_map='AnatomicalUVM02';normal.inputs['Strength'].default_value=.48
 ...
 return m
mats=[mat('M02 anatomical shield',shield_maps),mat('M02 dermal pectorals',pectoral_maps),mat('M02 quiet scaleless posterior',body_maps),mat('M02 internal oral mucosa',oral=True),mat('M02 boundary-matched exterior oral rim',vertex=True),mat('M02 boundary-matched pectoral roots',vertex=True),mat('M02 narrow dermal articulation',vertex=True),mat('M02 boundary-matched rostral closure',vertex=True)]
me.materials.clear()
...
attr=me.color_attributes.new(name='AnatomicalPigmentM02',type='FLOAT_COLOR',domain='POINT')
for i,col in enumerate(colors):attr.data[i].color=(*col,1)
uvlayer=me.uv_layers.new(name='AnatomicalUVM02')
...
assert geometry_hash()==frozen_geometry,'STOP material correction changed sculpt or shape keys'
body.name='Bothriolepis V3 MATERIAL02 continuous anatomy'
body['source_material01_sha256']=EXPECTED;body['phase']='MATERIAL02 bounded appearance correction; art review required'
body['geometry_preserved_sha256']=frozen_geometry
...
print('BOTHRIOLEPIS_MATERIAL02_DONE '+str(OUT/'outputs-sha256.json'))
