"""Bothriolepis V3, newly authored continuous anatomical clay geometry.
...
def section(y,theta,relief=True):
 ...
 if y>1.64: ex*=.37+.63*abs(sin(theta))**1.8
...
def build_body():
 ...
 left=(23,37,48,69);right=(23,37,123,144)
 ...
 for t in [.25,.50,.75,1.0]:
  ...
  m.bridge(ring,new,0 if t<.75 else 2);ring=new
 mouth_ids=ring[:]
 for depth,scale in [(.010,.99),(.034,.84),(.072,.58),(.107,.30)]:
  new=[m.vertex((.143*scale*cos(a),-1.455+.035*depth/.107+.091*scale*sin(a),-.353+depth),'oral_cavity',.50*max(0,sin(a))*(1-depth/.107)) for a in phis]
  m.bridge(ring,new,2);ring=new
 m.fan(ring,(0,-1.416,-.237),2,'throat')
 ...
 for sign,patch in [(1,left),(-1,right)]:
  ...
  stations=[
   (0.00,(sign*.515,-1.035,-.169),.060,.095),
   (.06,(sign*.555,-.982,-.179),.064,.111),
   (.18,(sign*.617,-.865,-.199),.058,.125),
   (.35,(sign*.709,-.700,-.225),.049,.123),
   (.53,(sign*.800,-.532,-.250),.042,.111),
