...
reports=[];graphs=[];binds=[];action_signatures={}
for suffix in ('','.lod1'):
    path=OUT/('coccosteus'+suffix+'.glb');raw=path.read_bytes();n=struct.unpack_from('<I',raw,12)[0]
    ...
    action_signatures[suffix]=dict(zip(names,signatures))
    ...
    reports.append({'file':str(path),'sha256':sha(path),'bytes':len(raw),'triangles':tris,
                    ...
                    'materials':prims,'all_three_nested_anchors':True,'under_25mb_raw':len(raw)<25*1024*1024,'root_stable':True,
                    'no_scale_channels':True,'weights_normalized':True})
assert action_signatures['']==action_signatures['.lod1'],'Full/LOD action sample values differ'
...
assert reports[1]['triangles']==59194
assert sha(OUT/'coccosteus.glb')==report['fullCopiedByteExactlyFrom']['sha256']
assert report['lip_basis_repair']['nonBasisBytesPreserved'] and len(report['lip_basis_repair']['rows'])==4
from lip_basis_07 import GLB as LipGLB
full_lip=LipGLB((OUT/'coccosteus.glb').read_bytes()).lip();lod_lip=LipGLB((OUT/'coccosteus.lod1.glb').read_bytes()).lip()
for target in lod_lip:
    expected=next(v for v in full_lip if v['role']==target['role'] and max(abs(v['uv']-target['uv']))<2e-6)
    assert all(target['normal']==expected['normal']) and all(target['tangent']==expected['tangent'])
assert sha(OUT/'coccosteus.json')==sha(OUT.parent/'candidate-04/coccosteus.json')
...
print('COCCOSTEUS_EXPORT_STRUCTURE_PASS',[(r['bytes'],len(r['clips']))for r in reports])
