...
from body_uv_02 import assign as assign_body_uv, check_coverage
verify()
if BAKE.exists():raise RuntimeError('Preserve existing baked-02 evidence')
...
for oi,ob in enumerate(meshes):
    ...
    for channel,size in sizes.items():
        ...
        if kind=='body':coverage.append(check_coverage(image,channel))
...
s.cycles.samples=48
blend=BAKE/'coccosteus-baked-02.blend';bpy.ops.wm.save_as_mainfile(filepath=str(blend))
report={'phase':'bake candidate, exported appearance review pending','material_blend_sha256':SOURCE_SHA,
        ...
        'textures':textures,'materials':material_records,'linear_pigment':pigment,'body_uv':body_uv,'body_atlas_coverage':coverage,'files':[record(blend)]}
(BAKE/'bake-report.json').write_text(json.dumps(report,indent=2)+'\n');verify()
print('COCCOSTEUS_BAKE_02_COMPLETE',flush=True)
