...
from production_common_04 import *
...
OUT.mkdir();bpy.ops.wm.open_mainfile(filepath=str(BAKE/'coccosteus-baked-03.blend'));scene=bpy.context.scene
...
for ob,source in zip(parts,meshes):
    ...
    bpy.context.view_layer.objects.active=ob;dec=ob.modifiers.new('Coccosteus authored LOD','DECIMATE');dec.ratio=.25 if source is body else .70 if 'orbital study'in source.name else .45
    ...
    assert np.isfinite(values).all() and np.max(np.abs(values-1))<1e-5
    # Continuous body UVs provide adequate texel area; preserve posterior bars and fin rays.
    footprint=0.
...
blend=OUT/'coccosteus-production-04.blend';bpy.ops.wm.save_as_mainfile(filepath=str(blend))
...
print('COCCOSTEUS_CANDIDATE_04_COMPLETE',flush=True)
