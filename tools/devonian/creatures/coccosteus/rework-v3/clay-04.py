"""Coccosteus clay-04. Shared lip/collar boundary and one continuous oral lumen.
...
REPO=HERE.parents[4]
OUT=REPO.parent/'devonian-authoring/coccosteus/rework-v3/clay-04'
SCRIPT=Path(__file__).resolve()
VIEWS=HERE/'views-03.json'
BLEND=OUT/'coccosteus-clay-04.blend'
SCULPT=[]
...
FROZEN_ORAL_SHA='698be716534a6e5cf1a35c32d4946a0cbbfe45e2577dcee55664f280b91d9c79'
COLLAR_Y=-.72
THROAT_END=-.22


def base_torso_point(y,a):
...
def base_head_point(t,a):
    """Curved muzzle/crown/cheek surface ends at the actual shared thoracic collar."""
    p=lip_point(a);q=base_torso_point(COLLAR_Y,a)
    after=base_torso_point(COLLAR_Y+.0001,a)
    span=COLLAR_Y-p[1]
...
def head_point(t,a):
    # External-only refinement: the successful clay-03 lumen samples base_head_point.
    x,y,z=base_head_point(t,a)
    sn,cs=math.sin(a),math.cos(a)
    rise=.205*(1-math.exp(-t/.075))*(1-t)**2
    z+=rise*max(0,sn)**.72
    # Broad preorbital shoulders and cheek tissue keep the orbit on the flank.
    shoulder=.105*(1-math.exp(-t/.12))*(1-t)**1.8
    x+=shoulder*cs*(.42+.58*max(0,sn))
    cheek=.031*math.exp(-((y+1.34)/.17)**2-((z+.09)/.15)**2)*math.sin(math.pi*t)**2
    x+=(1 if cs>=0 else -1)*cheek*abs(cs)
    orbital_angle=math.asin(max(-1,min(1,sn)))
    socket=math.exp(-((t-.18)/.070)**2-((orbital_angle-.90)/.18)**2)
    x-=(1 if cs>=0 else -1)*.010*socket*abs(cs)
    z-=.006*socket*max(0,sn)
    # Narrow, curved cranial articulation is sculpted into the connected skin.
    line=-.94+.085*cs*cs
    groove=.018*math.exp(-((y-line)/.023)**2)*math.sin(math.pi*t)**2
    x-=groove*cs;z-=groove*sn
    # The cranial crown remains low, with a broader roof and a sloping cheek panel.
    z-=.016*math.exp(-((y+1.12)/.23)**2)*max(0,sn)*cs*cs*math.sin(math.pi*t)**2
    return (x,y,z)


def torso_point(y,a):
    x,yy,z=base_torso_point(y,a)
    sn,cs=math.sin(a),math.cos(a)
    fade=smooth((y-COLLAR_Y)/.16)*(1-smooth((y-.04)/.25))
    roof=.019*math.exp(-((y+.33)/.32)**4)*max(0,sn)**5
    panel=.020*math.exp(-((y+.33)/.33)**4)*math.exp(-((sn-.32)/.44)**2)
    x+=fade*panel*cs
    z+=fade*roof
    # A curved rear shield edge and broad underlying panel relief remain subtle.
    border=.14-.23*(1-sn*sn)*smooth((sn+.6)/.8)
    seam=.006*math.exp(-((y-border)/.021)**2)*fade
    x-=seam*cs;z-=seam*sn
    return (x,yy,z)
...
def inner_point(s,a):
    ...
    u=(s-join)/(1-join)
    q=inset(1)
    top=base_head_point(1,math.pi/2);bottom=base_head_point(1,3*math.pi/2)
...
def local_integrity(vv,ff,weights,parts):
    ...
    oral_indices=sorted({i for r in parts['innerRings'] for i in r}|{parts['throatPole']})
    oral_set=set(oral_indices)
    oral_faces=[face for face in ff if all(i in oral_set for i in face)]
    payload=json.dumps({'indices':oral_indices,'vertices':[vv[i] for i in oral_indices],
        'weights':[weights[i] for i in oral_indices],'faces':oral_faces},separators=(',',':')).encode()
    oral_sha=hashlib.sha256(payload).hexdigest()
    assert oral_sha==FROZEN_ORAL_SHA, 'Proven clay-03 oral geometry changed'
    return {'twoFaceEdges':True,'preservedClay03OralSha256':oral_sha,'minimumNonterminalOralWidth':minimum_width,
...
def build():
    ...
    # Parametric lateral orbit, chosen from the actual anatomical surface.
    # A fixed side ray no longer selects a narrow high forehead section.
    orbital_records=[]
    for sign in [-1,1]:
        t=.18;a=.90 if sign>0 else math.pi-.90
        surface=Vector(head_point(t,a))
        tangent_t=Vector(head_point(t+.0001,a))-Vector(head_point(t-.0001,a))
        tangent_a=Vector(head_point(t,a+.0001))-Vector(head_point(t,a-.0001))
        normal=tangent_a.cross(tangent_t).normalized()
        if normal.x*sign<0:normal=-normal
        assert normal.x*sign>.72, 'Orbital site is not lateral'
        center=surface-normal*.034
        ...
        o=bpy.context.object;o.name='Lateral anatomical orbital study '+str(sign)
        for vertex in o.data.vertices:
            q=vertex.co.copy();vertex.co=center+u*(q.x*.078)+v*(q.y*.071)+normal*(q.z*.060)
        ...
        for i,vertex in enumerate(o.data.vertices):key.data[i].co=moved(tuple(vertex.co),(1,0,0))
        orbital_records.append({'side':sign,'surface':list(surface),'center':list(center),'normal':list(normal),
            'radii':[.078,.071,.060],'parameter':[t,a]})
        SCULPT.append(o)
    for sign in [-1,1]:
    ...
    bpy.context.scene['candidate']='Coccosteus clay-04 PREVIEW; exterior cranial/cheek and lateral orbit refinement'
    bpy.context.scene['source_sha256']=sha(SCRIPT);bpy.context.scene['views_sha256']=sha(VIEWS)
    bpy.ops.wm.save_as_mainfile(filepath=str(BLEND))
    report={'stage':'build','candidate':'clay-04','sourceSha256':sha(SCRIPT),'viewsSha256':sha(VIEWS),
            'blend':str(BLEND),'blendSha256':sha(BLEND),'notApproved':True,'localConstructionChecks':check,'orbitalSites':orbital_records,
            'objects':[{'name':o.name,'vertices':len(o.data.vertices),'polygons':len(o.data.polygons)} for o in SCULPT]}
    (OUT/'build-report.json').write_text(json.dumps(report,indent=2)+'\n')
    print('COCCOSTEUS_CLAY_04_BUILD_COMPLETE',flush=True)
...
def render():
    ...
    print('COCCOSTEUS_CLAY_04_RENDER_COMPLETE',flush=True)
