"""Build only the structured LOD05, reusing accepted candidate04 rig/actions.
...
verify();assert not OUT.exists(),'Preserve existing candidate05'
...
for name in plan['materials']:
    mat=bpy.data.materials.new(name+' LOD structured05');mat.use_nodes=True;bs=mat.node_tree.nodes.get('Principled BSDF')
...
for row in plan['meshes']:
    ...
    pos=zup(a['positions']);faces=a['faces'];mesh=bpy.data.meshes.new(row['name']+' structured05');mesh.from_pydata(pos.tolist(),[],faces.tolist());mesh.update()
    ...
    uv=a['uv'].reshape(-1,2).copy();uv[:,1]=1-uv[:,1];layer=mesh.uv_layers.new(name='UVMap');layer.data.foreach_set('uv',uv.ravel())
...
report={'phase':'LOD05 source built; actual matched art review pending','source_sha256':sources(),
