...
path=HERE/'clay-03.py'
ns={'math':math}
keep={'smooth','pchip','torso_point','lip_point','hermite','head_point','inner_point','skin_weights','moved','surface_arrays','local_integrity'}
for n in ast.parse(path.read_text()).body:
    if isinstance(n,ast.FunctionDef) and n.name in keep:exec(compile(ast.Module([n],type_ignores=[]),str(path),'exec'),ns)
    elif isinstance(n,ast.Assign) and any(isinstance(t,ast.Name) and t.id in {'TORSO','COLLAR_Y','THROAT_END'} for t in n.targets):exec(compile(ast.Module([n],type_ignores=[]),str(path),'exec'),ns)
