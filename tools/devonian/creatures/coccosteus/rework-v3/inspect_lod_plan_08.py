...
folder=OUT/'field-plots-08';assert not folder.exists();folder.mkdir()
...
wrapped=np.ptp(q[:,:,1],axis=1)>64
q[wrapped,:,1]=np.where(q[wrapped,:,1]<64,q[wrapped,:,1]+128,q[wrapped,:,1])
assert np.ptp(q[:,:,1],axis=1).max()<16,'Invalid angular parameter span'
...
(OUT/'plan-validation-08.json').write_text(json.dumps(validation,indent=2)+'\n');print(json.dumps(validation,indent=2))
