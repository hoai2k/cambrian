...
const root=path.resolve(import.meta.dirname,'../../../..'),local=path.resolve(root,'../devonian-authoring/coccosteus'),out=path.join(local,lod?'v2-eye-audit-lod':'v2-eye-audit');await fs.mkdir(out,{recursive:true});
const file=path.join(local,'v2-candidate/coccosteus'+(lod?'.lod1':'')+'.glb'),bytes=await fs.readFile(file),doc=await new NodeIO().readBinary(bytes),meshes=[];
...
await fs.writeFile(path.join(out,'coccosteus.geometry.json'),JSON.stringify({id:'coccosteus',revision:'V2 candidate',sha256:createHash('sha256').update(bytes).digest('hex'),meshes}));
await fs.writeFile(path.join(out,'selectors.json'),JSON.stringify({coccosteus:{headMesh:'head_envelope_closed',eyeMeshes:['eye_globe_L','eye_globe_R']}}));
