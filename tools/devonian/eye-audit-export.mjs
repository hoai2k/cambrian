...
const args=process.argv.slice(3), lod=args.includes('--lod');
const requested=args.filter(a=>a!=='--lod');
const label=(working?'working-'+new Date().toISOString().replace(/[:.]/g,'-'):revision)+(lod?'-lod1':'');
const out=path.resolve(root,'../devonian-authoring/eye-audit',label); await fs.mkdir(out,{recursive:true});
const ids=requested.length?requested:JSON.parse(execFileSync('git',['show',`${working?'HEAD':revision}:tools/devonian/shipped.json`],{cwd:root})).creatures;
...
for(const id of ids){
 const asset=`public/assets/devonian/creatures/${id}${lod?'.lod1':''}.glb`, bytes=working?await fs.readFile(path.join(root,asset)):execFileSync('git',['show',`${revision}:${asset}`],{cwd:root,maxBuffer:100*1024*1024});
 ...
 await fs.writeFile(path.join(out,`${id}.geometry.json`),JSON.stringify({id,revision:label,asset,sha256:createHash('sha256').update(bytes).digest('hex'),coordinateSystem:'glTF +Y up +Z forward, rest/bind mesh world space',meshes}));
